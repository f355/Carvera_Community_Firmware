...
extern "C" void _start(void) {
  ...
  if (MRI_ENABLE) {
    ...
    if (MRI_BREAK_ON_INIT) __debugbreak();
  }
...
static unsigned int alignTo32Bytes(unsigned int value) { return (value + 31) & ~31; }
...
extern "C" int __wrap__read(int file, char* ptr, int len) {
  if (MRI_SEMIHOST_STDIO && file < 3) return mriNewlib_SemihostRead(file, ptr, len);
...
extern "C" int __wrap__write(int file, char* ptr, int len) {
  if (MRI_SEMIHOST_STDIO && file < 3) return mriNewlib_SemihostWrite(file, ptr, len);
...
static int doesHeapCollideWithStack(unsigned int newHeap) {
  return ((newHeap >= __get_MSP()) || (STACK_SIZE && newHeap >= g_maximumHeapAddress));
}
...
extern "C" void* mallocWithTag(size_t size, unsigned int tag) {
  ...
  if (!p && __smoothieHeapBase) return p;
  setTag(p, tag);
  return p;
}
...
extern "C" void __wrap_free(void* ptr) {
  if (!isChunkInUse(ptr)) __debugbreak();
  __real_free(ptr);
}
