...
int LocalFileHandle::isatty() { return semihost_istty(_fh); }
...
int LocalFileHandle::fsync() { return semihost_ensure(_fh); }

off_t LocalFileHandle::flen() { return semihost_flen(_fh); }
...
class LocalDirHandle : public DirHandle {
 public:
  ...
  virtual void rewinddir() { info.fileID = 0; }

  virtual off_t telldir() { return info.fileID; }

  virtual void seekdir(off_t offset) { info.fileID = offset; }
};
...
int LocalFileSystem::remove(const char* filename) { return semihost_remove(filename); }

DirHandle* LocalFileSystem::opendir(const char* name) { return new LocalDirHandle(); }
