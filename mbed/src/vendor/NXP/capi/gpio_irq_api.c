...
static void handle_interrupt_in(void) {
  ...
  for (i = 0; i < 32; i++) {
    ...
    if (rise0 & pmask) {
      mask0 |= pmask;
      if (channel_ids[i] != 0) irq_handler(channel_ids[i], IRQ_RISE);
    }
    if (fall0 & pmask) {
      mask0 |= pmask;
      if (channel_ids[i] != 0) irq_handler(channel_ids[i], IRQ_FALL);
    }
  }
  ...
  for (i = 0; i < 16; i++) {
    ...
    if (rise2 & pmask) {
      mask2 |= pmask;
      if (channel_ids[channel_index] != 0) irq_handler(channel_ids[channel_index], IRQ_RISE);
    }
    if (fall2 & pmask) {
      mask2 |= pmask;
      if (channel_ids[channel_index] != 0) irq_handler(channel_ids[channel_index], IRQ_FALL);
    }
  }
...
static inline void handle_interrupt_in(uint32_t channel) {
  ...
  if (((LPC_GPIO_PIN_INT->IST & ch_bit) == 0) || (channel_ids[channel] == 0) || (LPC_GPIO_PIN_INT->ISEL & ch_bit))
    return;
...
void gpio_irq_set(gpio_irq_t* obj, gpio_irq_event event, uint32_t enable) {
  ...
  // Clear interrupt
  if (!(LPC_GPIO_PIN_INT->ISEL & ch_bit)) LPC_GPIO_PIN_INT->IST = ch_bit;
