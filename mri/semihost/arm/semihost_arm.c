...
static int handleArmSemihostFileLengthRequest(PlatformSemihostParameters* pSemihostParameters) {
  ...
  if (returnValue && GetSemihostReturnCode() == 0) {
    ...
    Platform_SetSemihostCallReturnAndErrnoValues(extractWordFromBigEndianByteArray(&gdbFileStats.totalSizeLowerWord),
                                                 0);
  }
...
static uint32_t extractWordFromBigEndianByteArray(const void* pBigEndianValueToExtract) {
  ...
  return pBigEndianValue[3] | (pBigEndianValue[2] << 8) | (pBigEndianValue[1] << 16) | (pBigEndianValue[0] << 24);
}
...
static int handleMbedSemihostUidRequest(PlatformSemihostParameters* pParameters) {
  ...
  */
  ...
  if (copySize > uidSize) copySize = uidSize;
