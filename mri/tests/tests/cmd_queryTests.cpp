...
TEST(cmdQuery, QuerySupported) {
  ...
  STRCMP_EQUAL(
      platformMock_CommChecksumData("$T05responseT#"
...
TEST(cmdQuery, QueryXferFeatures_ReadAllData) {
  ...
  mriDebugException(platformMock_GetContext());
  STRCMP_EQUAL(platformMock_CommChecksumData("$T05responseT#+$mtest!#+"), platformMock_CommGetTransmittedData());
}
...
TEST(cmdQuery, QueryRcmd_Reset_ShouldOutputMessage_MakesSureAckTransmitCompleted_CallsPlatformResetDevice) {
  ...
  STRCMP_EQUAL(platformMock_CommChecksumData(expectedTransmitData), platformMock_CommGetTransmittedData());
...
TEST(cmdQuery, QueryRcmd_Help_ShouldDisplaySupportedCommands) {
  ...
  snprintf(expectedTransmitData, sizeof(expectedTransmitData), "$T05responseT#+$O%s#$O%s#$O%s#$OK#+",
           expectedConsoleOutput[0], expectedConsoleOutput[1], expectedConsoleOutput[2]);
  STRCMP_EQUAL(platformMock_CommChecksumData(expectedTransmitData), platformMock_CommGetTransmittedData());
}
...
TEST(cmdQuery, QueryRcmd_UnknownMonitorCommand_ShouldDisplayErrorAndHelp) {
  ...
  snprintf(expectedTransmitData, sizeof(expectedTransmitData), "$T05responseT#+$O%s#$O%s#$O%s#$O%s#$OK#+",
           expectedConsoleOutput[0], expectedConsoleOutput[1], expectedConsoleOutput[2], expectedConsoleOutput[3]);
  STRCMP_EQUAL(platformMock_CommChecksumData(expectedTransmitData), platformMock_CommGetTransmittedData());
}
