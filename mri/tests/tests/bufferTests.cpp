...
TEST_GROUP(Buffer) {
  ...
  void allocateBuffer(size_t sizeOfBuffer) {
    ...
    if (sizeOfBuffer > 0) memset(m_pCharacterArray, m_fillChar, sizeOfBuffer);
  ...
  void validateBufferLimits() {
    if (!m_validateBufferLimits) return;

    POINTERS_EQUAL(m_pCharacterArray, m_buffer.pStart);
    POINTERS_EQUAL(m_pCharacterArray + m_bufferSize, m_buffer.pEnd);
  }

  void validateResetBuffer() { POINTERS_EQUAL(m_pCharacterArray, m_buffer.pCurrent); }
...
TEST(Buffer, Buffer_ByteLeft_Overrun_NoThrow) {
  ...
  __catch FAIL("Buffer_BytesLeft() threw exception.");
