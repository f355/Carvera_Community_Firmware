...
typedef struct {         /* EMC Structure          */
  ...
  __IO uint32_t STATICWAITOEN0; /* Selects the delay from chip select n or address change, whichever is later, to output
                                   enable. */
  __IO uint32_t STATICWAITRD0;  /* Selects the delay from chip select n to a read access. */
  __IO uint32_t
      STATICWAITPAG0;          /* Selects the delay for asynchronous page mode sequential accesses for chip select n. */
  ...
  __IO uint32_t STATICWAITOEN1; /* Selects the delay from chip select n or address change, whichever is later, to output
                                   enable. */
  __IO uint32_t STATICWAITRD1;  /* Selects the delay from chip select n to a read access. */
  __IO uint32_t
      STATICWAITPAG1;          /* Selects the delay for asynchronous page mode sequential accesses for chip select n. */
  ...
  __IO uint32_t STATICWAITOEN2; /* Selects the delay from chip select n or address change, whichever is later, to output
                                   enable. */
  __IO uint32_t STATICWAITRD2;  /* Selects the delay from chip select n to a read access. */
  __IO uint32_t
      STATICWAITPAG2;          /* Selects the delay for asynchronous page mode sequential accesses for chip select n. */
  ...
  __IO uint32_t STATICWAITOEN3; /* Selects the delay from chip select n or address change, whichever is later, to output
                                   enable. */
  __IO uint32_t STATICWAITRD3;  /* Selects the delay from chip select n to a read access. */
  __IO uint32_t
      STATICWAITPAG3;          /* Selects the delay for asynchronous page mode sequential accesses for chip select n. */
 ...
 */
typedef enum CGU_USB_AUDIO_PLL { CGU_USB_PLL, CGU_AUDIO_PLL } CGU_USB_AUDIO_PLL_T;
...
typedef struct { /* WWDT Structure         */
  ...
  __O uint32_t FEED; /* Watchdog feed sequence register. Writing 0xAA followed by 0x55 to this register reloads the
  ...
  __IO uint32_t WARNINT; /* Watchdog warning interrupt register. This register contains the Watchdog warning interrupt
                            compare value. */
...
typedef struct { /* USARTn Structure       */
  ...
  union {
    __IO uint32_t DLL; /* Divisor Latch LSB. Least significant byte of the baud rate divisor value. The full divisor is
                          used to generate a baud rate from the fractional rate divider (DLAB = 1). */
  ...
  union {
    __IO uint32_t IER; /* Interrupt Enable Register. Contains individual interrupt enable bits for the 7 potential UART
                          interrupts (DLAB = 0). */
    __IO uint32_t DLM; /* Divisor Latch MSB. Most significant byte of the baud rate divisor value. The full divisor is
                          used to generate a baud rate from the fractional rate divider (DLAB = 1). */
  };
  ...
  __IO uint32_t
      OSR; /* Oversampling Register. Controls the degree of oversampling during each bit time. Only on some UARTS. */
...
typedef struct {     /* TIMERn Structure       */
  ...
  __IO uint32_t TC;  /* Timer Counter. The 32 bit TC is incremented every PR+1 cycles of PCLK. The TC is controlled
                        through the TCR. */
  ...
  __IO uint32_t PC; /* Prescale Counter. The 32 bit PC is a counter which is incremented to the value stored in PR. When
                       the value in PR is reached, the TC is incremented and the PC is cleared. The PC is observable and
                       controllable through the bus interface. */
  __IO uint32_t MCR; /* Match Control Register. The MCR is used to control if an interrupt is generated and if the TC is
                        reset when a Match occurs. */
  __IO uint32_t MR[4]; /* Match Register. MR can be enabled through the MCR to reset the TC, stop both the TC and PC,
                          and/or generate an interrupt every time MR matches the TC. */
  __IO uint32_t
      CCR; /* Capture Control Register. The CCR controls which edges of the capture inputs are used to load the Capture
              Registers and whether or not an interrupt is generated when a capture takes place. */
  __IO uint32_t
      CR[4]; /* Capture Register. CR is loaded with the value of TC when there is an event on the CAPn.0 input. */
  __IO uint32_t EMR; /* External Match Register. The EMR controls the external match pins MATn.0-3 (MAT0.0-3 and
                        MAT1.0-3 respectively). */
  __I uint32_t RESERVED0[12];
  __IO uint32_t CTCR; /* Count Control Register. The CTCR selects between Timer and Counter mode, and in Counter mode
...
#define I2C0_FAST_MODE_PLUS \
  (2 << 1 | 1 << 3 | 1 << 7 | 1 << 10 | 1 << 11) /* Pin configuration for Fast-mode Plus I2C */
...
typedef struct {        /* I2C0 Structure         */
  __IO uint32_t CONSET; /* I2C Control Set Register. When a one is written to a bit of this register, the corresponding
                           bit in the I2C control register is set. Writing a zero has no effect on the corresponding bit
                           in the I2C control register. */
  __I uint32_t STAT;    /* I2C Status Register. During I2C operation, this register provides detailed status codes that
                           allow software to determine the next action needed. */
  __IO uint32_t DAT;    /* I2C Data Register. During master or slave transmit mode, data to be transmitted is written to
                           this register. During master or slave receive mode, data that has been received may be read from
                           this register. */
  __IO uint32_t ADR0;   /* I2C Slave Address Register 0. Contains the 7-bit slave address for operation of the I2C
                           interface in slave mode, and is not used in master mode. The least significant bit determines
  ...
  __IO uint32_t
      SCLL; /* SCL Duty Cycle Register Low Half Word. Determines the low time of the I2C clock. SCLL and SCLH together
               determine the clock frequency generated by an I2C master and certain times used in slave mode. */
  __O uint32_t CONCLR; /* I2C Control Clear Register. When a one is written to a bit of this register, the corresponding
                          bit in the I2C control register is cleared. Writing a zero has no effect on the corresponding
                          bit in the I2C control register. */
  __IO uint32_t MMCTRL; /* Monitor mode control register. */
  __IO uint32_t ADR1; /* I2C Slave Address Register. Contains the 7-bit slave address for operation of the I2C interface
                         in slave mode, and is not used in master mode. The least significant bit determines whether a
                         slave responds to the General Call address. */
  __IO uint32_t ADR2; /* I2C Slave Address Register. Contains the 7-bit slave address for operation of the I2C interface
                         in slave mode, and is not used in master mode. The least significant bit determines whether a
                         slave responds to the General Call address. */
  __IO uint32_t ADR3; /* I2C Slave Address Register. Contains the 7-bit slave address for operation of the I2C interface
                         in slave mode, and is not used in master mode. The least significant bit determines whether a
                         slave responds to the General Call address. */
  __I uint32_t DATA_BUFFER; /* Data buffer register. The contents of the 8 MSBs of the DAT shift register will be
                               transferred to the DATA_BUFFER automatically after every nine bits (8 bits of data plus
                               ACK or NACK) has been received on the bus. */
...
typedef struct {       /* I2S Structure */
  ...
  __IO uint32_t IRQ; /* I2S Interrupt Request Control Register. Contains bits that control how the I2S interrupt request
                        is generated */
  __IO uint32_t TXRATE;    /* I2S Transmit MCLK divider. This register determines the I2S TX MCLK rate by specifying the
                              value to divide PCLK by in order to produce MCLK */
  __IO uint32_t RXRATE;    /* I2S Receive MCLK divider. This register determines the I2S RX MCLK rate by specifying the
                              value to divide PCLK by in order to produce MCLK */
  __IO uint32_t TXBITRATE; /* I2S Transmit bit rate divider. This register determines the I2S transmit bit rate by
                              specifying the value to divide TX_MCLK by in order to produce the transmit bit clock */
  __IO uint32_t RXBITRATE; /* I2S Receive bit rate divider. This register determines the I2S receive bit rate by
                              specifying the value to divide RX_MCLK by in order to produce the receive bit clock */
...
typedef struct {         /* RITIMER Structure      */
  ...
  __IO uint32_t MASK; /* Mask register. This register holds the 32-bit mask value. A 1 written to any bit will force a
...
typedef struct {    /* ADCn Structure */
  __IO uint32_t CR; /* A/D Control Register. The AD0CR register must be written to select the operating mode before A/D
                       conversion can occur. */
  __I uint32_t GDR; /* A/D Global Data Register. Contains the result of the most recent A/D conversion. */
  __I uint32_t RESERVED0;
  __IO uint32_t
      INTEN; /* A/D Interrupt Enable Register. This register contains enable bits that allow the DONE flag of each A/D
                channel to be included or excluded from contributing to the generation of an A/D interrupt. */
  __I uint32_t DR[8]; /* A/D Channel Data Register. This register contains the result of the most recent conversion
                         completed on channel n. */
  __I uint32_t STAT;  /* A/D Status Register. This register contains DONE and OVERRUN flags for all of the A/D channels,
                         as well as the A/D interrupt flag. */
} LPC_ADC_T;
...
#define ADC_CR_CLKDIV(n)                                                                                             \
  ((((n) & 0xFF) << 8)) /* The APB clock (PCLK) is divided by (this value plus one) to produce the clock for the A/D \
                         */
...
#define ADC_CR_START_MCOA2 \
  ((6UL << 24)) /* Start conversion when the edge selected by bit 27 occurs on Motocon PWM output MCOA2 */
...
typedef struct {    /* SPI Structure          */
  ...
  __IO uint32_t
      DR; /* SPI Data Register. This bi-directional register provides the transmit and receive data for the SPI. */
...
#define SPI_CR_LSB_FIRST_EN \
  ((uint32_t)(1 << 6)) /* Data will be transmitted and received in reverse order (LSB first).*/
...
typedef struct {                   /* SGPIO Structure        */
  ...
  __IO uint32_t REG[16];    /* Slice data registers. Eachtime COUNT0 reaches 0x0 the register shifts loading bit 31
                               withdata captured from DIN(n). DOUT(n) is set to REG(0) */
  __IO uint32_t REG_SS[16]; /* Slice data shadow registers. Each time POSreaches 0x0 the contents of REG_SS is exchanged
                               with the contentof REG */
