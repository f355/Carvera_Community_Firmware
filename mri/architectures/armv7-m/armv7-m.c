...
static const uint32_t g_fakeStack[] = {
    0xDEADDEAD, 0xDEADDEAD, 0xDEADDEAD, 0xDEADDEAD, 0xDEADDEAD, 0xDEADDEAD, 0xDEADDEAD, 0xDEADDEAD,
...
static void clearState(void) { mri_memset(&mriCortexMState, 0, sizeof(mriCortexMState)); }
...
static void determinePriorityBitShift(void) {
  ...
  if (zeroBitCount > priorityBitShift) priorityBitShift = zeroBitCount;
...
static void defaultExternalInterruptsToLowerPriority(uint8_t priority, uint32_t highestExternalIrq) {
  ...
  for (irq = 0; irq <= highestExternalIrq; irq++) mriCortexMSetPriority((uint32_t)irq, priority, 0);
}

static void disableDebuggerStack(void) { mriCortexMFlags |= CORTEXM_FLAGS_NO_DEBUG_STACK; }
...
void mriCortexMSetPriority(uint32_t irq, uint8_t priority, uint8_t subPriority) {
  uint8_t fullPriority =
      (priority << mriCortexMState.priorityBitShift) | (subPriority & ((1 << mriCortexMState.priorityBitShift) - 1));
...
static uint32_t shouldRestorePriorityRegs(void) { return mriCortexMFlags & CORTEXM_FLAGS_RESTORE_PRI; }

static void clearPriorityRestoreFlag() { mriCortexMFlags &= ~CORTEXM_FLAGS_RESTORE_PRI; }
...
static int shouldRemoveHardwareBreakpointOnSvcHandler(void) { return mriCortexMFlags & CORTEXM_FLAGS_SVC_STEP; }

static void clearSvcStepFlag(void) { mriCortexMFlags &= ~CORTEXM_FLAGS_SVC_STEP; }
...
static void clearSingleSteppingFlag(void) { mriCortexMFlags &= ~CORTEXM_FLAGS_SINGLE_STEPPING; }
...
static void setHardwareBreakpointOnSvcHandler(void) { Platform_SetHardwareBreakpoint(getNvicVector(SVCall_IRQn) & ~1); }

static void setSvcStepFlag(void) { mriCortexMFlags |= CORTEXM_FLAGS_SVC_STEP; }

static void setSingleSteppingFlag(void) { mriCortexMFlags |= CORTEXM_FLAGS_SINGLE_STEPPING; }
...
static int checkCurrentInstruction(void) {
  ...
  if (isInstructionMSR(firstWord, secondWord)) return processInstructionMSR(firstWord, secondWord);
  if (isInstructionCPS(firstWord)) return processInstructionCPS(firstWord);
  /* MRS instructions might want to read the real values of PRIMASK and BASEPRI and not the values as modified
     by MRI. */
  if (isInstructionMRS(firstWord, secondWord)) return processInstructionMRS(firstWord, secondWord);
  return 0;
}

static uint16_t getFirstHalfWordOfCurrentInstruction(void) { return throwingMemRead16(Platform_GetProgramCounter()); }
...
static uint16_t throwingMemRead16(uint32_t address) {
  ...
  if (Platform_WasMemoryFaultEncountered()) __throw_and_return(memFaultException, 0);
...
static int isInstructionCPS(uint16_t firstWord) { return (firstWord & 0xFFEC) == 0xB660; }
...
static void setRestorePriorityRegsFlag(void) { mriCortexMFlags |= CORTEXM_FLAGS_RESTORE_PRI; }
...
int Platform_IsSingleStepping(void) { return mriCortexMFlags & CORTEXM_FLAGS_SINGLE_STEPPING; }

char* Platform_GetPacketBuffer(void) { return mriCortexMState.packetBuffer; }

size_t Platform_GetPacketBufferSize(void) { return sizeof(mriCortexMState.packetBuffer); }
...
static uint32_t encounteredStackingException(void) { return mriCortexMState.cfsr & CFSR_STACK_ERROR_BITS; }
...
static uint32_t hasControlCBeenDetected() { return mriCortexMFlags & CORTEXM_FLAGS_CTRL_C; }
...
static uint8_t determineCauseOfDebugEvent(void) {
  static struct {
    ...
    unsigned char signalToReturn;
      ...
      {SCB_DFSR_EXTERNAL, SIGSTOP}, {SCB_DFSR_DWTTRAP, SIGTRAP}, {SCB_DFSR_BKPT, SIGTRAP}, {SCB_DFSR_HALTED, SIGTRAP}};
  uint32_t debugFaultStatus = mriCortexMState.dfsr;
  size_t i;
...
static void displayHardFaultCauseToGdbConsole(void) {
  ...
  if (hardFaultStatusRegister & debugEventBit) WriteStringToGdbConsole("\n    Debug Event");

  if (hardFaultStatusRegister & vectorTableReadBit) WriteStringToGdbConsole("\n    Vector Table Read");
...
static void displayMemFaultCauseToGdbConsole(void) {
  ...
  if (memManageFaultStatusRegister == 0) return;
  ...
  if (memManageFaultStatusRegister & FPLazyStatePreservationBit) WriteStringToGdbConsole("\n    FP Lazy Preservation");
  ...
  if (memManageFaultStatusRegister & dataAccess) WriteStringToGdbConsole("\n    Data Access");

  if (memManageFaultStatusRegister & instructionFetch) WriteStringToGdbConsole("\n    Instruction Fetch");
}
...
static void displayBusFaultCauseToGdbConsole(void) {
  ...
  if (busFaultStatusRegister == 0) return;
  ...
  if (busFaultStatusRegister & FPLazyStatePreservationBit) WriteStringToGdbConsole("\n    FP Lazy Preservation");
  ...
  if (busFaultStatusRegister & impreciseDataAccessBit) WriteStringToGdbConsole("\n    Imprecise Data Access");

  if (busFaultStatusRegister & preciseDataAccessBit) WriteStringToGdbConsole("\n    Precise Data Access");

  if (busFaultStatusRegister & instructionPrefetch) WriteStringToGdbConsole("\n    Instruction Prefetch");
}
...
static void displayUsageFaultCauseToGdbConsole(void) {
  ...
  if (usageFaultStatusRegister == 0) return;

  WriteStringToGdbConsole("\n**Usage Fault**");
  WriteStringToGdbConsole("\n  Status Register: ");
  WriteHexValueToGdbConsole(usageFaultStatusRegister);

  if (usageFaultStatusRegister & divideByZeroBit) WriteStringToGdbConsole("\n    Divide by Zero");

  if (usageFaultStatusRegister & unalignedBit) WriteStringToGdbConsole("\n    Unaligned Access");

  if (usageFaultStatusRegister & coProcessorAccessBit) WriteStringToGdbConsole("\n    Coprocessor Access");

  if (usageFaultStatusRegister & invalidPCBit) WriteStringToGdbConsole("\n    Invalid Exception Return State");

  if (usageFaultStatusRegister & invalidStateBit) WriteStringToGdbConsole("\n    Invalid State");

  if (usageFaultStatusRegister & undefinedInstructionBit) WriteStringToGdbConsole("\n    Undefined Instruction");
}
...
void Platform_EnteringDebugger(void) {
  ...
  Platform_DisableSingleStep();
  if (isExternalInterrupt(mriCortexMState.exceptionNumber)) setControlCFlag();
  setActiveDebugFlag();
}

static void clearMemoryFaultFlag(void) { mriCortexMFlags &= ~CORTEXM_FLAGS_FAULT_DURING_DEBUG; }
...
static void setControlCFlag(void) { mriCortexMFlags |= CORTEXM_FLAGS_CTRL_C; }

static void setActiveDebugFlag(void) { mriCortexMFlags |= CORTEXM_FLAGS_ACTIVE_DEBUG; }
...
static void clearControlCFlag(void) { mriCortexMFlags &= ~CORTEXM_FLAGS_CTRL_C; }

static void clearActiveDebugFlag(void) { mriCortexMFlags &= ~CORTEXM_FLAGS_ACTIVE_DEBUG; }

static void clearPendedFromFaultFlag(void) { mriCortexMFlags &= ~CORTEXM_FLAGS_PEND_FROM_FAULT; }
...
static void checkStack(void) {
  ...
  while ((uint8_t*)pCurr < pEnd && *pCurr == CORTEXM_DEBUGGER_STACK_FILL) pCurr++;

  spaceUsed = pEnd - (uint8_t*)pCurr;
  if (spaceUsed > mriCortexMState.maxStackUsed) mriCortexMState.maxStackUsed = spaceUsed;
}

uint32_t Platform_GetProgramCounter(void) { return Context_Get(&mriCortexMState.context, PC); }

void Platform_SetProgramCounter(uint32_t newPC) { Context_Set(&mriCortexMState.context, PC, newPC); }
...
static int isInstruction32Bit(uint16_t firstWordOfInstruction) {
  ...
  return (maskedOffUpper5BitsOfWord == 0xE800 || maskedOffUpper5BitsOfWord == 0xF000 ||
...
static void writeBytesToBufferAsHex(Buffer* pBuffer, void* pBytes, size_t byteCount) {
  ...
  for (i = 0; i < byteCount; i++) Buffer_WriteByteAsHex(pBuffer, *pByte++);
}
...
void Platform_SetHardwareBreakpointOfGdbKind(uint32_t address, uint32_t kind) {
  ...
  if (!pFPBBreakpointComparator) __throw(exceededHardwareResourcesException);
}
...
void Platform_SetHardwareWatchpoint(uint32_t address, uint32_t size, PlatformWatchpointType type) {
  ...
  if (!isValidDWTComparatorSetting(address, size, nativeType)) __throw(invalidArgumentException);

  pComparator = enableDWTWatchpoint(address, size, nativeType);
  if (!pComparator) __throw(exceededHardwareResourcesException);
}
...
void Platform_ResetDevice(void) { NVIC_SystemReset(); }
...
void Platform_SyncICacheToDCache(uintmri_t address, size_t size) {
  if (size == 0 || levelOfCacheUnification() == 0) return;
...
static void invalidateAllBranchPredictions(void) { SCB_BPIALL = 1; }
...
static void setFaultDetectedFlag(void) { mriCortexMFlags |= CORTEXM_FLAGS_FAULT_DURING_DEBUG; }
...
static int wasFaultCausedByStackingException(void) { return SCB->CFSR & CFSR_STACK_ERROR_BITS; }
...
static void setPendedFromFaultBit(void) { mriCortexMFlags |= CORTEXM_FLAGS_PEND_FROM_FAULT; }
...
static int prepareThreadContext(ExceptionStack* pExceptionStack, IntegerRegisters* pIntegerRegs,
                                uint32_t* pFloatingRegs);
...
void mriCortexMExceptionHandler(IntegerRegisters* pIntegerRegs, uint32_t* pFloatingRegs) {
  ...
  if (encounteredStackingException()) pExceptionStack = (ExceptionStack*)g_fakeStack;
...
static int wasPendedFromFault(void) { return mriCortexMFlags & CORTEXM_FLAGS_PEND_FROM_FAULT; }
