...
static __INLINE int isDebuggerAttached(void) { return (CoreDebug->DHCSR & CoreDebug_DHCSR_C_DEBUGEN); }

static __INLINE void waitForDebuggerToDetach(uint32_t timeOut) {
  while (timeOut-- > 0 && isDebuggerAttached()) {
  }

  if (isDebuggerAttached()) __throw(timeoutException);
}

static __INLINE void enableDebugMonitor(void) { CoreDebug->DEMCR |= CoreDebug_DEMCR_MON_EN; }

static __INLINE void enableDWTandITM(void) { CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA; }

static __INLINE void disableDWTandITM(void) { CoreDebug->DEMCR &= ~CoreDebug_DEMCR_TRCENA; }

static __INLINE void disableSingleStep(void) { CoreDebug->DEMCR &= ~CoreDebug_DEMCR_MON_STEP; }

static __INLINE void enableSingleStep(void) { CoreDebug->DEMCR |= CoreDebug_DEMCR_MON_STEP; }

static __INLINE void clearMonitorPending(void) { CoreDebug->DEMCR &= ~CoreDebug_DEMCR_MON_PEND; }

static __INLINE void setMonitorPending(void) { CoreDebug->DEMCR |= CoreDebug_DEMCR_MON_PEND; }

static __INLINE uint32_t isMonitorPending(void) { return CoreDebug->DEMCR & CoreDebug_DEMCR_MON_PEND; }
...
static __INLINE uint32_t getDWTComparatorCount(void) { return (DWT->CTRL >> 28); }

static __INLINE void clearDWTComparator(DWT_COMP_Type* pComparatorStruct) {
  pComparatorStruct->COMP = 0;
  pComparatorStruct->MASK = 0;
  pComparatorStruct->FUNCTION &= ~(DWT_COMP_FUNCTION_DATAVMATCH | DWT_COMP_FUNCTION_CYCMATCH |
                                   DWT_COMP_FUNCTION_EMITRANGE | DWT_COMP_FUNCTION_FUNCTION_MASK);
}
...
static __INLINE void initDWT(void) { clearDWTComparators(); }
...
static __INLINE uint32_t maskOffDWTFunctionBits(uint32_t functionValue) {
  return functionValue & (DWT_COMP_FUNCTION_DATAVADDR1 | DWT_COMP_FUNCTION_DATAVADDR0 |
                          DWT_COMP_FUNCTION_DATAVSIZE_MASK | DWT_COMP_FUNCTION_DATAVMATCH | DWT_COMP_FUNCTION_CYCMATCH |
                          DWT_COMP_FUNCTION_EMITRANGE | DWT_COMP_FUNCTION_FUNCTION_MASK);
}
...
static __INLINE int doesDWTComparatorMatch(DWT_COMP_Type* pComparator, uint32_t address, uint32_t size,
                                           uint32_t function) {
  return doesDWTComparatorFunctionMatch(pComparator, function) && doesDWTComparatorAddressMatch(pComparator, address) &&
         doesDWTComparatorMaskMatch(pComparator, size);
}

static __INLINE DWT_COMP_Type* findDWTComparator(uint32_t watchpointAddress, uint32_t watchpointSize,
...
static __INLINE int isPowerOf2(uint32_t value) { return (value & (value - 1)) == 0; }
...
static __INLINE int isValidDWTComparatorSize(uint32_t watchpointSize) { return isPowerOf2(watchpointSize); }
...
static __INLINE int isValidDWTComparatorSetting(uint32_t watchpointAddress, uint32_t watchpointSize,
                                                uint32_t watchpointType) {
  return isValidDWTComparatorSize(watchpointSize) && isValidDWTComparatorAddress(watchpointAddress, watchpointSize) &&
...
static __INLINE int attemptToSetDWTComparator(DWT_COMP_Type* pComparator, uint32_t watchpointAddress,
                                              uint32_t watchpointSize, uint32_t watchpointType) {
  if (!attemptToSetDWTComparatorMask(pComparator, watchpointSize)) return 0;
...
static __INLINE DWT_COMP_Type* enableDWTWatchpoint(uint32_t watchpointAddress, uint32_t watchpointSize,
                                                   uint32_t watchpointType) {
...
static __INLINE DWT_COMP_Type* disableDWTWatchpoint(uint32_t watchpointAddress, uint32_t watchpointSize,
                                                    uint32_t watchpointType) {
  DWT_COMP_Type* pComparator = NULL;
...
static __INLINE uint32_t getFPBCodeComparatorCount(void) {
  uint32_t controlValue = FPB->CTRL;
  return (((controlValue & FP_CTRL_NUM_CODE_MSB_MASK) >> 8) | ((controlValue & FP_CTRL_NUM_CODE_LSB_MASK) >> 4));
}
...
static __INLINE void clearFPBComparator(uint32_t* pComparator) { *pComparator = 0; }

static __INLINE int isAddressAboveLowestHalfGig(uint32_t address) { return (int)(address & 0xE0000000); }

static __INLINE int isAddressOdd(uint32_t address) { return (int)(address & 0x1); }
...
static __INLINE int isAddressInUpperHalfword(uint32_t address) { return (int)(address & 0x2); }
...
static __INLINE uint32_t calculateFPBComparatorValue(uint32_t breakpointAddress, int32_t is32BitInstruction) {
  if (isBreakpointAddressInvalid(breakpointAddress)) return (uint32_t)~0U;
  if (getFPBRevision() == FP_CTRL_REVISION2)
...
static __INLINE int isFPBComparatorEnabledRevision1(uint32_t comparator) { return (int)(comparator & FP_COMP_ENABLE); }
...
static __INLINE uint32_t* findFPBBreakpointComparator(uint32_t breakpointAddress, int32_t is32BitInstruction) {
  ...
  for (i = 0; i < codeComparatorCount; i++) {
    ...
    if (comparatorValueForThisBreakpoint == maskOffReservedBits) return pCurrentComparator;
...
static __INLINE uint32_t* findFreeFPBBreakpointComparator(void) {
  ...
  codeComparatorCount = getFPBCodeComparatorCount();
  for (i = 0; i < codeComparatorCount; i++) {
    if (!isFPBComparatorEnabled(*pCurrentComparator)) return pCurrentComparator;
...
static __INLINE uint32_t* disableFPBBreakpointComparator(uint32_t breakpointAddress, int32_t is32BitInstruction) {
  ...
  pExistingFPBBreakpoint = findFPBBreakpointComparator(breakpointAddress, is32BitInstruction);
  if (pExistingFPBBreakpoint) clearFPBComparator(pExistingFPBBreakpoint);
...
static __INLINE void enableFPB(void) { FPB->CTRL |= (FP_CTRL_KEY | FP_CTRL_ENABLE); }

static __INLINE void disableFPB(void) { FPB->CTRL = FP_CTRL_KEY | (FPB->CTRL & ~FP_CTRL_ENABLE); }
...
static __INLINE uint32_t getHighestMPUDataRegionIndex(void) { return getMPUDataRegionCount() - 1; }

static __INLINE int isMPURegionNumberValid(uint32_t regionNumber) { return regionNumber < getMPUDataRegionCount(); }

static __INLINE int isMPUNotPresent(void) { return getMPUDataRegionCount() == 0; }

static __INLINE uint32_t getMPUControlValue(void) {
  if (isMPUNotPresent()) return ~0U;

  return (MPU->CTRL);
}

static __INLINE void setMPUControlValue(uint32_t newControlValue) {
  if (isMPUNotPresent()) return;
...
static __INLINE int prepareToAccessMPURegion(uint32_t regionNumber) {
  if (!isMPURegionNumberValid(regionNumber)) return 0;

  MPU->RNR = regionNumber;
  return 1;
}

static __INLINE uint32_t getCurrentMPURegionNumber(void) { return MPU->RNR; }
...
static __INLINE uint32_t getCurrentlyExecutingExceptionNumber(void) { return (__get_IPSR() & 0xFF); }

static __INLINE uint32_t getCurrentSysTickControlValue(void) { return SysTick->CTRL; }

static __INLINE uint32_t getCurrentSysTickReloadValue(void) { return SysTick->LOAD; }

static __INLINE void setSysTickControlValue(uint32_t controlValue) { SysTick->CTRL = controlValue; }

static __INLINE void setSysTickReloadValue(uint32_t reloadValue) {
  SysTick->LOAD = reloadValue & SysTick_LOAD_RELOAD_Msk;
}

static __INLINE uint32_t getSysTick10MillisecondInterval(void) { return SysTick->CALIB & SysTick_CALIB_TENMS_Msk; }

static __INLINE void disableSysTick(void) { SysTick->CTRL = 0; }
...
static __INLINE void start10MillisecondSysTick(void) {
  if (getSysTick10MillisecondInterval() == 0) return;
...
static __INLINE int has10MillisecondSysTickExpired(void) {
  if (getSysTick10MillisecondInterval() == 0) return 1;
