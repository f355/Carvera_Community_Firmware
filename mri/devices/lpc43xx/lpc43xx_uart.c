...
static const UartConfiguration g_uartConfigurations[] = {
    {LPC_USART0, CLK_BASE_UART0, CLK_MX_UART0, CLK_APB0_UART0, SCU_PIN(6, 4), SCU_MODE_FUNC2, SCU_PIN(6, 5),
     SCU_MODE_FUNC2},
    {LPC_UART1, CLK_BASE_UART1, CLK_MX_UART1, CLK_APB0_UART1, SCU_PIN(5, 6), SCU_MODE_FUNC4, SCU_PIN(1, 14),
     SCU_MODE_FUNC1},
    {LPC_USART2, CLK_BASE_UART2, CLK_MX_UART2, CLK_APB2_UART2, SCU_PIN(2, 10), SCU_MODE_FUNC2, SCU_PIN(2, 11),
     SCU_MODE_FUNC2},
    {LPC_USART3, CLK_BASE_UART3, CLK_MX_UART3, CLK_APB2_UART3, SCU_PIN(2, 3), SCU_MODE_FUNC2, SCU_PIN(2, 4),
     SCU_MODE_FUNC2}};
...
static void parseUartParameters(Token* pParameterTokens, UartConfiguration* pUart, UartParameters* pParameters) {
  ...
  if (pUart->pUartRegisters == pRxUartRegisters) pParameters->pUart = pUart;

  /* The Bambino210E only exposes a single set of pins per UART so these are specific to that board. */
  if (Token_MatchingString(pParameterTokens, "MRI_UART_MBED_USB")) pParameters->pUart = &g_uartConfigurations[2];
  if (Token_MatchingString(pParameterTokens, "MRI_UART_0")) pParameters->pUart = &g_uartConfigurations[0];
  if (Token_MatchingString(pParameterTokens, "MRI_UART_1")) pParameters->pUart = &g_uartConfigurations[1];
  if (Token_MatchingString(pParameterTokens, "MRI_UART_2")) pParameters->pUart = &g_uartConfigurations[2];
  if (Token_MatchingString(pParameterTokens, "MRI_UART_3")) pParameters->pUart = &g_uartConfigurations[3];

  /* Default to MRI_UART_MBED_USB if nothing else was specified. */
  if (!pParameters->pUart) pParameters->pUart = &g_uartConfigurations[2];
...
static void saveUartToBeUsedByDebugger(const UartConfiguration* pUart) { mriLpc43xxState.pCurrentUart = pUart; }
...
static void clearUartFractionalBaudDivisor(void) { mriLpc43xxState.pCurrentUart->pUartRegisters->FDR = 0x10; }
...
static void setDivisorLatchBit(void) { mriLpc43xxState.pCurrentUart->pUartRegisters->LCR |= LPC176x_UART_LCR_DLAB; }

static void clearDivisorLatchBit(void) { mriLpc43xxState.pCurrentUart->pUartRegisters->LCR &= ~LPC176x_UART_LCR_DLAB; }
