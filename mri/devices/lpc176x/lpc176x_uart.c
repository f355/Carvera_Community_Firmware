...
static const UartConfiguration g_uartConfigurations[] = {
    {&(LPC_SC->PCLKSEL0), &(LPC_PINCON->PINSEL0), &(LPC_PINCON->PINSEL0), (LPC_UART_TypeDef*)LPC_UART0, 1 << 3, 3 << 6,
     3 << 4, 3 << 6, 0x55555555},
    {&(LPC_SC->PCLKSEL0), &(LPC_PINCON->PINSEL0), &(LPC_PINCON->PINSEL1), (LPC_UART_TypeDef*)LPC_UART1, 1 << 4, 3 << 8,
     3 << 30, 3 << 0, 0x55555555},
    {&(LPC_SC->PCLKSEL1), &(LPC_PINCON->PINSEL4), &(LPC_PINCON->PINSEL4), (LPC_UART_TypeDef*)LPC_UART2, 1 << 24,
     3 << 16, 3 << 16, 3 << 18, 0xAAAAAAAA},
    {&(LPC_SC->PCLKSEL1), &(LPC_PINCON->PINSEL0), &(LPC_PINCON->PINSEL0), (LPC_UART_TypeDef*)LPC_UART3, 1 << 25,
     3 << 18, 3 << 0, 3 << 2, 0xAAAAAAAA}};
...
static void parseUartParameters(Token* pParameterTokens, UartParameters* pParameters) {
  ...
  if (Token_MatchingString(pParameterTokens, "MRI_UART_MBED_USB")) pParameters->uartIndex = 0;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_MBED_P9_P10")) pParameters->uartIndex = 3;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_MBED_P13_P14")) pParameters->uartIndex = 1;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_MBED_P28_P27")) pParameters->uartIndex = 2;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_0")) pParameters->uartIndex = 0;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_1")) pParameters->uartIndex = 1;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_2")) pParameters->uartIndex = 2;
  if (Token_MatchingString(pParameterTokens, "MRI_UART_3")) pParameters->uartIndex = 3;
...
static void enablePowerToUart(void) { LPC_SC->PCONP |= mriLpc176xState.pCurrentUart->powerConfigurationBit; }
...
static void clearUartFractionalBaudDivisor(void) { mriLpc176xState.pCurrentUart->pUartRegisters->FDR = 0x10; }
...
static BaudRateDivisors calculateBaudRateDivisors(uint32_t baudRate, uint32_t peripheralRate) {
  ...
  if (isNoFractionalDivisorRequired(&calcDivisors)) return closestDivisors(&calcDivisors);
...
static BaudRateDivisors calculateFractionalBaudRateDivisors(CalculateDivisors* pThis) {
  for (pThis->mul = 1; pThis->mul <= 15; pThis->mul++) {
    for (pThis->divAdd = 1; pThis->divAdd < pThis->mul; pThis->divAdd++) checkTheseFractionalDivisors(pThis);
  }
...
static void setDivisorLatchBit(void) { mriLpc176xState.pCurrentUart->pUartRegisters->LCR |= LPC176x_UART_LCR_DLAB; }

static void clearDivisorLatchBit(void) { mriLpc176xState.pCurrentUart->pUartRegisters->LCR &= ~LPC176x_UART_LCR_DLAB; }
...
static void selectPinForTx(void) {
  *mriLpc176xState.pCurrentUart->pTxPinSelection = calculatePinSelectionValue(
      *mriLpc176xState.pCurrentUart->pTxPinSelection, mriLpc176xState.pCurrentUart->txPinSelectionMask);
}

static void selectPinForRx(void) {
  *mriLpc176xState.pCurrentUart->pRxPinSelection = calculatePinSelectionValue(
      *mriLpc176xState.pCurrentUart->pRxPinSelection, mriLpc176xState.pCurrentUart->rxPinSelectionMask);
}
...
static int commUartIndex(void) { return mriLpc176xState.pCurrentUart - g_uartConfigurations; }
