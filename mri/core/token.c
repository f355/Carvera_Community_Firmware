...
void Token_Init(Token* pToken) { Token_InitWith(pToken, " \t"); }
...
static void copyStringIntoToken(Token* pToken, const char* pStringToCopy) {
  ...
  *pDest = '\0';
  ...
  if (*pSource) __throw(bufferOverrunException);
}
...
static char* findFirstNonSeparator(Token* pToken, char* p) {
  while (*p && charIsSeparator(pToken, *p)) p++;
...
static void addToken(Token* pToken, const char* p) {
  ...
  if (pToken->tokenCount >= ARRAY_SIZE(pToken->tokenPointers)) __throw(bufferOverrunException);

  pToken->tokenPointers[pToken->tokenCount++] = p;
}

size_t Token_GetTokenCount(Token* pToken) { return pToken->tokenCount; }

const char* Token_GetToken(Token* pToken, size_t tokenIndex) {
  if (tokenIndex >= pToken->tokenCount) __throw_and_return(invalidIndexException, NULL);
...
const char* Token_MatchingString(Token* pToken, const char* pTokenToSearchFor) {
  ...
  for (i = 0; i < pToken->tokenCount; i++) {
    if (0 == mri_strcmp(pToken->tokenPointers[i], pTokenToSearchFor)) return pToken->tokenPointers[i];
  }
  return NULL;
}
