...
static uintmri_t readMemoryBytesIntoHexBuffer(Buffer* pBuffer, uintmri_t address, uintmri_t readByteCount) {
  ...
  while (readByteCount-- > 0) {
    ...
    if (Platform_WasMemoryFaultEncountered()) break;
...
static uintmri_t readMemoryHalfWordIntoHexBuffer(Buffer* pBuffer, uintmri_t address) {
  ...
  if (isNotHalfWordAligned(address)) return readMemoryBytesIntoHexBuffer(pBuffer, address, sizeof(uint16_t));

  value = Platform_MemRead16(address);
  if (Platform_WasMemoryFaultEncountered()) return 0;
  writeBytesToBufferAsHex(pBuffer, &value, sizeof(value));

  return sizeof(value);
}

static int isNotHalfWordAligned(uintmri_t address) { return address & 1; }

static void writeBytesToBufferAsHex(Buffer* pBuffer, const void* pv, size_t length) {
  uint8_t* pBytes = (uint8_t*)pv;
  while (length--) Buffer_WriteByteAsHex(pBuffer, *pBytes++);
}

static uintmri_t readMemoryWordIntoHexBuffer(Buffer* pBuffer, uintmri_t address) {
  uint32_t value;

  if (isNotWordAligned(address)) return readMemoryBytesIntoHexBuffer(pBuffer, address, sizeof(uint32_t));
...
static int isNotWordAligned(uintmri_t address) { return address & 3; }

static uintmri_t readMemoryDoubleWordIntoHexBuffer(Buffer* pBuffer, uintmri_t address) {
  if (sizeof(uintmri_t) >= 8) {
    uint64_t value;

    if (isNot64BitAligned(address)) return readMemoryBytesIntoHexBuffer(pBuffer, address, sizeof(uint64_t));
...
static int isNot64BitAligned(uintmri_t address) { return address & 7; }
...
static int writeHexBufferToHalfWordMemory(Buffer* pBuffer, uintmri_t address) {
  ...
  if (isNotHalfWordAligned(address)) return writeHexBufferToByteMemory(pBuffer, address, sizeof(uint16_t));

  if (!readBytesFromHexBuffer(pBuffer, &value, sizeof(value))) return 0;
...
static int writeHexBufferToWordMemory(Buffer* pBuffer, uintmri_t address) {
  ...
  if (isNotWordAligned(address)) return writeHexBufferToByteMemory(pBuffer, address, sizeof(uint32_t));
...
static int writeHexBufferToDoubleWordMemory(Buffer* pBuffer, uintmri_t address) {
  if (sizeof(uintmri_t) >= 8) {
    ...
    if (isNot64BitAligned(address)) return writeHexBufferToByteMemory(pBuffer, address, sizeof(uint64_t));
...
static int writeBinaryBufferToHalfWordMemory(Buffer* pBuffer, uintmri_t address) {
  ...
  if (isNotHalfWordAligned(address)) return writeBinaryBufferToByteMemory(pBuffer, address, sizeof(uint16_t));

  if (!readBytesFromBinaryBuffer(pBuffer, &value, sizeof(value))) return 0;
...
static int writeBinaryBufferToWordMemory(Buffer* pBuffer, uintmri_t address) {
  ...
  if (isNotWordAligned(address)) return writeBinaryBufferToByteMemory(pBuffer, address, sizeof(uint32_t));
...
static int writeBinaryBufferToDoubleWordMemory(Buffer* pBuffer, uintmri_t address) {
  if (sizeof(uintmri_t) >= 8) {
    ...
    if (isNot64BitAligned(address)) return writeBinaryBufferToByteMemory(pBuffer, address, sizeof(uint64_t));
