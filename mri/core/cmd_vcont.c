...
typedef enum { THREAD_ID_ALL = -1, THREAD_ID_NONE, THREAD_ID_SPECIFIC } ThreadIdType;
...
typedef enum { ACTION_NONE, ACTION_CONTINUE, ACTION_SINGLE_STEP, ACTION_RANGED_SINGLE_STEP } ActionType;
...
static uint32_t handleSingleStepAndContinueCommandsWithSetThreadState(Buffer* pBuffer, const Action* pContinueAction,
...
static void firstParsePass(Buffer* pBuffer, Action* pContinueAction, Action* pStepAction) {
  ...
  while (Buffer_BytesLeft(pBuffer) > 0 && Buffer_IsNextCharEqualTo(pBuffer, ';')) {
    ...
    switch (action.type) {
      ...
      case ACTION_SINGLE_STEP:
        if (isActionMoreSpecificThanCurrent(pStepAction, &action)) *pStepAction = action;
        break;
      case ACTION_CONTINUE:
        if (isActionMoreSpecificThanCurrent(pContinueAction, &action)) *pContinueAction = action;
        break;
      default:
        break;
    }
  ...
  if (!Platform_RtosIsSetThreadStateSupported() && pContinueAction->type == ACTION_NONE &&
      pStepAction->type == ACTION_NONE) {
    PrepareStringResponse(MRI_ERROR_INVALID_ARGUMENT);
    __throw(invalidArgumentException);
  }
...
static Action parseContinueWithSignalAction(Buffer* pBuffer) {
  ...
  __catch { __rethrow_and_return(action); }
  return action;
}
...
static AddressRange parseAddressRange(Buffer* pBuffer) {
  ...
  __catch { __rethrow_and_return(range); }
  return range;
}
...
static ThreadId parseOptionalThreadId(Buffer* pBuffer) {
  ...
  if (Buffer_BytesLeft(pBuffer) == 0 || !Buffer_IsNextCharEqualTo(pBuffer, ':')) return threadId;
  ...
  __catch { __rethrow_and_return(threadId); }
...
static uint32_t handleAction(const Action* pAction) {
  ...
  switch (pAction->type) {
    ...
    case ACTION_RANGED_SINGLE_STEP:
      ...
      if (returnValue) SetSingleSteppingRange(&pAction->range);
      return returnValue;
    default:
      PrepareStringResponse(MRI_ERROR_INVALID_ARGUMENT);
      return 0;
...
void RestoreThreadStates(void) {
  if (!Platform_RtosIsSetThreadStateSupported()) return;
