...
static void waitForStartOfNextPacket(Packet* pPacket) {
  ...
  while (nextChar != '$') nextChar = getNextCharFromGdb(pPacket);
}
...
static void clearChecksum(Packet* pPacket) { pPacket->calculatedChecksum = 0; }

static void updateChecksum(Packet* pPacket, char nextChar) { pPacket->calculatedChecksum += (unsigned char)nextChar; }

static int isEscapePrefixChar(char charToCheck) { return charToCheck == '}'; }

static char unescapeChar(char charToUnescape) { return charToUnescape ^ 0x20; }
...
static int isChecksumValid(Packet* pPacket) { return (pPacket->expectedChecksum == pPacket->calculatedChecksum); }

static void sendACKToGDB(void) { Platform_CommSendChar('+'); }

static void sendNAKToGDB(void) { Platform_CommSendChar('-'); }
...
static void storePacketHeaderByte(Packet* pPacket) { Buffer_WriteChar(&pPacket->packetBuffer, '$'); }
...
__attribute__((weak)) void Platform_CommSendBuffer(Buffer* pBuffer) {
  while (Buffer_BytesLeft(pBuffer)) Platform_CommSendChar(Buffer_ReadChar(pBuffer));
}
