...
static void clearCoreStructure(void) { mri_memset(&g_mri, 0, sizeof(g_mri)); }
...
static void setFirstExceptionFlag(void) { g_mri.flags |= MRI_FLAGS_FIRST_EXCEPTION; }

static void setSuccessfulInitFlag(void) { g_mri.flags |= MRI_FLAGS_SUCCESSFUL_INIT; }
...
int SetTempBreakpoint(uint32_t breakpointAddress, TempBreakpointCallbackPtr pCallback, void* pvContext) {
  if (isTempBreakpointSet()) return 0;
...
static int isTempBreakpointSet(void) { return g_mri.flags & MRI_FLAGS_TEMP_BREAKPOINT; }

static uint32_t clearThumbBitOfAddress(uint32_t address) { return address & ~1; }

static void setTempBreakpointFlag(void) { g_mri.flags |= MRI_FLAGS_TEMP_BREAKPOINT; }
...
void mriDebugException(MriContext* pContext) {
  ...
  if (g_mri.pEnteringHook) g_mri.pEnteringHook(g_mri.pvEnteringLeavingContext);
  Platform_EnteringDebugger();

  if (isDebugTrap() && Semihost_IsDebuggeeMakingSemihostCall() && Semihost_HandleSemihostRequest() &&
      !justSingleStepped) {
    RestoreThreadStates();
  ...
  if (!IsFirstException()) Platform_DisplayFaultCauseToGdbConsole();
  Send_T_StopResponse();
...
static void clearControlCEncounteredFlag(void) { g_mri.flags &= ~MRI_FLAGS_ENCOUNTERED_CTRL_C; }

static int wasTempBreakpointHit(void) {
  return (isTempBreakpointSet() && clearThumbBitOfAddress(Platform_GetProgramCounter()) == g_mri.tempBreakpointAddress);
}
...
static void clearTempBreakpointFlag(void) { g_mri.flags &= ~MRI_FLAGS_TEMP_BREAKPOINT; }
...
static int areSingleSteppingInRange(void) {
  switch (g_mri.signalValue) {
    ...
    case SIGTRAP: {
      ...
      if (reason.type != MRI_PLATFORM_TRAP_TYPE_UNKNOWN) return 0;
      break;
    }
    default:
      break;
...
static void clearSingleSteppingInRange(void) { g_mri.flags &= ~MRI_FLAGS_RANGED_SINGLE_STEP; }

static void determineSignalValue(void) { g_mri.signalValue = Platform_DetermineCauseOfException(); }

static int isDebugTrap(void) { return g_mri.signalValue == SIGTRAP; }
...
static void prepareForDebuggerExit(void) {
  ...
  if (g_mri.pLeavingHook) g_mri.pLeavingHook(g_mri.pvEnteringLeavingContext);
...
static void clearFirstExceptionFlag(void) { g_mri.flags &= ~MRI_FLAGS_FIRST_EXCEPTION; }
...
static int handleGDBCommand(void) {
  ...
  static const struct {
    ...
    char commandChar;
  } commandTable[] = {{Send_T_StopResponse, '?'},
  ...
  if (Platform_HandleGDBCommand) handlerResult = Platform_HandleGDBCommand(pBuffer);
  if (handlerResult == 0) {
    ...
    if (ARRAY_SIZE(commandTable) == i) PrepareEmptyResponseForUnknownCommand();
  }

  if ((handlerResult & HANDLER_RETURN_RETURN_IMMEDIATELY) == 0) SendPacketToGdb();
...
void InitPacketBuffers(void) { Packet_Init(&g_mri.packet, Platform_GetPacketBuffer(), Platform_GetPacketBufferSize()); }
...
int WasSuccessfullyInit(void) { return (int)(g_mri.flags & MRI_FLAGS_SUCCESSFUL_INIT); }

int WasControlCFlagSentFromGdb(void) { return (int)(g_mri.flags & MRI_FLAGS_SEMIHOST_CTRL_C); }

void RequestResetOnNextContinue(void) { g_mri.flags |= MRI_FLAGS_RESET_ON_CONTINUE; }

int WasControlCEncountered(void) { return (int)(g_mri.flags & MRI_FLAGS_ENCOUNTERED_CTRL_C); }

void ControlCEncountered(void) { g_mri.flags |= MRI_FLAGS_ENCOUNTERED_CTRL_C; }

void CancelResetRequestOnNextContinue(void) { g_mri.flags &= ~MRI_FLAGS_RESET_ON_CONTINUE; }
...
MriContext* GetContext(void) { return g_mri.pContext; }

void SetContext(MriContext* pContext) { g_mri.pContext = pContext; }
...
int WasSemihostCallCancelledByGdb(void) { return g_mri.semihostErrno == EINTR; }
...
int IsFirstException(void) { return (int)(g_mri.flags & MRI_FLAGS_FIRST_EXCEPTION); }

void SetSignalValue(uint8_t signalValue) { g_mri.signalValue = signalValue; }

uint8_t GetSignalValue(void) { return g_mri.signalValue; }
...
int GetSemihostReturnCode(void) { return g_mri.semihostReturnCode; }

int GetSemihostErrno(void) { return g_mri.semihostErrno; }

Buffer* GetBuffer(void) { return &g_mri.packet.dataBuffer; }
