...
void Switch::on_halt(void* arg) {
  if (arg == nullptr) {
    ...
    switch (this->output_type) {
      case DIGITAL:
        this->digital_pin->set(this->failsafe);
        break;
      case SIGMADELTA:
        this->sigmadelta_pin->set(this->failsafe);
        break;
      case HWPWM:
        if (this->failsafe == 1) {
        ...
        break;
      case SWPWM:
        this->swpwm_pin->write(switch_value / 100.0F);
        break;
      case DIGITALPWM:
        this->digital_pin->set(this->failsafe);
        break;
...
void Switch::on_config_reload(void* argument) {
  this->subcode =
      THEKERNEL->config->value(switch_checksum, this->name_checksum, command_subcode_checksum)->as_number(0);
  std::string input_on_command =
      THEKERNEL->config->value(switch_checksum, this->name_checksum, input_on_command_checksum)->as_string("");
  std::string input_off_command =
      THEKERNEL->config->value(switch_checksum, this->name_checksum, input_off_command_checksum)->as_string("");
  this->output_on_command =
      THEKERNEL->config->value(switch_checksum, this->name_checksum, output_on_command_checksum)->as_string("");
  this->output_off_command =
      THEKERNEL->config->value(switch_checksum, this->name_checksum, output_off_command_checksum)->as_string("");
  this->switch_state =
      THEKERNEL->config->value(switch_checksum, this->name_checksum, startup_state_checksum)->as_bool(false);

  this->input_pin = new Pin();
  this->input_pin
      ->from_string(THEKERNEL->config->value(switch_checksum, this->name_checksum, input_pin_checksum)->as_string("nc"))
      ->as_input();

  bool is_input;

  if (this->input_pin->connected()) {
    std::string ipb = THEKERNEL->config->value(switch_checksum, this->name_checksum, input_pin_behavior_checksum)
                          ->as_string("momentary");
  ...
  if (!is_input) {
    string type =
        THEKERNEL->config->value(switch_checksum, this->name_checksum, output_type_checksum)->as_string("digital");
    this->failsafe = THEKERNEL->config->value(switch_checksum, this->name_checksum, failsafe_checksum)->as_number(0);
    this->ignore_on_halt =
        THEKERNEL->config->value(switch_checksum, this->name_checksum, ignore_on_halt_checksum)->as_bool(false);
    ...
    if (type == "pwm") {
      ...
      this->sigmadelta_pin
          ->from_string(
              THEKERNEL->config->value(switch_checksum, this->name_checksum, output_pin_checksum)->as_string("nc"))
          ->as_output();
      ...
      Pin* pin = new Pin();
      pin->from_string(
             THEKERNEL->config->value(switch_checksum, this->name_checksum, pwm_pin_checksum)->as_string("nc"))
          ->as_output();
      this->pwm_pin = pin->hardware_pwm();
  ...
  if (!is_input) {
    if (this->output_type == SIGMADELTA) {
      this->sigmadelta_pin->max_pwm(
          THEKERNEL->config->value(switch_checksum, this->name_checksum, max_pwm_checksum)->as_number(255));
      this->switch_value = THEKERNEL->config->value(switch_checksum, this->name_checksum, startup_value_checksum)
                               ->as_number(this->sigmadelta_pin->max_pwm());
      ...
      // default is 20Hz
      float p = THEKERNEL->config->value(switch_checksum, this->name_checksum, pwm_period_ms_checksum)->as_number(20) *
                1000.0F;  // ms but fractions are allowed
      this->pwm_pin->period_us(p);

      // default is 0% duty cycle
      this->switch_value =
          THEKERNEL->config->value(switch_checksum, this->name_checksum, startup_value_checksum)->as_number(0);
      this->default_on_value =
          THEKERNEL->config->value(switch_checksum, this->name_checksum, default_on_value_checksum)->as_number(50);
      if (this->switch_state) {
      ...
      // default is 50Hz
      float p = THEKERNEL->config->value(switch_checksum, this->name_checksum, pwm_period_ms_checksum)
                    ->as_number(10);  // ms fractions are not allowed
      ...
      this->switch_value =
          THEKERNEL->config->value(switch_checksum, this->name_checksum, startup_value_checksum)->as_number(0);
      this->default_on_value =
          THEKERNEL->config->value(switch_checksum, this->name_checksum, default_on_value_checksum)->as_number(100);
      if (this->switch_state) {
...
void Switch::turn_on_switch(float value) {
  if (this->output_type == SIGMADELTA) {
    ...
    if (value >= 0) {
      int v = roundf(value * sigmadelta_pin->max_pwm() /
                     255.0F);  // scale by max_pwm so input of 255 and max_pwm of 128 would set value to 128
...
void Switch::on_main_loop(void* argument) {
  if (this->switch_changed) {
    if (this->switch_state) {
      ...
      if (this->output_type == SIGMADELTA) {
        this->sigmadelta_pin->pwm(this->switch_value);  // this requires the value has been set otherwise it switches on
                                                        // to whatever it last was
