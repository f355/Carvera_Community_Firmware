...
class ATCHandler : public Module {
 ...
 private:
  ...
  //
  void fill_change_scripts(int new_tool, bool clear_z, int old_tool, bool wait_after_empty, uint8_t colletIndex,
                           float custom_TLO);
  ...
  struct atc_tool {
    ...
    bool is_valid() const { return (mx_mm != -1 && my_mm != -1 && mz_mm != -1); }
