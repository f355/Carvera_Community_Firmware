  ...
  if (isnan(z_mm)) {
    ...
    if (isnan(last_valid_z)) {
      THEKERNEL->streams->printf(
          "ERROR: Z coordinate is required for the first tool slot (no previous Z value found)\n");
      return;
    }
...
void ATCHandler::fill_calibrate_probe_anchor_scripts(bool invert_probe) {
  ...
  snprintf(buff, sizeof(buff), "G91 G53 G0 X%.3f Y%.3f",
           this->anchor1_x + this->anchor2_offset_x - this->anchor_width / 2,
           this->anchor1_y + this->anchor2_offset_y + 15);
  this->script_queue.push(buff);
  ...
  if (!gcode->has_letter('P')) {
    ...
    switch ((int)gcode->get_value('P')) {
      ...
      case 1:
        ...
        THEKERNEL->streams->printf(
            "These values have been temporarily set. You can test the position with M496.3\nTo make them permanent "
      ...
      case 2:
        ...
        THEKERNEL->streams->printf(
            "These values have been temporarily set. You can test the position with M496.4\nTo make them permanent "
            "run:\nconfig-set sd coordinate.anchor2_x %.3f\nconfig-set sd coordinate.anchor2_y %.3f\n you will need to "
            "check calibration on all other machine positions\n",
            final_x, final_y);
        break;
      case 3:
      ...
      case 4:
        ...
        THEKERNEL->streams->printf(
            "These values have been temporarily set. \nTo make them permanent run:\nconfig-set sd "
            "coordinate.rotation_offset_y %.3f\n",
            final_y);
        break;
      case 5:
        // caibrate 4th axis height
        // M469.7 A#120 B#119 C#118 Z%.3f P5
        if (!gcode->has_letter('A') && !gcode->has_letter('B') && !gcode->has_letter('C') && !gcode->has_letter('D') &&
            !gcode->has_letter('E') && !gcode->has_letter('Z') && !gcode->has_letter('R')) {
          THEKERNEL->streams->printf("Not enough variables given to M469\n Abort\n");
          return;
        }
        THEKERNEL->streams->printf("Previous 4th Height: %.3f\n", gcode->get_value('Z'));

        final_y = -(gcode->get_value('B') + gcode->get_value('C') + gcode->get_value('D') + gcode->get_value('E')) / 4 +
                  gcode->get_value('A') + gcode->get_value('R') / 2;
        THEKERNEL->streams->printf("New 4th Height: %.3f\n", final_y);
        // this->rotation_offset_z = final_y;
        THEKERNEL->streams->printf(
            "These values have been temporarily set. \nTo make them permanent run:\nconfig-set sd "
            "coordinate.rotation_offset_z %.3f\n",
            final_y);
        break;
      case 6:
...
void ATCHandler::calibrate_anchor2(Gcode* gcode)  // M469.2
{
  ...
  // move to anchor 2 probe position
  snprintf(buff, sizeof(buff), "G91 G53 G0 X%.3f Y%.3f", this->anchor1_x + this->anchor2_offset_x - 7,
           this->anchor1_y + this->anchor2_offset_y - 7);
  this->script_queue.push(buff);
...
void ATCHandler::calibrate_a_axis_height(Gcode* gcode)  // M469.5
{
  ...
  // execute calibration with specific values
  snprintf(buff, sizeof(buff), "M469.7 A#120 B#119 C#118 D#117 E#116 R%.3f Z%.3f P5", pin_diameter,
...
const char* cor_probe_cmd(bool invert) { return invert ? "G38.4" : "G38.2"; }
...
void ATCHandler::cor_queue_probe_y_front() {
  ...
  snprintf(buff, sizeof(buff), "G53 G0 G90 Y%.3f A%.3f F%.3f", y_front, a_axis_cor.start_a + 90.0f,
           a_axis_cor.pos_feed);
  this->script_queue.push(buff);
  snprintf(buff, sizeof(buff), "G53 G0 Z%.3f F%.3f", a_axis_cor.z_centerline, a_axis_cor.pos_feed);
  this->script_queue.push(buff);
  snprintf(buff, sizeof(buff), "G91 %s Y%.3f F%.3f", cor_probe_cmd(a_axis_cor.invert_probe), a_axis_cor.probe_travel,
           this->probe_slow_rate);
  this->script_queue.push(buff);
...
void ATCHandler::cor_queue_probe_z_top() {
  char buff[100];
  snprintf(buff, sizeof(buff), "G53 G0 Y%.3f A%.3f F%.3f", a_axis_cor.y_center, a_axis_cor.start_a,
           a_axis_cor.pos_feed);
  this->script_queue.push(buff);
  snprintf(buff, sizeof(buff), "G91 %s Z%.3f F%.3f", cor_probe_cmd(a_axis_cor.invert_probe), -a_axis_cor.probe_travel,
           this->probe_slow_rate);
  this->script_queue.push(buff);
  this->script_queue.push("M469.7 P6");
}
...
void ATCHandler::cor_queue_probe_module_ref() {
  ...
  this->script_queue.push(buff);
  snprintf(buff, sizeof(buff), "G91 %s Z%.3f F%.3f", cor_probe_cmd(a_axis_cor.invert_probe), -120.0f,
           this->probe_slow_rate);
  this->script_queue.push(buff);
  this->script_queue.push("M469.7 P6");
}
...
void ATCHandler::calibrate_a_axis_cor(Gcode* gcode)  // M469.6
{
  ...
  const bool probe_tool = (this->active_tool == 0) || (this->active_tool == 9999) || (this->active_tool >= 999990) ||
                          (THEKERNEL->eeprom_data->TOOL == 9999);
  if (!probe_tool) {
    THEKERNEL->streams->printf(
        "ERROR: Attempted to probe with an improper tool. A 3-axis probe must be installed (tool 0, 9999 or tool >= "
        "999990).\n");
  ...
  this->script_queue.push(
      ";After CoR finding, probes the standard 4th-axis Z reference (same as M469.5) to compute rotation_offset_z");
  this->script_queue.push("G90 G21");
  snprintf(buff, sizeof(buff), "G91 %s Z%.3f F%.3f", cor_probe_cmd(invert_probe), -a_axis_cor.probe_travel,
           this->probe_slow_rate);
  this->script_queue.push(buff);
  this->script_queue.push("M469.7 P6");
}
...
void ATCHandler::fill_change_scripts(int new_tool, bool clear_z, int old_tool = -1, bool wait_after_empty = false,
                                     uint8_t colletIndex = 0, float custom_TLO = NAN) {
  char buff[100];
  ...
  snprintf(buff, sizeof(buff), "G53 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(probe_mx_mm - 22.0),
           THEROBOT->from_millimeters(probe_my_mm));
  this->script_queue.push(buff);
...
void ATCHandler::fill_manual_drop_scripts(int old_tool) {
  ...
  this->script_queue.push(buff);
  // move to clearance
  snprintf(buff, sizeof(buff), "G90 G53 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(probe_mx_mm - 22.0),
           THEROBOT->from_millimeters(probe_my_mm));
  this->script_queue.push(buff);
...
void ATCHandler::fill_manual_pickup_scripts(int new_tool, bool clear_z, bool auto_calibrate = false,
...
void ATCHandler::fill_drop_scripts(int old_tool) {
  ...
  this->script_queue.push(buff);
  // move x and y to active tool position
  snprintf(buff, sizeof(buff), "G53 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(current_tool->get_mx_mm()),
           THEROBOT->from_millimeters(current_tool->get_my_mm()));
  this->script_queue.push(buff);
  ...
  snprintf(buff, sizeof(buff), "G53 G1 Z%.3f F%.3f",
           THEROBOT->from_millimeters(current_tool->get_mz_mm() + safe_z_offset_mm),
           THEROBOT->from_millimeters(fast_z_rate));
  this->script_queue.push(buff);
  // drop z axis with slow speed
  snprintf(buff, sizeof(buff), "G53 G1 Z%.3f F%.3f", THEROBOT->from_millimeters(current_tool->get_mz_mm()),
           THEROBOT->from_millimeters(slow_z_rate));
  this->script_queue.push(buff);
  // loose tool
  this->script_queue.push("M490.2");
  // lift z to safe position with fast speed
...
void ATCHandler::fill_pick_scripts(int new_tool, bool clear_z) {
  ...
  this->script_queue.push("M497.2");
  // lift z to safe position with fast speed
  snprintf(buff, sizeof(buff), "G53 G0 Z%.3f",
           THEROBOT->from_millimeters(clear_z ? this->clearance_z : this->safe_z_empty_mm));
  this->script_queue.push(buff);
...
void ATCHandler::fill_cali_scripts(bool is_probe, bool clear_z, int repeat_count) {
  ...
  for (int i = 1; i <= repeat_count; i++) {
    if (i == 1) {
      // lift z to safe position with fast speed
      snprintf(buff, sizeof(buff), "G53 G0 Z%.3f",
               THEROBOT->from_millimeters(clear_z ? this->clearance_z : this->safe_z_mm));
      this->script_queue.push(buff);
    }
    ...
    snprintf(buff, sizeof(buff), "G53 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(probe_x),
             THEROBOT->from_millimeters(probe_y));
    this->script_queue.push(buff);
...
void ATCHandler::fill_margin_scripts(float x_pos, float y_pos, float x_pos_max, float y_pos_max) {
  ...
  snprintf(buff, sizeof(buff), "G90 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(x_pos),
           THEROBOT->from_millimeters(y_pos));
  this->script_queue.push(buff);

  // goto margin top left corner
  snprintf(buff, sizeof(buff), "G90 G1 X%.3f Y%.3f F%.3f", THEROBOT->from_millimeters(x_pos),
           THEROBOT->from_millimeters(y_pos_max), THEROBOT->from_millimeters(this->margin_rate));
  this->script_queue.push(buff);

  // goto margin top right corner
  snprintf(buff, sizeof(buff), "G90 G1 X%.3f Y%.3f F%.3f", THEROBOT->from_millimeters(x_pos_max),
           THEROBOT->from_millimeters(y_pos_max), THEROBOT->from_millimeters(this->margin_rate));
  this->script_queue.push(buff);

  // goto margin bottom right corner
  snprintf(buff, sizeof(buff), "G90 G1 X%.3f Y%.3f F%.3f", THEROBOT->from_millimeters(x_pos_max),
           THEROBOT->from_millimeters(y_pos), THEROBOT->from_millimeters(this->margin_rate));
  this->script_queue.push(buff);
...
void ATCHandler::fill_zprobe_scripts(float x_pos, float y_pos, float x_offset, float y_offset) {
  ...
  snprintf(buff, sizeof(buff), "G90 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(x_pos + x_offset),
           THEROBOT->from_millimeters(y_pos + y_offset));
  this->script_queue.push(buff);
...
void ATCHandler::fill_zprobe_abs_scripts() {
  ...
  // goto z probe position
  if (!(THEKERNEL->factory_set->FuncSetting & (1 << 0))) {
    snprintf(buff, sizeof(buff), "G53 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(anchor1_x + rotation_offset_x - 3),
             THEROBOT->from_millimeters(anchor1_y + rotation_offset_y));
  } else {
    // snprintf(buff, sizeof(buff), "G53 G0 X%.3f Y%.3f", THEROBOT->from_millimeters(anchor1_x + rotation_offset_x - 7),
    // THEROBOT->from_millimeters(anchor1_y + rotation_offset_y));
    //  move y to clearance
...
void ATCHandler::fill_xyzprobe_scripts(float tool_dia, float probe_height) {
  ...
  // move to XY zero
  snprintf(buff, sizeof(buff), "G91 G0 X%.3f Y%0.3f", THEROBOT->from_millimeters(-5 - tool_dia / 2),
           THEROBOT->from_millimeters(-5 - tool_dia / 2));
  this->script_queue.push(buff);
...
void ATCHandler::fill_autolevel_scripts(float x_pos, float y_pos, float x_size, float y_size, int x_grids, int y_grids,
                                        float height) {
  char buff[100];
  if (!THEROBOT->is_homed_all_axes()) {
    return;
...
void ATCHandler::on_config_reload(void* argument) {
  ...
  if (CARVERA == THEKERNEL->factory_set->MachineModel) {
    atc_home_info.pin.from_string(THEKERNEL->config->value(atc_checksum, endstop_pin_checksum)->as_string("1.0^"))
        ->as_input();
  } else {
    atc_home_info.pin.from_string(THEKERNEL->config->value(atc_checksum, endstop_pin_checksum)->as_string("1.4^"))
        ->as_input();
  }
  ...
  detector_info.detect_pin
      .from_string(THEKERNEL->config->value(atc_checksum, detector_checksum, detect_pin_checksum)->as_string("0.20^"))
      ->as_input();
  detector_info.detect_rate =
      THEKERNEL->config->value(atc_checksum, detector_checksum, detect_rate_mm_s_checksum)->as_number(1);
  detector_info.detect_travel =
      THEKERNEL->config->value(atc_checksum, detector_checksum, detect_travel_mm_checksum)->as_number(1);
  ...
  this->probe_fast_rate =
      THEKERNEL->config->value(atc_checksum, probe_checksum, fast_rate_mm_m_checksum)->as_number(300);
  this->probe_slow_rate =
      THEKERNEL->config->value(atc_checksum, probe_checksum, slow_rate_mm_m_checksum)->as_number(60);
  this->probe_retract_mm = THEKERNEL->config->value(atc_checksum, probe_checksum, retract_mm_checksum)->as_number(2);
  this->probe_height_mm =
      THEKERNEL->config->value(atc_checksum, probe_checksum, probe_height_mm_checksum)->as_number(0);
  ...
  if (CARVERA == THEKERNEL->factory_set->MachineModel) {
    ...
    this->rotation_offset_y =
        THEKERNEL->config->value(coordinate_checksum, rotation_offset_y_checksum)->as_number(37.5F);
    this->rotation_offset_z =
        THEKERNEL->config->value(coordinate_checksum, rotation_offset_z_checksum)->as_number(22.5F);
    ...
    this->rotation_offset_x =
        THEKERNEL->config->value(coordinate_checksum, rotation_offset_x_checksum)->as_number(30.0F);
    this->rotation_offset_y =
        THEKERNEL->config->value(coordinate_checksum, rotation_offset_y_checksum)->as_number(82.5F);
    this->rotation_offset_z =
        THEKERNEL->config->value(coordinate_checksum, rotation_offset_z_checksum)->as_number(23.0F);
  ...
  this->three_axis_probe_tlo_correction =
      THEKERNEL->config->value(zprobe_checksum, three_axis_probe_tlo_correction_checksum)->as_number(0.0f);
  if (CARVERA == THEKERNEL->factory_set->MachineModel || CARVERA_AIR == THEKERNEL->factory_set->MachineModel) {
    this->ref_tool_mz = THEKERNEL->config->value(coordinate_checksum, reference_tool_mz_checksum)
                            ->as_number(-115.34f);  // Represents the machine Z coordinate when the tool length is 0
  } else {
    this->ref_tool_mz =
        THEKERNEL->config->value(coordinate_checksum, reference_tool_mz_checksum)
            ->as_number(-115.34f);  // In preparation for the Z1. Update this value when the Z1 is implemented
  }
}
...
void ATCHandler::set_tool_offset(uint8_t repeat_count) {
  ...
  if (ps == 1) {
    ...
    if (ref_tool_mz < 1) {
      ...
      THEKERNEL->streams->printf(
          "ERROR: warning, unexpected reference tool length found, reset machine then recalibrate tool\n");
      THEKERNEL->call_event(ON_HALT, nullptr);
      return;
    }
  }
...
void ATCHandler::set_tlo_by_offset(float z_axis_offset) {
  ...
  if (THEROBOT->compensationTransform)
    THEROBOT->compensationTransform(mpos.data(), true, false);  // get inverse compensation transform
    ...
    if (gcode->m == 6 && gcode->has_letter('T')) {
      ...
      if (THEKERNEL->factory_set->FuncSetting & (1 << 2))  // ATC
      {
        if (new_tool > this->max_manual_tool_number) {
          ...
          bool active_is_atc_tool = (this->active_tool >= this->lowest_tool_number &&
                                     this->active_tool <= this->tool_number)  // tool is inside the atc tool range
                                    || this->is_custom_tool_defined(this->active_tool);  // or tool is a custom tool

          bool new_is_atc_tool = (new_tool >= this->lowest_tool_number &&
                                  new_tool <= this->tool_number)             // tool is inside the atc tool range
          ...
          if (manual_pickup) {
            ...
            this->fill_cali_scripts(false, true);
                   ...
                   (new_tool != -1 ||
                    (CARVERA == THEKERNEL->factory_set->MachineModel && THEKERNEL->get_laser_mode()))) {
      ...
      if (gcode->subcode == 0 || gcode->subcode == 1) {
        ...
        if (gcode->has_letter('D')) {  // set stored tool diameter
          ...
          if (THEROBOT->is_compensation_active()) {
            ...
            if (is_playing) {
              ...
              THEKERNEL->streams->printf(
                  "WARNING: Tool diameter not changed - G41/G42 compensation is active. Issue G40 first.\n");
              return;
            }
          }
        ...
        if (gcode->has_letter('W')) {  // set additive diameter wear
          if (THEROBOT->is_compensation_active()) {
            ...
            if (is_playing) {
              THEKERNEL->streams->printf(
                  "ALARM: Cannot change tool diameter wear while G41/G42 compensation is active\n");
              THEKERNEL->call_event(ON_HALT, nullptr);
              THEKERNEL->set_halt_reason(MANUAL);
              return;
            } else {
              THEKERNEL->streams->printf(
                  "WARNING: Tool diameter wear not changed - G41/G42 compensation is active. Issue G40 first.\n");
              return;
            }
          }
      ...
      if (gcode->subcode == 3) {
        ...
        if (gcode->has_letter('X') && gcode->has_letter('Y')) {
          ...
          if ((this->active_tool == 0 || this->active_tool >= 999990 || this->active_tool == 9999) &&
              THEROBOT->get_tool_not_calibrated()) {
          ...
          if (gcode->has_letter('A') && gcode->has_letter('B') && gcode->has_letter('I') && gcode->has_letter('J') &&
              gcode->has_letter('H')) {
            leveling = true;
          ...
          if (margin || zprobe || leveling) {
            ...
            if (active_tool != 0 && active_tool < 999990) {
              ...
              {
                // drop current tool
                if (this->active_tool >= this->lowest_tool_number &&
                    (this->is_custom_tool_defined(this->active_tool) ||
                     this->active_tool <= this->tool_number)) {  // drop atc tool
                  THEKERNEL->streams->printf("Start dropping current tool: T%d\r\n", this->active_tool);
                  // just drop tool
                  this->fill_drop_scripts(active_tool);
                } else if ((this->active_tool > this->tool_number || this->active_tool < this->lowest_tool_number) &&
                           !this->is_custom_tool_defined(this->active_tool) &&
                           this->active_tool != -1) {  // drop manual tool
                  THEKERNEL->streams->printf("Start manually dropping current tool: T%d\r\n", this->active_tool);
                  this->target_tool = -1;
                  if (CARVERA == THEKERNEL->factory_set->MachineModel) {
            ...
            if (leveling) {
              gcode->stream->printf("Auto leveling, grid: %d * %d height: %1.2f\r\n", x_level_grids, y_level_grids,
                                    z_level_height);
              this->fill_autolevel_scripts(x_path_pos, y_path_pos, x_level_size, y_level_size, x_level_grids,
                                           y_level_grids, z_level_height);
            }
            if (gcode->has_letter('P')) {
      ...
      if (gcode->subcode == 0 || gcode->subcode == 1) {
        ...
        for (int wcs_index = 0; wcs_index < 6; wcs_index++) {
          THEKERNEL->streams->printf(
              "EEPRROM Data: G5%d: %1.3f, %1.3f, %1.3f, %1.3f | R:%1.3f\n", wcs_index + 4,
              THEKERNEL->eeprom_data->WCScoord[wcs_index][0], THEKERNEL->eeprom_data->WCScoord[wcs_index][1],
              THEKERNEL->eeprom_data->WCScoord[wcs_index][2], THEKERNEL->eeprom_data->WCScoord[wcs_index][3],
              THEKERNEL->eeprom_data->WCSrotation[wcs_index]);
        }
      ...
      if (gcode->subcode == 0 || gcode->subcode == 1) {
        ...
        for (int i = 0; i <= tool_number; i++) {
          THEKERNEL->streams->printf("tool%d -- mx:%1.1f my:%1.1f mz:%1.1f\n", atc_tools[i].num,
                                     atc_tools[i].get_mx_mm(), atc_tools[i].get_my_mm(), atc_tools[i].get_mz_mm());
        }
      } else if (gcode->subcode == 3) {
        this->beep_complete();
      } else if (gcode->subcode == 4) {
...
void ATCHandler::on_main_loop(void* argument) {
  if (this->atc_status != NONE) {
    ...
    if (goto_position == 0 || goto_position == 1) {
      ...
      // goto anchor 2
      rapid_move(true, this->anchor1_x + this->anchor2_offset_x, this->anchor1_y + this->anchor2_offset_y, NAN, NAN,
                 NAN);
