...
void AnalogSpindleControl::on_module_loaded() {
  ...
  std::string switch_on_pin =
      THEKERNEL->config->value(spindle_checksum, spindle_switch_on_pin_checksum)->as_string("nc");
  switch_on = NULL;
...
void AnalogSpindleControl::turn_on() {
  if (switch_on != NULL) switch_on->set(true);
  spindle_on = true;
...
void AnalogSpindleControl::turn_off() {
  if (switch_on != NULL) switch_on->set(false);
  spindle_on = false;
