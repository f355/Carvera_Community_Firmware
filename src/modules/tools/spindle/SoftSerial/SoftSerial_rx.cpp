...
uint32_t overhead_us = 200 * 1000000 / SystemCoreClock;  // Random estimation of the overhead of mbed libs, makes slow
                                                         // devices like LPC812 @ 12MHz perform better
...
int SoftSerial::readable(void) { return out_valid; }
...
void SoftSerial::rx_handler(void) {
  ...
  if (rx_bit == _bits + 1) {
    switch (_parity) {
      case Forced1:
        if (val == 0) rx_error = true;
        return;
      case Forced0:
        if (val == 1) rx_error = true;
        return;
      case Even:
      case Odd:
        parity_count = val;
        for (int i = 0; i < _bits; i++) {
          if (((read_buffer >> i) & 0x01) == 1) parity_count = !parity_count;
        }
        if ((parity_count) && (_parity == Even)) rx_error = true;
        if ((!parity_count) && (_parity == Odd)) rx_error = true;
        return;
      case None:
