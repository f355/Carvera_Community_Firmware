...
void PWMSpindleControl::on_module_loaded() {
  ...
  alarm_pin.from_string(THEKERNEL->config->value(spindle_checksum, spindle_alarm_pin_checksum)->as_string("nc"))
      ->as_input();

  // Smoothing value is low pass filter time constant in seconds.
  float smoothing_time =
      THEKERNEL->config->value(spindle_checksum, spindle_control_smoothing_checksum)->as_number(0.1f);
  ...
  {
    Pin* smoothie_pin = new Pin();
    smoothie_pin->from_string(
        THEKERNEL->config->value(spindle_checksum, spindle_feedback_pin_checksum)->as_string("nc"));
    ...
    if (update_count > UPDATE_FREQ / 5) {
      ...
      if (CARVERA_AIR == THEKERNEL->factory_set->MachineModel) {
        if (target_change_count >
            UPDATE_FREQ * 5)  // when speed changed time < 7s,we didn't use D_ter,to rapid speed up/down
        {
...
void PWMSpindleControl::set_p_term(float p) { control_P_term = p; }

void PWMSpindleControl::set_i_term(float i) { control_I_term = i; }

void PWMSpindleControl::set_d_term(float d) { control_D_term = d; }
...
void PWMSpindleControl::set_factor(float new_factor) { factor = new_factor; }
