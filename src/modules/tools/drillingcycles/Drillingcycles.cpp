...
void Drillingcycles::make_hole(Gcode* gcode) {
  ...
  char x[16] = "";
  char y[16] = "";
  if (gcode->has_letter('X')) snprintf(x, sizeof(x), " X%1.4f", gcode->get_value('X'));
  if (gcode->has_letter('Y')) snprintf(y, sizeof(y), " Y%1.4f", gcode->get_value('Y'));
