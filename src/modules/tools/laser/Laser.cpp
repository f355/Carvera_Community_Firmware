...
void Laser::on_module_loaded() {
  ...
  if (pwm_pin == NULL) {
    THEKERNEL->streams->printf(
        "Error: Laser cannot use P%d.%d (P2.0 - P2.5, P1.18, P1.20, P1.21, P1.23, P1.24, P1.26, P3.25, P3.26 only). "
...
bool Laser::get_laser_power(float& power) const {
  ...
  // first, as this is an interrupt if that flag is not clear then it cannot be cleared while this is running and the
  // block will still be valid (albeit it may have finished)
  if (block != nullptr && block->is_ready && block->is_g123) {
    // 2024
    float requested_power =
        (float)block->s_value / (1 << 11) / this->laser_maximum_s_value;  // s_value is 1.11 Fixed point
    ...
    float requested_power = (float)block->s_values[idx] / (1 << 11) / this->laser_maximum_s_value; // s_value is 1.11
    Fixed point float ratio = current_speed_ratio(block); power = requested_power * ratio * scale; return true;
    */
  }
...
uint32_t Laser::set_proportional_power(uint32_t dummy) {
  ...
  if (laser_on) {
    ...
    if (get_laser_power(power)) {
      ...
      float proportional_power =
          ((this->laser_maximum_power - this->laser_minimum_power) * power) + this->laser_minimum_power;
