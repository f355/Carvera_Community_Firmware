...
Endstops::Endstops() { this->status = NOT_HOMING; }
...
bool Endstops::load_old_config() {
  ...
  if (THEKERNEL->factory_set->FuncSetting & ((1 << 0) | (1 << 1))) {
    for (int i = X_AXIS; i <= B_AXIS; ++i) {  // X_AXIS to B_AXIS
      ...
      hinfo.home_direction =
          THEKERNEL->config->value(checksums[i][DIRECTION])->as_string("home_to_min") != "home_to_max";

      // homing cartesian position
      hinfo.homing_position = hinfo.home_direction ? THEKERNEL->config->value(checksums[i][MIN])->as_number(0)
                                                   : THEKERNEL->config->value(checksums[i][MAX])->as_number(200);
...
bool Endstops::load_config() {
  ...
  for (auto cs : modules) {
    ...
    pin_info->pin.from_string(THEKERNEL->config->value(endstop_checksum, cs, pin_checksum)->as_string("nc"))
        ->as_input();
    ...
    // homing cartesian position
    hinfo.homing_position =
        THEKERNEL->config->value(endstop_checksum, cs, position_checksum)->as_number(hinfo.home_direction ? 0 : 200);
...
void Endstops::back_off_home(axis_bitmap_t axis) {
  ...
  if (is_delta) {
    ...
    params.push_back(
        {'Z', THEROBOT->from_millimeters(homing_axis[Z_AXIS].retract * (homing_axis[Z_AXIS].home_direction ? 1 : -1))});
...
void Endstops::move_to_origin(axis_bitmap_t axis) {
  ...
  snprintf(buf, sizeof(buf), "G53 G0 X0 Y0 F%1.4f",
           THEROBOT->from_millimeters(rate));  // must use machine coordinates in case G92 or WCS is in effect
...
uint32_t Endstops::read_endstops(uint32_t dummy) {
  ...
  for (auto& e : homing_axis) {           // check all axis homing endstops
    ...
    // for corexy homing in X or Y we must only check the associated endstop, works as we only home one axis at a time
    // for corexy
...
bool Endstops::slow_approach_axis(int axis_index, float feed_rate_mm_s, float& result_mm,
                                  float approach_retract_multiplier) {
...
void Endstops::test_endstop_repeatability(Gcode* gcode) {
  ...
  for (auto& p : homing_axis) {
    ...
    gcode->stream->printf("  step: %.4f %s, poll: %.4f %s @ %.2f %s\n", step_resolution, unit, poll_resolution, unit,
                          tap_feed_rate, rate_unit);
...
void Endstops::process_home_command(Gcode* gcode) {
  ...
  if (!home_in_z_only) {  // ie not a delta
    ...
    if (home_z) {
      ...
      haxis.set(Z_AXIS);
      // we also set the kinematics to a known good position, this is necessary for a rotary delta, but doesn't hurt for
      // linear delta
  ...
  if (homing_order != 0 && !is_scara) {
    ...
    // homing order is 0bfffeeedddcccbbbaaa where aaa is 1,2,3,4,5,6 to specify the first axis (XYZABC), bbb is the
    // second and ccc is the third etc eg 0b0101011001010 would be Y X Z A, 011 010 001 100 101 would be  B A X Y Z
  ...
  if (home_in_z_only || is_scara) {  // deltas and scaras only
    ...
    // home_offset is the theta offset for each actuator, so M206 is used to set theta offset for each actuator in
    // degrees
    // FIXME not sure this will work with compensation transforms on.
    float ideal_position[3] = {homing_axis[X_AXIS].homing_position + homing_axis[X_AXIS].home_offset,
    ...
    if (has_endstop_trim) {
      ...
      ActuatorCoordinates real_actuator_position = {ideal_actuator_position[X_AXIS] - this->trim_mm[X_AXIS],
      ...
      if (is_rdelta) {
        // with a rotary delta we set the actuators angle then use the FK to calculate the resulting cartesian
        // coordinates
    ...
    // NOTE that if compensation is active the Z will be set based on where XY are, so make sure XY are homed first then
    // Z so XY are at a known consistent position.  (especially true if using a proximity probe)
    for (auto& p : homing_axis) {
      if (haxis[p.axis_index]) {  // if we requested this axis to home
        THEROBOT->reset_axis_position(p.homing_position + p.home_offset, p.axis_index);
        // set flag indicating axis was homed, it stays set once set until H/W reset or unhomed
  ...
  if (!is_delta) {
    ...
    // deltas are not left at 0,0 because of the trim settings, so move to 0,0 if requested, but we need to back off
    // endstops first also need to back off endstops if limits are enabled
    back_off_home(haxis);
    move_to_origin(haxis);
  }
  THEKERNEL->disable_endstops = previous_disable_endstops;
...
void Endstops::set_homing_offset(Gcode* gcode) {
  ...
  // Basically it finds the delta between the current MCS position and the requested position and adds it to the homing
  // offset then will not let it be set again until that axis is homed.
  float pos[3];
  ...
  gcode->stream->printf("Homing Offset: X %5.3f Y %5.3f Z %5.3f will take effect next home\n",
                        homing_axis[X_AXIS].home_offset, homing_axis[Y_AXIS].home_offset,
                        homing_axis[Z_AXIS].home_offset);
}
...
void Endstops::handle_park_g28() {
  ...
  // THEROBOT->push_state();
  // THEROBOT->absolute_mode = true;
  // snprintf(buf, sizeof(buf), "G53 G0 X%f Y%f", THEROBOT->from_millimeters(g28_position[X_AXIS]),
  // THEROBOT->from_millimeters(g28_position[Y_AXIS])); // must use machine coordinates in case G92 or WCS is in effect
...
void Endstops::on_gcode_received(void* argument) {
  ...
  if (gcode->has_g && gcode->g == 28) {
    switch (gcode->subcode) {
      ...
      case 4: {  // G28.4 is a smoothie special it sets manual homing based on the actuator position (used for rotary
                 // delta)
        // do a manual homing based on given coordinates, no endstops required
