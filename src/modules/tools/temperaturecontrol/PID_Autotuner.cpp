...
uint32_t PID_Autotuner::on_tick(uint32_t dummy) {
  if (temp_control != NULL) tick = true;
...
void PID_Autotuner::on_idle(void*) {
  ...
  if ((tickCnt % 1000) == 0) {
    THEKERNEL->streams->printf("// Autopid Status - %5.1f/%5.1f @%d %d/%d\n", refVal, target_temperature, output,
                               peakCount, requested_cycles);
  }
  ...
  if (justchanged && peakCount >= 4) {
    ...
    float avgSeparation =
        (fabsf(peaks[peakCount - 1] - peaks[peakCount - 2]) + fabsf(peaks[peakCount - 2] - peaks[peakCount - 3])) / 2;
    THEKERNEL->streams->printf("// Cycle %d: max: %g, min: %g, avg separation: %g\n", peakCount, absMax, absMin,
                               avgSeparation);
  ...
  if ((tickCnt % 1000) == 0) {
    DEBUG_PRINTF("lookBackCnt= %d, peakCount= %d, absmax= %g, absmin= %g, peak1= %lu, peak2= %lu\n", lookBackCnt,
                 peakCount, absMax, absMin, peak1, peak2);
  }
