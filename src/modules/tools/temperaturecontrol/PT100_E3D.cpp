...
void PT100_E3D::UpdateConfig(uint16_t module_checksum, uint16_t name_checksum) {
  ...
  this->amplifier_pin.from_string(
      THEKERNEL->config->value(module_checksum, name_checksum, e3d_amplifier_pin_checksum)->required()->as_string());
...
int PT100_E3D::new_pt100_reading() { return THEKERNEL->adc->read(&amplifier_pin); }
