...
void AD8495::UpdateConfig(uint16_t module_checksum, uint16_t name_checksum) {
  ...
  this->AD8495_pin.from_string(
      THEKERNEL->config->value(module_checksum, name_checksum, AD8495_pin_checksum)->required()->as_string());
  this->AD8495_offset = THEKERNEL->config->value(module_checksum, name_checksum, AD8495_offset_checksum)
                            ->as_number(0);  // Stated offset. For Adafruit board it is 250C. If pin 2(REF) of amplifier
                                             // is connected to 0V then there is 0C offset.
...
float AD8495::adc_value_to_temperature(uint32_t adc_value) {
  const uint32_t max_adc_value = THEKERNEL->adc->get_max_value();
  if ((adc_value >= max_adc_value)) return infinityf();
