...
void Thermistor::UpdateConfig(uint16_t module_checksum, uint16_t name_checksum) {
  ...
  bool use_beta_table =
      THEKERNEL->config->value(module_checksum, name_checksum, use_beta_table_checksum)->as_bool(false);
  ...
  if (!use_steinhart_hart) {
    this->beta =
        THEKERNEL->config->value(module_checksum, name_checksum, beta_checksum)
            ->as_number(
                this->beta);  // Thermistor beta rating. See http://reprap.org/bin/view/Main/MeasuringThermistorBeta
  }
  this->r0 = THEKERNEL->config->value(module_checksum, name_checksum, r0_checksum)
                 ->as_number(this->r0);  // Stated resistance eg. 100K
  this->t0 = THEKERNEL->config->value(module_checksum, name_checksum, t0_checksum)
                 ->as_number(this->t0);  // Temperature at stated resistance, eg. 25C
  ...
  this->thermistor_pin.from_string(
      THEKERNEL->config->value(module_checksum, name_checksum, thermistor_pin_checksum)->required()->as_string());
  ...
  if (!rtc.empty()) {
    // use the http://en.wikipedia.org/wiki/Steinhart-Hart_equation instead of beta, as it is more accurate over the
    // entire temp range we use three temps/resistor values taken from the thermistor R-C curve found in most datasheets
    // eg http://sensing.honeywell.com/resistance-temperature-conversion-table-no-16, we take the resistance for
    // 25°,150°,240° and the resistance in that table is 100000*R-T Curve coefficient eg
    // http://www.atcsemitec.co.uk/gt-2_thermistors.html for the semitec is even easier as they give the resistance in
    // column 8 of the R/T table
    ...
    std::tie(this->c1, this->c2, this->c3) =
...
std::tuple<float, float, float> Thermistor::calculate_steinhart_hart_coefficients(float t1, float r1, float t2,
                                                                                  float r2, float t3, float r3) {
  float l1 = logf(r1);
  float l2 = logf(r2);
  float l3 = logf(r3);
  ...
  if (c < 0) {
    THEKERNEL->streams->printf(
        "WARNING: negative coefficient in calculate_steinhart_hart_coefficients. Something may be wrong with the "
        "measurements\n");
    c = -c;
  }
...
void Thermistor::get_raw() {
  ...
  if (thermistor_number != 0) {
    string name = (thermistor_number & 0x80) ? predefined_thermistors_beta[(thermistor_number & 0x7F) - 1].name
                                             : predefined_thermistors[thermistor_number - 1].name;
    THEKERNEL->streams->printf("Using predefined thermistor %d in %s table: %s\n", thermistor_number & 0x7F,
                               (thermistor_number & 0x80) ? "Beta" : "S/H", name.c_str());
  }
...
float Thermistor::adc_value_to_temperature(uint32_t adc_value) {
  const uint32_t max_adc_value = THEKERNEL->adc->get_max_value();
  if ((adc_value >= max_adc_value) || (adc_value == 0)) return infinityf();
...
bool Thermistor::set_optional(const sensor_options_t& options) {
  ...
  bool error = false;
  // if in Steinhart-Hart mode make sure B is specified, if in beta mode make sure all C1,C2,C3 are set and no beta
  // settings this is needed if swapping between modes
  if (use_steinhart_hart && define_shh == 0 && !define_beta)
    error = true;  // if switching from SHH to beta need to specify new beta
  if (!use_steinhart_hart && define_shh > 0 && (define_beta || change_beta))
    error = true;  // if in beta mode and switching to SHH malke sure no beta settings are set
  if (!use_steinhart_hart && !(define_beta || change_beta) && define_shh != 3)
    error = true;  // if in beta mode and switching to SHH must specify all three SHH
  if (use_steinhart_hart && define_shh > 0 && (define_beta || change_beta))
    error = true;  // if setting SHH anfd already in SHH do not specify any beta values
