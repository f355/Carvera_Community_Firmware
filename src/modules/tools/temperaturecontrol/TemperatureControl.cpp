...
TemperatureControl::~TemperatureControl() { delete sensor; }
...
void TemperatureControl::on_idle(void* arg) { sensor->on_idle(); }
    ...
    if (isinf(this->get_temperature())) {  // undefined
      if (this->name_checksum == spindle_temperature_checksum) {
        ...
        THEKERNEL->streams->printf(
            "ERROR: Power cabinet temperature undefined, check wiring, max - %f°C, current - %f°C !\n", max_temp,
            get_temperature());
...
void TemperatureControl::load_config() {
  // General config
  this->set_m_code =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, set_m_code_checksum)->as_number(104);
  this->set_and_wait_m_code =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, set_and_wait_m_code_checksum)
          ->as_number(109);
  this->get_m_code =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, get_m_code_checksum)->as_number(105);
  this->readings_per_second =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, readings_per_second_checksum)
          ->as_number(20);

  this->designator = THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, designator_checksum)
                         ->as_string(string("T"));

  // Runaway parameters
  uint32_t n = THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, runaway_range_checksum)
                   ->as_number(20);
  if (n > 63) n = 63;
  this->runaway_range = n;

  // these need to fit in 9 bits after dividing by 8 so max is 4088 secs or 68 minutes
  n = THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, runaway_heating_timeout_checksum)
          ->as_number(900);
  if (n > 4088) n = 4088;
  this->runaway_heating_timeout = n / 8;  // we have 8 second ticks
  n = THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, runaway_cooling_timeout_checksum)
          ->as_number(0);  // disable by default
  if (n > 4088) n = 4088;
  this->runaway_cooling_timeout = n / 8;

  this->runaway_error_range =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, runaway_error_range_checksum)
          ->as_number(1.0F);

  // Max and min temperatures we are not allowed to get over (Safety)
  this->max_temp =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, max_temp_checksum)->as_number(300);
  this->min_temp =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, min_temp_checksum)->as_number(0);

  // Heater pin
  this->heater_pin.from_string(
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, heater_pin_checksum)
          ->as_string("nc"));
  ...
  std::string sensor_type = THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, sensor_checksum)
                                ->as_string("thermistor");
  ...
  this->preset1 =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, preset1_checksum)->as_number(0);
  this->preset2 =
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, preset2_checksum)->as_number(0);
  ...
  if (!this->readonly) {
    ...
    this->use_bangbang =
        THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, bang_bang_checksum)->as_bool(false);
    this->hysteresis =
        THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, hysteresis_checksum)->as_number(2);
    this->windup =
        THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, windup_checksum)->as_bool(false);
    this->heater_pin.max_pwm(
        THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, max_pwm_checksum)->as_number(255));
    this->heater_pin.set(0);
    set_low_on_debug(heater_pin.port_number, heater_pin.pin);
    // activate SD-DAC timer
    THEKERNEL->slow_ticker->attach(
        THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, pwm_frequency_checksum)
            ->as_number(2000),
        &heater_pin, &Pwm::on_tick);
  }
  ...
  // PID
  setPIDp(
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, p_factor_checksum)->as_number(10));
  setPIDi(
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, i_factor_checksum)->as_number(0.3f));
  setPIDd(
      THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, d_factor_checksum)->as_number(200));

  if (!this->readonly) {
    // set to the same as max_pwm by default
    this->i_max = THEKERNEL->config->value(temperature_control_checksum, this->name_checksum, i_max_checksum)
                      ->as_number(this->heater_pin.max_pwm());
  }
  ...
  if (gcode->has_m) {
    if (gcode->m == this->get_m_code) {
      ...
      int n = snprintf(buf, sizeof(buf), "%s:%3.1f /%3.1f @%d ", this->designator.c_str(), this->get_temperature(),
                       ((target_temperature <= 0) ? 0.0 : target_temperature), this->o);
      ...
      if (gcode->has_letter('S') && (gcode->get_value('S') == this->pool_index)) {
        ...
        if (sensor->get_optional(options)) {
          for (auto& i : options) {
            ...
            gcode->stream->printf("%s(S%d): %c %1.18f\n", this->designator.c_str(), this->pool_index, i.first,
                                  i.second);
          }
        }
      }
    ...
    if (gcode->m == 143) {
      if (gcode->has_letter('S') && (gcode->get_value('S') == this->pool_index)) {
        if (gcode->has_letter('P')) {
          ...
          gcode->stream->printf(
              "Nothing set NOTE Usage is M143 S0 P300 where <S> is the hotend index and <P> is the maximum temp to "
              "set\n");
        }
      ...
      if (gcode->has_letter('S') && (gcode->get_value('S') == this->pool_index)) {
        if (gcode->has_letter('P')) setPIDp(gcode->get_value('P'));
        if (gcode->has_letter('I')) setPIDi(gcode->get_value('I'));
        if (gcode->has_letter('D')) setPIDd(gcode->get_value('D'));
        if (gcode->has_letter('X')) this->i_max = gcode->get_value('X');
        if (gcode->has_letter('Y')) this->heater_pin.max_pwm(gcode->get_value('Y'));

      } else if (!gcode->has_letter('S')) {
        gcode->stream->printf("%s(S%d): Pf:%g If:%g Df:%g X(I_max):%g Y(max pwm):%d O:%d\n", this->designator.c_str(),
                              this->pool_index, this->p_factor, this->i_factor / this->PIDdt,
                              this->d_factor * this->PIDdt, this->i_max, this->heater_pin.max_pwm(), o);
      }
      ...
      gcode->stream->printf(";PID settings, i_max, max_pwm:\nM301 S%d P%1.4f I%1.4f D%1.4f X%1.4f Y%d\n",
                            this->pool_index, this->p_factor, this->i_factor / this->PIDdt,
                            this->d_factor * this->PIDdt, this->i_max, this->heater_pin.max_pwm());
      ...
      // this is safe as old configs as well as single extruder configs the toolmanager will not be running so will
      // return false this will also ignore anything that the tool manager is not controlling and return false,
      // otherwise it returns the active tool
      void* returned_data;
      bool ok =
          PublicData::get_value(tool_manager_checksum, is_active_tool_checksum, this->name_checksum, &returned_data);
      if (ok) {
        ...
        if (v == 0.0) {
          ...
          if (gcode->m == this->set_and_wait_m_code) {
            if (isinf(get_temperature()) && isinf(sensor->get_temperature())) {
              THEKERNEL->streams->printf(
                  "ERROR: Temperature reading is unreliable on %s HALT asserted - reset or M999 required\n",
                  designator.c_str());
              THEKERNEL->call_event(ON_HALT, nullptr);
              return;
            }
...
void TemperatureControl::set_desired_temperature(float desired_temperature) {
  ...
  if (desired_temperature <= 0.0F) {
    ...
    // set to whatever the output currently is See
...
float TemperatureControl::get_temperature() { return last_reading; }
...
void TemperatureControl::pid_process(float temperature) {
  if (use_bangbang) {
    // bang bang is very simple, if temp is < target - hysteresis turn on full else if  temp is > target + hysteresis
    // turn heater off good for relays
...
void TemperatureControl::on_second_tick(void* argument) {
  if (this->readonly) {
    ...
    if (waiting)
      THEKERNEL->streams->printf("%s:%3.1f /%3.1f @%d\n", designator.c_str(), get_temperature(),
                                 ((target_temperature <= 0) ? 0.0 : target_temperature), o);
    ...
    if (this->target_temperature <= 0) {  // If we are not trying to heat, state is NOT_HEATING
      ...
      switch (this->runaway_state) {
        case NOT_HEATING:  // If we were previously not trying to heat, but we are now, change to state
                           // WAITING_FOR_TEMP_TO_BE_REACHED
          this->runaway_state = (this->target_temperature >= current_temperature || this->runaway_cooling_timeout == 0)
                                    ? HEATING_UP
                                    : COOLING_DOWN;
        ...
        case COOLING_DOWN:
          ...
          if ((runaway_state == HEATING_UP &&
               current_temperature >= (this->target_temperature - this->runaway_error_range)) ||
              (runaway_state == COOLING_DOWN &&
               current_temperature <= (this->target_temperature + this->runaway_error_range))) {
            ...
            if (t > 0 && ++this->runaway_timer > t) {
              THEKERNEL->streams->printf(
                  "ERROR: Temperature took too long to be reached on %s, HALT asserted, TURN POWER OFF IMMEDIATELY - "
        ...
        case TARGET_TEMPERATURE_REACHED:
          if (this->runaway_range != 0) {
            ...
            // If the temperature is outside the acceptable range for 8 seconds, this allows for some noise spikes
            // without halting
            if (fabsf(delta) > this->runaway_range) {
              if (this->runaway_timer++ >= 1) {  // this being 8 seconds
                THEKERNEL->streams->printf(
                    "ERROR: Temperature runaway on %s (delta temp %f), HALT asserted, TURN POWER OFF IMMEDIATELY - "
                    "reset or M999 required\n",
...
void TemperatureControl::setPIDp(float p) { this->p_factor = p; }

void TemperatureControl::setPIDi(float i) { this->i_factor = i * this->PIDdt; }

void TemperatureControl::setPIDd(float d) { this->d_factor = d / this->PIDdt; }
