  ...
  if (gcode->has_m) {
    switch (gcode->m) {
      ...
      case 306: {
        ...
        // by doing M306 A-56.17 it will calculate the M206 A value (the theta offset for actuator A) based on the
        // difference between what it thinks is the current angle and what the current angle actually is specified by A
        // (ditto for B and C)

        ActuatorCoordinates current_angle;
        // get the current angle for each actuator, NOTE we only deal with  ABC so if there are more than 3 actuators
        // this will probably go wonky
        for (size_t i = 0; i < 3; i++) {
          current_angle[i] = THEROBOT->actuators[i]->get_current_position();
        }

        if (gcode->has_letter('L') && gcode->get_value('L') != 0) {
          // specifying L1 it will take the last probe position (set by G30 or G38.x ) and set the home offset based on
          // that this allows the use of G30 to find the angle tool
          uint8_t ok;
