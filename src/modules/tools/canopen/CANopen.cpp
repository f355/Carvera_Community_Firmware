...
{
    return command == 1 || command == 2 || command == 128 ||
...
CANopen::CANopen()
    : can(nullptr), enabled(false), master(true), bus_ready(false),
      bitrate(1000000), node_id(10), slave_node_id(1), heartbeat_ms(1000),
      last_heartbeat_us(0), last_remote_heartbeat_us(0), last_sdo_abort(0),
      remote_state(0), have_remote_heartbeat(false), have_last_rx(false)
{
}
...
void CANopen::configure()
{
    ...
    const int configured_bitrate =
        THEKERNEL->config->value(canopen_checksum, can_bitrate_checksum)->as_int(1000000);
    bitrate = configured_bitrate > 0 ? static_cast<uint32_t>(configured_bitrate) : 0;

    const int configured_node =
        THEKERNEL->config->value(canopen_checksum, can_node_id_checksum)->as_int(10);
    node_id = canopen::valid_node_id(configured_node) ? static_cast<uint8_t>(configured_node) : 0;

    const int configured_slave =
        THEKERNEL->config->value(canopen_checksum, slave_node_id_checksum)->as_int(1);
    slave_node_id = canopen::valid_node_id(configured_slave)
        ? static_cast<uint8_t>(configured_slave) : 0;
    ...
    heartbeat_ms = configured_heartbeat >= 0 && configured_heartbeat <= 60000
        ? static_cast<uint32_t>(configured_heartbeat) : UINT32_MAX;

    const std::string role =
        THEKERNEL->config->value(canopen_checksum, can_role_checksum)->as_string("master");
    master = role != "slave";
...
bool CANopen::start()
{
    if(!supported_bitrate(bitrate) || !canopen::valid_node_id(node_id) ||
       !canopen::valid_node_id(slave_node_id) || heartbeat_ms == UINT32_MAX) {
...
void CANopen::drain_rx()
{
    ...
    while(can->read(message)) handle_rx(message, us_ticker_read());
}

void CANopen::send_heartbeat(uint32_t now)
{
    if(heartbeat_ms == 0 || now - last_heartbeat_us < heartbeat_ms * 1000U) return;
    ...
    message.len = 1;
    message.data[0] = 5;
    if(!can->write(message, tx_timeout_us)) bus_ready = can->is_ready();
}
...
{
    if(argument != nullptr) have_last_rx = false;
}
...
bool CANopen::await_sdo(uint8_t node, uint16_t index, uint8_t subindex,
                        bool upload, uint32_t timeout_us, uint32_t& value, uint8_t& size)
{
    ...
    while(us_ticker_read() - started < timeout_us) {
        if(!can->read(message)) continue;
        handle_rx(message, us_ticker_read());
        const canopen::FrameView frame{
            message.id, message.len, message.format == CANStandard,
            message.type == CANData, message.data
        };
        const auto response =
            canopen::decode_sdo_response(frame, node, index, subindex, upload);
...
bool CANopen::sdo_read(uint8_t node, uint16_t index, uint8_t subindex,
...
bool CANopen::sdo_write(uint8_t node, uint16_t index, uint8_t subindex,
                        uint32_t value, uint8_t size)
{
    last_sdo_abort = 0;
    ...
    return await_sdo(node, index, subindex, false, sdo_timeout_us,
                     ignored_value, ignored_size);
}
...
void CANopen::report(StreamOutput* stream)
{
    ...
    const CANBus::Statistics statistics = can != nullptr
        ? can->statistics() : CANBus::Statistics{};
    stream->printf("  TX: %lu RX: %lu RX overflow: %lu TX failed: %lu TX timeout: %lu\n",
                   statistics.transmitted, statistics.received, statistics.rx_overflow,
                   statistics.tx_failed, statistics.tx_timeout);
    ...
    if(have_remote_heartbeat) {
        stream->printf("  Remote heartbeat: state %u, age %lu ms, %s\n",
                       remote_state, (now - last_remote_heartbeat_us) / 1000U,
                       remote_online(now) ? "online" : "stale");
    } else {
...
void CANopen::on_get_public_data(void* argument)
{
    ...
    if(have_last_rx) status.last_rx = last_rx;
    status.have_remote_heartbeat = have_remote_heartbeat;
    status.remote_online = remote_online(now);
    status.remote_state = remote_state;
    status.remote_heartbeat_age_ms = have_remote_heartbeat
        ? (now - last_remote_heartbeat_us) / 1000U : 0;
...
{
    PublicDataRequest* request = static_cast<PublicDataRequest*>(argument);
    if(!request->starts_with(canopen_checksum) ||
       !request->second_element_is(send_frame_checksum)) return;

    const canopen_frame* frame = static_cast<const canopen_frame*>(request->get_data_ptr());
    if(frame == nullptr || !enabled || !bus_ready || frame->len > 8 ||
       frame->type != CANData || frame->format != CANStandard) return;

    mbed::CANMessage message;
    message.id = frame->id;
    message.len = frame->len;
    std::memcpy(message.data, frame->data, frame->len);
    if(can->write(message, tx_timeout_us)) request->set_taken();
    bus_ready = can->is_ready();
}
