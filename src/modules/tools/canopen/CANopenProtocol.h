...
constexpr SdoResponse decode_sdo_response(FrameView frame, uint8_t node,
                                           ...
                                           bool expect_upload)
{
    ...
    if(frame.data[1] != (index & 0xff) || frame.data[2] != (index >> 8) ||
    ...
    if(!expect_upload) {
        return command == 0x60
            ? SdoResponse{SdoResponseKind::download, 0, 0, 0}
            : malformed;
    }

    if((command & 0xe0) != 0x40 || (command & 0x10) != 0 ||
       (command & 0x02) == 0) return malformed;
