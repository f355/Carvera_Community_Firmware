...
void ZProbe::config_load() {
  ...
  this->calibrate_pin.from_string(THEKERNEL->config->value(zprobe_checksum, calibrate_pin_checksum)->as_string("0.5^"))
      ->as_input();
  ...
  this->probe_calibration_safety_margin =
      THEKERNEL->config->value(zprobe_checksum, probe_calibration_safety_margin_checksum)->as_number(0.1F);
  ...
  this->slow_feedrate =
      THEKERNEL->config->value(zprobe_checksum, slow_feedrate_checksum)->as_number(5);  // feedrate in mm/sec
  this->fast_feedrate =
      THEKERNEL->config->value(zprobe_checksum, fast_feedrate_checksum)->as_number(100);  // feedrate in mm/sec
  this->return_feedrate =
      THEKERNEL->config->value(zprobe_checksum, return_feedrate_checksum)->as_number(5);  // feedrate in mm/sec
  this->reverse_z = THEKERNEL->config->value(zprobe_checksum, reverse_z_direction_checksum)
                        ->as_bool(false);  // Z probe moves in reverse direction
  ...
  THEKERNEL->probe_tip_diameter =
      THEKERNEL->config->value(zprobe_checksum, probe_tip_diameter_checksum)->as_number(2);  // probe tip diameter
  ...
  this->dwell_before_probing = THEKERNEL->config->value(zprobe_checksum, dwell_before_probing_checksum)
                                   ->as_number(0);  // dwell time in seconds before probing
}
...
uint32_t ZProbe::probe_doubleHit(uint32_t dummy) {
  ...
  if (this->pin.get()) {
    ...
    if (bfirstHitDetected) bNoHited = true;
    ...
    switch (gcode->m) {
      ...
      case 466:
        if (gcode->subcode == 1) {
          if (!gcode->has_letter('X') || !gcode->has_letter('Y') || !gcode->has_letter('H')) {
            gcode->stream->printf(
                "ERROR: Probe fail: Both X and Y axis and height need to be set for Square Probing\n");
            THEKERNEL->call_event(ON_HALT, nullptr);
            THEKERNEL->set_halt_reason(PROBE_FAIL);
            return;
          }
      ...
      case 670:
        ...
        if (gcode->has_letter('I')) {  // NOTE this is temporary and toggles the invertion status of the pin
          ...
          pin.set_inverting(pin.is_inverting() !=
                            invert_override);  // XOR so inverted pin is not inverted and vice versa
      ...
      case 503:  // print settings
        gcode->stream->printf(
            ...
            "R%1.2f Z%1.2f H%1.2f D%1.2f\n",
            this->slow_feedrate, this->fast_feedrate, this->return_feedrate, this->max_z, this->probe_height,
            this->dwell_before_probing);

        // fall through is intended so leveling strategies can handle m-codes too. Next line informs the compiler, do
        // not edit it fall through
...
bool ZProbe::probe_XYZ(Gcode* gcode) {
  ...
  if (THEKERNEL->is_flex_compensation_active()) {
    if (THEROBOT->compensationTransform)
      THEROBOT->compensationTransform(pos, true, false);  // get inverse compensation transform
  }
  ...
  gcode->stream->printf("[PRB:%1.3f,%1.3f,%1.3f:%d]\n", THEKERNEL->robot->from_millimeters(pos[X_AXIS]),
                        ...
                        THEKERNEL->robot->from_millimeters(pos[Z_AXIS]), probeok);
...
void ZProbe::set_tlo_calibrating(bool state) { tlo_calibrating = state; }
...
void ZProbe::calibrate_Z(Gcode* gcode) {
  ...
  gcode->stream->printf("[PRB:%1.3f,%1.3f,%1.3f:%d]\n", THEKERNEL->robot->from_millimeters(pos[X_AXIS]),
                        THEKERNEL->robot->from_millimeters(pos[Y_AXIS]),
                        THEKERNEL->robot->from_millimeters(pos[Z_AXIS]), calibrateok);
...
float ZProbe::get_xyz_move_length(float x, float y, float z) { return sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2)); }
...
bool ZProbe::fast_slow_probe_sequence(int axis, int direction) {
  ...
  std::sprintf(this->buff, "G38.%i X%.3f Y%.3f Z%.3f F%.3f", 2 + param.probe_g38_subcode, THEROBOT->from_millimeters(x),
               THEROBOT->from_millimeters(y), THEROBOT->from_millimeters(z), param.feed_rate);
  ...
  // slow probe
  memset(&this->buff, 0, sizeof(this->buff));
  std::sprintf(this->buff, "G38.%i X%.3f Y%.3f Z%.3f", 2 + param.probe_g38_subcode, THEROBOT->from_millimeters(x),
               THEROBOT->from_millimeters(y), THEROBOT->from_millimeters(z));
  this->gcodeBuffer = new Gcode(this->buff, &StreamOutput::NullStream);
  probe_XYZ(this->gcodeBuffer);
  delete gcodeBuffer;
  ...
  if (THEKERNEL->is_flex_compensation_active()) {
    if (THEROBOT->compensationTransform)
      THEROBOT->compensationTransform(mpos, true, false);  // get inverse compensation transform
  }
...
int ZProbe::xy_probe_move_alarm_when_hit(int direction, int probe_g38_subcode, float x, float y, float feed_rate) {
  ...
  memset(&this->buff, 0, sizeof(this->buff));
  std::sprintf(this->buff, "G38.%i X%.3f Y%.3f F%.3f", 3 + probe_g38_subcode, THEROBOT->from_millimeters(direction * x),
               THEROBOT->from_millimeters(direction * y), feed_rate);
  this->gcodeBuffer = new Gcode(this->buff, &StreamOutput::NullStream);
...
bool ZProbe::parse_parameters(Gcode* gcode, bool override_probe_check) {
  ...
  if (!((override_probe_check && THEKERNEL->eeprom_data->TOOL == 0) ||
        (this->tool_0_3axis && THEKERNEL->eeprom_data->TOOL == 0) || THEKERNEL->eeprom_data->TOOL >= 999990 ||
        THEKERNEL->eeprom_data->TOOL == 9999)) {
    THEKERNEL->streams->printf(
        "ERROR: Attempted to 3 axis probe with an improper tool number. Tool number needs to be >= 999990\n or you "
        "need to set tool 0 as a 3 axis probe with: \n config-set sd zprobe.tool_zero_is_3axis true \n");
    THEKERNEL->call_event(ON_HALT, nullptr);
    THEKERNEL->set_halt_reason(PROBE_FAIL);
    return false;
  } else if (THEROBOT->get_tool_not_calibrated() && gcode->has_letter('S') &&
             (gcode->has_letter('H') || gcode->has_letter('Z'))) {
...
void ZProbe::probe_bore(bool calibration)  // M461
{
  ...
  for (int i = 0; i < param.repeat; i++) {
    if (param.x_axis_distance != 0) {
      ...
      THEKERNEL->streams->printf(
          "Distance Point 2 X surfaces (Diameter) is: %.3f and center is stored at variable #151\n",
...
void ZProbe::probe_boss(bool calibration)  // M462
{
  ...
  for (int i = 0; i < repeat_count; i++) {
    ...
    if (probe_x_axis) {
      ...
      if (xy_probe_move_alarm_when_hit(POS, param.probe_g38_subcode, param.x_rotated_x, param.x_rotated_y,
                                       param.rapid_rate) == 1) {
        return;
      }
      // probe z no hit no alarm -side_depth, retract slightly if probe point hit
      if (param.probe_height != 0) {
        z_probe_move_with_retract(param.probe_g38_subcode,
                                  -(param.side_depth + param.clearance_height + param.tool_dia / 2.0), 1.0,
                                  param.rapid_rate);
      } else {
        z_probe_move_with_retract(param.probe_g38_subcode, -(param.side_depth + param.tool_dia / 2.0), 1.0,
                                  param.rapid_rate);
      }
      ...
      // return if probe touches wall during outside move
      if (xy_probe_move_alarm_when_hit(NEG, param.probe_g38_subcode, param.x_rotated_x, param.x_rotated_y,
                                       param.rapid_rate) == 1) {
        return;
      }
    ...
    if (probe_y_axis) {
      // return if probe touches wall during outside move
      if (xy_probe_move_alarm_when_hit(POS, param.probe_g38_subcode, param.y_rotated_x, param.y_rotated_y,
                                       param.rapid_rate) == 1) {
        return;
      }
      ...
      // return if probe touches wall during outside move
      if (xy_probe_move_alarm_when_hit(NEG, param.probe_g38_subcode, param.y_rotated_x, param.y_rotated_y,
                                       param.rapid_rate) == 1) {
        return;
      }
...
void ZProbe::probe_insideCorner()  // M463
{
  ...
  rotate(X_AXIS, param.x_axis_distance, &param.x_rotated_x, &param.x_rotated_y, param.rotation_angle);
  rotate(Y_AXIS, param.y_axis_distance, &param.y_rotated_x, &param.y_rotated_y, param.rotation_angle);
  rotate(X_AXIS, (param.tool_dia / 2.0), &param.half_tool_dia_rotated_x_x, &param.half_tool_dia_rotated_x_y,
         param.rotation_angle_mcs);
  rotate(Y_AXIS, (param.tool_dia / 2.0), &param.half_tool_dia_rotated_y_x, &param.half_tool_dia_rotated_y_y,
         param.rotation_angle_mcs);
  ...
  for (int i = 0; i < param.repeat; i++) {
    ...
    out_coords.x_positive_x_out =
        out_coords.x_positive_x_out + (param.x_axis_distance >= 0 ? 1.0f : -1.0f) * param.half_tool_dia_rotated_x_x;
    out_coords.x_positive_y_out =
        out_coords.x_positive_y_out + (param.x_axis_distance >= 0 ? 1.0f : -1.0f) * param.half_tool_dia_rotated_x_y;
    ...
    out_coords.y_positive_y_out =
        out_coords.y_positive_y_out + (param.y_axis_distance >= 0 ? 1.0f : -1.0f) * param.half_tool_dia_rotated_y_y;
    out_coords.y_positive_x_out =
        out_coords.y_positive_x_out + (param.y_axis_distance >= 0 ? 1.0f : -1.0f) * param.half_tool_dia_rotated_y_x;
  ...
  THEKERNEL->streams->printf(
      "Corner found. X coordinate stored in #154 as MCS %.3f , Y coordinate in #155 as MCS %.3f \n",
      THEKERNEL->probe_outputs[3], THEKERNEL->probe_outputs[4]);
...
void ZProbe::probe_outsideCorner()  // M464
{
  ...
  if (param.save_position == 2) {
    ...
    THECONVEYOR->wait_for_idle();
    coordinated_move(THEKERNEL->probe_outputs[3], THEKERNEL->probe_outputs[4], out_coords.z_negative_z_out + 2,
                     param.rapid_rate);
    THECONVEYOR->wait_for_idle();
  } else {
    coordinated_move(NAN, NAN, param.clearance_world_pos, param.rapid_rate);
    THECONVEYOR->wait_for_idle();
    coordinated_move(THEKERNEL->probe_outputs[3], THEKERNEL->probe_outputs[4], param.clearance_world_pos,
                     param.rapid_rate);
    THECONVEYOR->wait_for_idle();
  }
}
...
void ZProbe::probe_axisangle(bool probe_a_axis, bool probe_with_offset)  // M465
{
  ...
  if (probe_with_offset) {
    ...
    if (isnan(machine_offset.anchor1_x) || isnan(machine_offset.anchor1_y) || isnan(machine_offset.rotation_offset_x) ||
        isnan(machine_offset.rotation_offset_y)) {
  ...
  if (!probe_a_axis) {
    if (param.x_axis_distance != 0) {
      ...
      if (param.visualize_path_distance != 0) {
        param.visualize_path_distance =
            fabs(param.visualize_path_distance) * (param.x_axis_distance / fabs(param.x_axis_distance));
      }
      param.y_axis_distance = param.side_depth;
    } else {
      if (param.visualize_path_distance != 0) {
        param.visualize_path_distance =
            fabs(param.visualize_path_distance) * (param.y_axis_distance / fabs(param.y_axis_distance));
      }
  ...
  THECONVEYOR->wait_for_idle();
  // save center position to use later
  THEROBOT->get_current_machine_position(mpos);  // current_position/mpos includes the compensation transform so we need
                                                 // to get the inverse to get actual position
  ...
  for (int i = 0; i < param.repeat; i++) {
    ...
    if (probe_a_axis) {
      ...
      if (xy_probe_move_alarm_when_hit(NEG, param.probe_g38_subcode, 0.0, 2 * param.y_rotated_y, param.rapid_rate) ==
          1) {
      ...
      float current_angle =
          atan((out_coords.y_positive_y_out - out_coords.y_negative_y_out) / (2 * param.y_axis_distance)) * 180 / pi;
      ...
      THEKERNEL->streams->printf("Measurement %d: Angle from A Axis is: %.3f degrees or %.3f radians\n", i + 1,
                                 current_angle, current_angle * pi / 180);
      ...
      coordinated_move(out_coords.origin_x + param.x_rotated_x, out_coords.origin_y + param.x_rotated_y, NAN,
                       param.rapid_rate);
      THECONVEYOR->wait_for_idle();
      // return to center position
      coordinated_move(out_coords.origin_x, out_coords.origin_y, NAN, param.rapid_rate);
      ...
      coordinated_move(out_coords.origin_x + param.y_rotated_x, out_coords.origin_y + param.y_rotated_y, NAN,
                       param.rapid_rate);
      THECONVEYOR->wait_for_idle();
      // return to center position
      coordinated_move(out_coords.origin_x, out_coords.origin_y, NAN, param.rapid_rate);
  ...
  if (!angle_measurements.empty()) {
    ...
    if (probe_a_axis) {
      THEKERNEL->streams->printf(
          "Average angle from A Axis is: %.3f degrees or %.3f radians and is stored in radians at variable #153\n",
          average_angle, average_angle * pi / 180);
    } else if (probe_x) {
      THEKERNEL->streams->printf(
          "Average angle from X Axis is: %.3f degrees or %.3f radians and is stored in radians at variable #153\n",
          average_angle, average_angle * pi / 180);
    } else {
      THEKERNEL->streams->printf(
          "Average angle from Y Axis is: %.3f degrees or %.3f radians and is stored in radians at variable #153\n",
          average_angle, average_angle * pi / 180);
    }
  }
  ...
  if (param.visualize_path_distance != 0) {
    if (probe_a_axis) {
      ...
      if (probe_x) {
        xy_probe_move_alarm_when_hit(
            POS, param.probe_g38_subcode,
        ...
        xy_probe_move_alarm_when_hit(POS, param.probe_g38_subcode,
                                     THEROBOT->from_millimeters(param.visualize_path_distance *
                                                                cos((THEKERNEL->probe_outputs[2] + 90) * pi / 180)),
                                     THEROBOT->from_millimeters(param.visualize_path_distance *
                                                                sin((THEKERNEL->probe_outputs[2] + 90) * pi / 180)),
                                     param.rapid_rate);
      }
      // return to center position
      THECONVEYOR->wait_for_idle();
...
void ZProbe::probe_square() {
  ...
  for (int i = 0; i < param.repeat; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      THEKERNEL->streams->printf("Point %d: X: %.3f, Y: %.3f, Z: %.3f\n", j + 1, std::get<X_AXIS>(pos),
                                 std::get<Y_AXIS>(pos), std::get<Z_AXIS>(pos));
      if (j == min_z_index - 1) {
