/*
 ...
 nearest 4 points.
    ...
    The radius of the bed must be specified with...
      ...
      this needs to be at least as big as the maximum printing radius as moves outside of this will not be compensated
 for correctly
 ...
 autoload the grid on boot
    ...
    Optionally an initial_height can be set that tell the intial probe where to stop the fast decent before it probes,
 this should be around 5-10mm above the bed leveling-strategy.delta-grid.initial_height  10


    Usage
    -----
    G29 test probes in a spiral pattern within the radius producing a map of offsets, this can be imported into a
 graphing program to visualize the bed heights optional parameters {{In}} sets the number of points to the value n,
 {{Jn}} sets the radius for this probe.
...
DeltaGridStrategy::DeltaGridStrategy(ZProbe* zprobe) : LevelingStrategy(zprobe) { grid = nullptr; }
...
bool DeltaGridStrategy::handleConfig() {
  grid_size =
      THEKERNEL->config->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, grid_size_checksum)
          ->as_number(7);
  tolerance =
      THEKERNEL->config->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, tolerance_checksum)
          ->as_number(0.03F);
  save = THEKERNEL->config->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, save_checksum)
             ->as_bool(false);
  do_home =
      THEKERNEL->config->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, do_home_checksum)
          ->as_bool(true);
  is_square =
      THEKERNEL->config->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, is_square_checksum)
          ->as_bool(false);
  grid_radius =
      THEKERNEL->config->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, grid_radius_checksum)
          ->as_number(50.0F);
  ...
  this->initial_height =
      THEKERNEL->config
          ->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, initial_height_checksum)
          ->as_number(10);
  ...
  // Probe offsets xxx,yyy,zzz
  {
    std::string po =
        THEKERNEL->config
            ->value(leveling_strategy_checksum, delta_grid_leveling_strategy_checksum, probe_offsets_checksum)
            ->as_string("0,0,0");
...
void DeltaGridStrategy::setAdjustFunction(bool on) {
  if (on) {
    ...
    THEROBOT->compensationTransform =
        std::bind(&DeltaGridStrategy::doCompensation, this, _1,
                  _2);  // [this](float *target, bool inverse) { doCompensation(target, inverse); };
  } else {
    // clear it
...
float DeltaGridStrategy::findBed() {
  ...
  // move to an initial position fast so as to not take all day, we move down max_z - initial_height, which is set in
  // config, default 10mm
...
void DeltaGridStrategy::extrapolate_one_point(int x, int y, int xdir, int ydir) {
  ...
  float c = 2 * grid[(x + xdir) + ((y + ydir) * grid_size)] -
            grid[(x + xdir * 2) + ((y + ydir * 2) * grid_size)];  // Diagonal.
