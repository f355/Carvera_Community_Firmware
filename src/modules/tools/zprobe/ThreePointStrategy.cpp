/*
    ...
    Three probe points must be defined, these are best if they are the three points of an equilateral triangle, as far
   apart as possible. They can be defined in the config file as:-
    ...
    Usage
    -----
    G29 probes the three probe points and reports the Z at each point, if a plane is active it will be used to level the
   probe. G32 probes the three probe points and defines the bed plane, this will remain in effect until reset or M561
...
ThreePointStrategy::~ThreePointStrategy() { delete plane; }

bool ThreePointStrategy::handleConfig() {
  // format is xxx,yyy for the probe points
  std::string p1 =
      THEKERNEL->config
          ->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, probe_point_1_checksum)
          ->as_string("");
  std::string p2 =
      THEKERNEL->config
          ->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, probe_point_2_checksum)
          ->as_string("");
  std::string p3 =
      THEKERNEL->config
          ->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, probe_point_3_checksum)
          ->as_string("");
  ...
  std::string po =
      THEKERNEL->config
          ->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, probe_offsets_checksum)
          ->as_string("0,0,0");
  this->probe_offsets = parseXYZ(po.c_str());

  this->home =
      THEKERNEL->config->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, home_checksum)
          ->as_bool(true);
  this->tolerance =
      THEKERNEL->config->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, tolerance_checksum)
          ->as_number(0.03F);
  this->save =
      THEKERNEL->config->value(leveling_strategy_checksum, three_point_leveling_strategy_checksum, save_plane_checksum)
          ->as_bool(false);
  return true;
}
...
bool ThreePointStrategy::doProbing(StreamOutput* stream) {
  ...
  // the first probe point becomes Z == 0 effectively so if we home Z or manually set z after this, it needs to be at
  ...
  // TODO if using probe then we probably need to set Z to 0 at first probe point, but take into account probe offset
  // from head
  THEROBOT->reset_axis_position(std::get<Z_AXIS>(this->probe_offsets), Z_AXIS);

  // move up to specified probe start position
  zprobe->coordinated_move(NAN, NAN, zprobe->getProbeHeight(),
                           zprobe->getSlowFeedrate());  // move to probe start position
...
void ThreePointStrategy::setAdjustFunction(bool on) {
  if (on) {
    ...
    THEROBOT->compensationTransform = [this](float* target, bool inverse, bool debug) {
      if (inverse)
