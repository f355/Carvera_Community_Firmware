...
bool DeltaCalibrationStrategy::handleConfig() {
  ...
  float r = THEKERNEL->config->value(leveling_strategy_checksum, delta_calibration_strategy_checksum, radius_checksum)
                ->as_number(-1);
  if (r == -1) {
  ...
  this->initial_height =
      THEKERNEL->config->value(leveling_strategy_checksum, delta_calibration_strategy_checksum, initial_height_checksum)
          ->as_number(10);
  return true;
}
...
bool DeltaCalibrationStrategy::calibrate_delta_endstops(Gcode* gcode) {
  ...
  for (int i = 1; i <= 10; ++i) {
    ...
    zprobe->home();
    zprobe->coordinated_move(NAN, NAN, -bedht, zprobe->getFastFeedrate(),
