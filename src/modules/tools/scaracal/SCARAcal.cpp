...
void SCARAcal::on_config_reload(void* argument) {
  this->slow_rate =
      THEKERNEL->config->value(scaracal_checksum, slow_feedrate_checksum)->as_number(5);  // feedrate in mm/sec
  this->z_move = THEKERNEL->config->value(scaracal_checksum, z_move_checksum)
                     ->as_number(0);  // Optional movement of Z relative to the home position.
...
void SCARAcal::SCARA_ang_move(float theta, float psi, float z, float feedrate) {
  ...
  // Assemble Gcode to add onto the queue
  snprintf(cmd, sizeof(cmd), "G0 X%1.3f Y%1.3f Z%1.3f F%1.1f", cartesian[0], cartesian[1], cartesian[2],
           feedrate * 60);  // use specified feedrate (mm/sec)
  ...
  if (gcode->has_m) {
    switch (gcode->m) {
      case 114: {  // Extra stuff for Morgan calibration
        ...
        int n = snprintf(buf, sizeof(buf), "  A: Th:%1.3f Ps:%1.3f", actuators[0],
                         actuators[1]);  // display actuator angles Theta and Psi.
        gcode->txt_after_ok.append(buf, n);
      } break;

      case 360: {
        float target[2] = {0.0F, 120.0F}, cartesian[3], S_trim[3];
      ...
      case 361: {
        ...
        if (gcode->has_letter('P')) {
          ...
          STEPPER[0]->change_steps_per_mm(actuators[0] / target[0] *
                                          STEPPER[0]->get_steps_per_mm());  // Find angle difference
          STEPPER[1]->change_steps_per_mm(
              STEPPER[0]->get_steps_per_mm());  // and change steps_per_mm to ensure correct steps per *angle*
        } else {
      ...
      case 362: {
        float target[2] = {0.0F, 90.0F}, cartesian[3], S_trim[3];
      ...
      case 363: {
        float target[2] = {180.0F, 270.0F}, cartesian[3];
        ...
        if (gcode->has_letter('P')) {
          ...
          STEPPER[1]->change_steps_per_mm(actuators[1] / target[1] *
                                          STEPPER[1]->get_steps_per_mm());  // Find angle difference
          // STEPPER[1]->change_steps_per_mm(STEPPER[0]->get_steps_per_mm());  // and change steps_per_mm to ensure
          // correct steps per *angle*
        } else {
          this->home();                            // home - This time leave trims as adjusted.
          THEROBOT->get_axis_position(cartesian);  // get actual position from robot
          SCARA_ang_move(target[0], target[1], cartesian[2] + this->z_move, slow_rate * 3.0F);  // move to target
      ...
      case 364: {
        float target[2] = {45.0F, 135.0F}, cartesian[3], S_trim[3];
        ...
        if (gcode->has_letter('P')) {
          ...
          THEROBOT->get_axis_position(cartesian);  // get actual position from robot
          THEROBOT->arm_solution->cartesian_to_actuator(cartesian,
                                                        actuators);  // translate it to get actual actuator angles

          S_delta[1] = (actuators[1] - actuators[0]) -
                       (target[1] - target[0]);  // Find difference in angle - not actuator difference, and
