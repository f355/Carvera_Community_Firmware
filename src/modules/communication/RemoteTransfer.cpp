...
Result receive_records(Transport &transport, RecordSink &sink, const PacketTypes &types, uint32_t overall_timeout_ms)
{
    ...
    auto timeout = [&] { return fail(Result::timeout); };

    send_request();
    for (;;) {
        const uint32_t now_ms = transport.now_ms();
        if (now_ms - started_ms >= overall_timeout_ms) {
            return timeout();
        }
        ...
        if (!transport.receive(packet, 100)) {
            ++receive_failures;
            if (receive_failures > 25)
                return timeout();
            if (receive_failures % 5 == 0)
                send_request();
            continue;
        }
        if (!belongs_to(types, packet.type))
            continue;
        receive_failures = 0;
