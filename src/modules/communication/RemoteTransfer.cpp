...
constexpr uint16_t config_data_size = 0x84;
...
int receive(SerialConsole& serial, makera::Packet& packet, uint32_t timeout_ms) {
  return serial.receive_packet(packet, timeout_ms);
}
...
Result receive_records(SerialConsole& serial, const PacketTypes& types, StoreRecords store_records,
  ...
  for (;;) {
    ...
    const int receive_result = receive(serial, packet, 100);
    if (receive_result != 0) {
...
Result receive_factory_settings(SerialConsole& serial, const FACTORY_SET& current, FACTORY_SET& received) {
  ...
  const Result result = receive_records(
      ...
      [&] { return candidate.MachineModel == Z1 || candidate.MachineModel == Z1PRO; }, false);
