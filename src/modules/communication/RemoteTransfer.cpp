...
{
    return type == types.start || type == types.view || type == types.data ||
           type == types.finish || type == types.cancel;
}

} // namespace

Result receive_records(Transport& transport, RecordSink& sink,
                       const PacketTypes& types, uint32_t overall_timeout_ms)
{
    ...
    for (;;) {
        ...
        if (phase == Phase::view) {
            const uint16_t record_limit = packet.payload_length == 6
                ? static_cast<uint16_t>(
                    (static_cast<uint16_t>(packet.payload[4]) << 8) |
                    packet.payload[5])
                : 0;
        ...
        const uint32_t accepted = sink.accept(
            index, packet.payload + 4, packet.payload_length - 4);
        if (accepted == 0 || accepted > record_count - (next_index - 1)) {
            return fail(accepted == 0 ? Result::sink_error
