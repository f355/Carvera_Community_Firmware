...
static const int allowed_mcodes[] = {2,   5,  9,  30,  105, 114, 115,
                                     119, 80, 81, 911, 503, 106, 107};  // get temp, get pos, get endstops etc
...
void GcodeDispatch::on_module_loaded() { this->register_for_event(ON_CONSOLE_LINE_RECEIVED); }
  ...
  if (first_char == 'G' || first_char == 'M' || first_char == 'T' || first_char == 'S' || first_char == 'N' ||
    ...
    while (possible_command.size() > 0) {
      ...
      if (!uploading || upload_stream != new_message.stream) {
          ...
          switch (gcode->m) {
            ...
            case 112:  // emergency stop, do the best we can with this
              ...
              THEKERNEL->streams->printf(
                  "ERROR: ok Emergency Stop Requested - reset or M999 required to exit HALT state\r\n");
            ...
            case 115: {  // M115 Get firmware version and capabilities
              ...
              new_message.stream->printf(
                  ...
                  "X-SOURCE_CODE_URL:https://github.com/Smoothieware/Smoothieware, FIRMWARE_VERSION:%s, "
                  "X-FIRMWARE_BUILD_DATE:%s, X-SYSTEM_CLOCK:%ldMHz, X-AXES:%d, X-GRBL_MODE:%d",
            ...
            case 117:  // M117 is a special non compliant Gcode as it allows arbitrary text on the line following the
                       // command
            ...
            case 118:  // M118 is a special non compliant Gcode as it allows arbitrary text on the line following the
                       // command
              ...
              if (gcode->subcode == 1) {
                if (gcode->has_letter('P')) {
                  ...
                  THEKERNEL->streams->printf(" %s = %.3f \r\n", str.c_str(),
                                             final_value);  // the space at the start is important for variable displays
                  delete gcode;
                  return;
                }
              }
            ...
            case 1000:  // M1000 is a special command that will pass thru the raw lowercased command to the simpleshell
                        // (for hosts that do not allow such things)
            {
        ...
        // new_message.stream->printf("dispatch gcode command: '%s' G%d M%d...", gcode->get_command(), gcode->g,
        // gcode->m);
        // Dispatch message!
        THEKERNEL->call_event(ON_GCODE_RECEIVED, gcode);
        ...
        if (gcode->is_error) {
          ...
          if (gcode->add_nl) new_message.stream->printf("\r\n");
          ...
          if (!gcode->txt_after_ok.empty()) {
            ...
            if (THEKERNEL->is_ok_per_line() || THEKERNEL->is_grbl_mode()) {
              ...
              if (possible_command.empty()) new_message.stream->printf("ok\r\n");
            } else {
    ...
    // an uppercase non command word on its own (except XYZAF) just returns ok, we could add an error but no hosts
    // expect that.
