...
SerialConsole::~SerialConsole() { delete this->serial; }
...
void SerialConsole::on_serial_char_received() {
  while (this->serial->readable()) {
    ...
    if (THEKERNEL->is_feed_hold_enabled()) {
      bool at_line_start =
          (this->buffer.head == this->buffer.tail) || (this->previous_char == '\n') || (this->previous_char == '\r');
      if (at_line_start) {
...
bool SerialConsole::makera_cmd_queue_empty() const { return makera_cmd_head == makera_cmd_tail; }
...
void SerialConsole::process_makera_byte(uint8_t received) {
  ...
  uint16_t footer = (serial_protocol_buffer[makera_received - 2] << 8) | serial_protocol_buffer[makera_received - 1];
  uint16_t received_crc =
      (serial_protocol_buffer[makera_received - 4] << 8) | serial_protocol_buffer[makera_received - 3];
...
void SerialConsole::reset() { reset_file_parser(); }
...
int SerialConsole::_putc(int c) { return this->serial->putc(c); }

int SerialConsole::_getc() { return this->serial->getc(); }

bool SerialConsole::ready() { return this->serial->readable(); }
