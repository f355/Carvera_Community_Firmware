...
// This is a gcode object. It represents a GCode string/command, and caches some important values about that command for
// the sake of performance. It gets passed around in events, and attached to the queue ( that'll change )
...
Gcode::Gcode(const Gcode& to_copy) {
  this->command = strdup(to_copy.command);  // TODO we can reference count this so we share copies, may save more ram
                                            // than the extra count we need to store
    ...
    } else  // system variables
    {
      ...
      switch (var_num) {
        ...
        case 5021:  // current machine X position
          ...
          // current_position/mpos includes the compensation transform so we need to get the inverse to get actual
          // position
