...
class RecordSink {
 public:
  ...
  virtual uint32_t accept(uint32_t index, const uint8_t* data, std::size_t size) = 0;
  virtual bool commit() { return true; }
  virtual void rollback() = 0;
};

Result receive_records(Transport& transport, RecordSink& sink, const PacketTypes& types,
