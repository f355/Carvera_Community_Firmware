...
void SerialConsole2::on_module_loaded() {
  ...
  this->serial->baud(
      THEKERNEL->config->value(uart_checksum, baud_rate_setting_checksum)->as_number(DEFAULT_SERIAL_BAUD_RATE));
...
int SerialConsole2::_putc(int c) { return this->serial->putc(c); }

int SerialConsole2::_getc() { return this->serial->getc(); }
