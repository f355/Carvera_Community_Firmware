...
const SimpleShell::ptentry_t SimpleShell::commands_table[] = {{"ls", SimpleShell::ls_command},
...
static uint32_t heapWalk(StreamOutput* stream, bool verbose) {
  ...
  while (chunkCurr < heapEnd) {
    ...
    chunkSize -= 8;
    if (verbose)
      stream->printf("  Chunk: %lu  Address: 0x%08lX  Size: %lu  %s\n", static_cast<unsigned long>(chunkNumber),
                     static_cast<unsigned long>(chunkCurr), static_cast<unsigned long>(chunkSize),
                     isChunkFree ? "CHUNK FREE" : "");
...
void SimpleShell::on_gcode_received(void* argument) {
  ...
  if (gcode->has_m) {
    if (gcode->m == 20) {  // list sd card
      ...
      if (!args.empty() && !THEKERNEL->is_grbl_mode()) rm_command("/sd/" + args, gcode->stream);
      ...
      gcode->stream->printf(
          "turning line by line execute mode on.\r\nPlaying file will pause after every valid gcode line, skipping "
  ...
  if (possible_command[0] == '$' && possible_command.size() >= 2) {
    ...
    if (cmd == "config-get") {
      ...
      config_default_command(possible_command, new_message.stream);
               ...
               cmd == "buffer" || cmd == "upload" || cmd == "download" || cmd == "goto") {
...
void SimpleShell::mem_command(string parameters, StreamOutput* stream) {
  ...
  unsigned long heap_unallocated_top = (STACK_SIZE && g_maximumHeapAddress != 0)
                                           ? g_maximumHeapAddress - heap_top
                                           : 0;  // Calculate unallocated space at the top if stack limit is set
  stream->printf("Main Heap Unallocated Top: %lu bytes\r\n", heap_unallocated_top);

  uint32_t heap_fragmented_free =
      heapWalk(stream, verbose);  // Calculates and prints used/free within allocated heap part
  ...
  stream->printf("Block size: %u bytes, Tickinfo size: %u bytes\n", sizeof(Block),
                 sizeof(Block::tickinfo_t) * Block::n_actuators);
}
...
void SimpleShell::wlan_command(string parameters, StreamOutput* stream) {
  ...
  void* returned_data;
  if (ssid.empty()) {
    if (!send_eof) stream->printf("Scanning wifi signals...\n");
...
void SimpleShell::ftype_command(string parameters, StreamOutput* stream) { stream->printf("ftype = %s\n", FILETYPE); }
...
void SimpleShell::model_command(string parameters, StreamOutput* stream) {
  switch (THEKERNEL->factory_set->MachineModel) {
    case CARVERA:
      stream->printf("model = %s, %d, %d, %d\n", "C1", THEKERNEL->factory_set->MachineModel,
                     THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr);
      break;
    case CARVERA_AIR:
      stream->printf("model = %s, %d, %d, %d\n", "CA1", THEKERNEL->factory_set->MachineModel,
                     THEKERNEL->factory_set->FuncSetting, THEKERNEL->probe_addr);
...
void SimpleShell::grblDP_command(string parameters, StreamOutput* stream) {
  ...
  */
  ...
  stream->printf("[G92:%1.4f,%1.4f,%1.4f,%1.4f,%1.4f]\n", THEROBOT->from_millimeters(std::get<0>(v[n + 1])),
                 THEROBOT->from_millimeters(std::get<1>(v[n + 1])), THEROBOT->from_millimeters(std::get<2>(v[n + 1])),
                 // THEROBOT->from_millimeters(std::get<3>(v[n+1])),
                 // THEROBOT->from_millimeters(std::get<4>(v[n+1])));
                 std::get<3>(v[n + 1]), std::get<4>(v[n + 1]));
  ...
  // this is the last probe position, updated when a probe completes, also stores the number of steps moved after a
  // homing cycle
  float px, py, pz;
  uint8_t ps;
  std::tie(px, py, pz, ps) = THEROBOT->get_last_probe_position();
  stream->printf("[PRB:%1.4f,%1.4f,%1.4f:%d]\n", THEROBOT->from_millimeters(px), THEROBOT->from_millimeters(py),
                 THEROBOT->from_millimeters(pz), ps);
}
...
void SimpleShell::get_command(string parameters, StreamOutput* stream) {
  ...
  if (what == "temp") {
    ...
    if (type.empty()) {
      ...
      if (ok) {
        for (auto& c : controllers) {
          stream->printf("%s (%d) temp: %f/%f @%d\r\n", c.designator.c_str(), c.id, c.current_temperature,
                         c.target_temperature, c.pwm);
        }
      ...
      bool ok =
          PublicData::get_value(temperature_control_checksum, current_temperature_checksum, get_checksum(type), &temp);

      if (ok) {
        stream->printf("%s temp: %f/%f @%d\r\n", type.c_str(), temp.current_temperature, temp.target_temperature,
                       temp.pwm);
      } else {
    ...
    if (move) {
      ...
      char cmd[64];
      snprintf(cmd, sizeof(cmd), "G53 G0 X%f Y%f Z%f", THEROBOT->from_millimeters(x), THEROBOT->from_millimeters(y),
               THEROBOT->from_millimeters(z));
    ...
    std::string buf;
    THEROBOT->print_position(0, buf);
    stream->printf("last %s\n", buf.c_str());
    buf.clear();
    THEROBOT->print_position(1, buf);
    stream->printf("realtime %s\n", buf.c_str());
    buf.clear();
    THEROBOT->print_position(2, buf);
    stream->printf("%s\n", buf.c_str());
    buf.clear();
    THEROBOT->print_position(3, buf);
    stream->printf("%s\n", buf.c_str());
    buf.clear();
    THEROBOT->print_position(4, buf);
    stream->printf("%s\n", buf.c_str());
    buf.clear();
    THEROBOT->print_position(5, buf);
    stream->printf("%s\n", buf.c_str());
    buf.clear();
    ...
    stream->printf(
        "[G%d %s G%d G%d G%d G%d M0 M%c M%c T%d F%1.4f S%1.4f]\n", THEKERNEL->gcode_dispatch->get_modal_command(),
        ...
        : THEROBOT->plane_axis_0 == Y_AXIS && THEROBOT->plane_axis_1 == Z_AXIS && THEROBOT->plane_axis_2 == X_AXIS ? 19
                                                                                                                   : 17,
        THEROBOT->inch_mode ? 20 : 21, THEROBOT->absolute_mode ? 90 : 91, THEROBOT->inverse_time_mode ? 93 : 94,
        get_switch_state("spindle") ? '3' : '5',
        get_switch_state("mist")    ? '7'
        : get_switch_state("flood") ? '8'
                                    : '9',
        get_active_tool(), THEROBOT->from_millimeters(THEROBOT->get_feed_rate()), THEROBOT->get_s_value());
    ...
    if (THEROBOT->compensationTransform)
      THEROBOT->compensationTransform(mpos, true, true);  // get inverse compensation transform
    stream->printf("Curr: %1.3f,%1.3f,%1.3f, Comp: %1.3f,%1.3f,%1.3f\n", old_mpos[0], old_mpos[1], old_mpos[2], mpos[0],
                   mpos[1], mpos[2]);
...
void SimpleShell::calc_thermistor_command(string parameters, StreamOutput* stream) {
  ...
  if (trl.size() == 6) {
    ...
    float c1, c2, c3;
    std::tie(c1, c2, c3) =
        Thermistor::calculate_steinhart_hart_coefficients(trl[0], trl[1], trl[2], trl[3], trl[4], trl[5]);
...
static void verify_hashed_data_files(char* data_path, char* md5_path, unsigned int& current, unsigned int total,
                                     unsigned int& intact_count, unsigned int& corrupt_count, StreamOutput* stream,
                                     bool& aborted) {
  ...
  for (size_t i = 0; i < subdirs.size(); i++) {
    ...
    data_path[baselen] = '/';
    strcpy(data_path + baselen + 1, subdirs[i].c_str());
    verify_hashed_data_files(data_path, md5_path, current, total, intact_count, corrupt_count, stream, aborted);
    data_path[baselen] = '\0';
    THEKERNEL->call_event(ON_IDLE);
  }
}
...
void SimpleShell::jog(string parameters, StreamOutput* stream) {
  ...
  if (parameters.empty()) {
    stream->printf(
        "usage: $J [-c] X0.01 [S0.5|Fnnn] - axis can be XYZABC, optional speed is scale of max_rate or feedrate. -c "
        "turns on continuous jog mode\n");
    return;
  }
  ...
  if (cont_mode) {
    ...
    if (THEROBOT->is_soft_endstop_enabled()) {
      ...
      for (int i = 0; i <= Z_AXIS; ++i) {
        ...
        dist_to_max[i] = (THEROBOT->get_soft_endstop_max(i) - current_pos[i]);
        // If moving towards a limit and brake distance is greater than distance to limit
        if (delta[i] < 0 && !isnan(THEROBOT->get_soft_endstop_min(i)) && dist_to_min[i] <= fabs(delta[i])) {
        ...
        if (delta[i] > 0 && !isnan(THEROBOT->get_soft_endstop_max(i)) && dist_to_max[i] <= fabs(delta[i])) {
...
void SimpleShell::spindle_override(string parameters, StreamOutput* stream)  // M223 SXXX equivalent
{
  ...
  while (!parameters.empty()) {
    ...
    if (ax == 'S') {
      ...
      if (factor > 300.0F) factor = 300.0F;
