...
unsigned char
    xbuff[XBUFF_LENGTH] LOCATED_IN_AHBSRAM; /* 2 for data length, 8192 for XModem + 3 head chars + 2 crc + nul */
...
void Player::sync_progress_max() {
  ...
  if (pos > 0 && (unsigned long)pos > this->played_cnt) this->played_cnt = (unsigned long)pos;
  if (this->file_line > this->played_lines) this->played_lines = this->file_line;
}
...
void Player::on_module_loaded() {
  ...
  this->spindle_suspend_restore_enable =
      THEKERNEL->config->value(spindle_suspend_restore_enable_checksum)->as_bool(true);
...
void Player::on_gcode_received(void* argument) {
  ...
  if (gcode->has_m) {
    ...
    if (gcode->m == 1) {  // optiional stop
      ...
      //} else if (gcode->m == 30) { // end file implementation for M30 returning from macros. M99 is the proper
      // formatting.
      //    this->end_of_file();
...
void Player::progress_command(string parameters, StreamOutput* stream) {
  ...
  if (file_size > 0) {
    ...
    if (this->elapsed_secs > 10) {
      ...
      if (bytespersec > 0) est = (file_size - played_cnt) / bytespersec;
    }
    ...
    if (!sdprinting) {
      stream->printf("file: %s, %u %% complete, elapsed time: %02lu:%02lu:%02lu", this->filename.c_str(),
                     (unsigned int)roundf(pcnt), this->elapsed_secs / 3600, (this->elapsed_secs % 3600) / 60,
...
void Player::abort_command(string parameters, StreamOutput* stream) {
  ...
  if (communication_protocol == PROTOCOL_SMOOTHIE) {
    ...
    if (parameters.empty()) {
      ...
      // MUST be called in on_main_loop to make sure there are no blocked main loops waiting to put something on the
      // queue
      THEKERNEL->conveyor->flush_queue();

      // now the position will think it is at the last received pos, so we need to do FK to get the actuator position
...
void Player::on_main_loop(void* argument) {
  if (!this->booted) {
    ...
    if (this->on_boot_gcode_enable) {
      StreamOutput* stream =
          THEKERNEL->serial != nullptr ? static_cast<StreamOutput*>(THEKERNEL->serial) : &StreamOutput::NullStream;
      ...
      if (buf[len - 1] == '\n' || fwfs::feof(this->current_file_handler)) {
        ...
        if (this->laser_clustering && THEKERNEL->get_laser_mode() && !THEROBOT->absolute_mode && played_lines > 100) {
            ...
            sum_value = sum_distance + distance;
                ...
                played_lines += 1;
                played_cnt += len;
                                        if (cluster_index >= 8 || (min_distance > 0 && sum_distance * 1.0 / min_distance
        > 7.9)) { sprintf(md5_str, "G1 X%.3f Y%.3f S", sum_x_value, sum_y_value); clustered_gcode = md5_str; for (int i
        = 0; i < cluster_index; i ++) { for (float j = min_distance; j < clustered_distance[i] + 0.01; j+= min_distance)
        { sprintf(md5_str, "%s%.2f", (i == 0 ? "" : ":"), clustered_s_value[i]); clustered_gcode.append(md5_str);
                            }
                        }
        ...
        &(StreamOutput::NullStream) : this->current_stream; message.line = played_lines + 1;
        ...
        clustered_gcode.c_str()); return;
                        ...
                        clustered_gcode.append("\n");
                                        ...
                                        struct SerialMessage message;
                                        message.message = clustered_gcode;
                                        message.stream = this->current_stream == nullptr ? &(StreamOutput::NullStream) :
        this->current_stream; message.line = played_lines + 1;
...
/*
bool Player::check_cluster(const char *gcode_str, float *x_value, float *y_value, float *distance, float *slope, float
*s_value)
{
        float new_slope = 0.0;
        bool is_cluster = false;
  ...
  if (pdr->second_element_is(is_playing_checksum) || pdr->second_element_is(is_suspended_checksum)) {
    ...
    if (file_size > 0 && playing_file) {
      if (!this->inner_playing) {
        ...
        // Note to avoid a race condition where the block is being cleared we check the is_ready flag which gets cleared
        // first, as this is an interrupt if that flag is not clear then it cannot be cleared while this is running and
        // the block will still be valid (albeit it may have finished) Only advance played lines, never go backward
...
void Player::resume_command(string parameters, StreamOutput* stream) {
  ...
  if (this->goto_line == 0) {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "G1 X%.3f Y%.3f Z%.3f F%.3f", saved_position[0], saved_position[1], saved_position[2],
             THEROBOT->from_millimeters(1000));
    struct SerialMessage message;
    message.message = buf;
    message.stream = &(StreamOutput::NullStream);
    message.line = 0;
...
int Player::inbyte(StreamOutput* stream, unsigned int timeout_ms) {
  ...
  while (us_ticker_read() - tick_us < timeout_ms * 1000) {
    if (stream->ready()) return stream->_getc();
    safe_delay_us(100);
  }
  return -1;
}
...
int Player::inbytes(StreamOutput* stream, char** buf, int size, unsigned int timeout_ms) {
  uint32_t tick_us = us_ticker_read();
  while (us_ticker_read() - tick_us < timeout_ms * 1000) {
    if (stream->ready()) return stream->gets(buf, size);
    safe_delay_us(100);
  }
  return -1;
}
...
void Player::flush_input(StreamOutput* stream) {
  while (inbyte(stream, TIMEOUT_MS) >= 0) continue;
}
...
int Player::decompress(string sfilename, string dfilename, uint32_t sfilesize, StreamOutput* stream) {
  ...
  for (i = 0; i < sfilesize - 2; i += BLOCK_HEADER_SIZE + u32BlockSize) {
    ...
    u32BlockSize = u8ReadBuffer_hdr[0] * (1 << 24) + u8ReadBuffer_hdr[1] * (1 << 16) + u8ReadBuffer_hdr[2] * (1 << 8) +
                   u8ReadBuffer_hdr[3];
    if (!u32BlockSize) {
      if (communication_protocol == PROTOCOL_SMOOTHIE) {
  ...
  if (f_in != NULL) fwfs::fclose(f_in);
  if (f_out != NULL) fwfs::fclose(f_out);
  if (communication_protocol == PROTOCOL_SMOOTHIE) {
    memset(fbuff, 0, sizeof(fbuff));
    sprintf((char*)fbuff, "#Info: decompart = %lu\r\n", u32BlockNum);
    stream->printf((char*)fbuff);
  ...
  for (;;) {
    if (communication_protocol == PROTOCOL_SMOOTHIE) {
      for (retry = 0; retry < MAXRETRANS; ++retry) {  // approx 3 seconds allowed to make connection
        if (trychar) stream->_putc(trychar);
    ...
    start_recv:
      ...
      *p++ = c;
      ...
      if (!md5_received && xbuff[1] == 0 && xbuff[1] == (unsigned char)(~xbuff[2]) &&
        ...
        md5_received = true;
        continue;
      } else if (xbuff[1] == (unsigned char)(~xbuff[2]) && xbuff[1] == packetno &&
                 check_crc(crc, &xbuff[3], bufsz + 1 + is_stx)) {
        // Set the file write system buffer 4096 Byte
      ...
      if (cmd > 0) {
        ...
        switch (cmd) {
          ...
          case PTYPE_FILE_DATA:
            if (!beretry) filesendseq = recv_buff[3] << 24 | recv_buff[4] << 16 | recv_buff[5] << 8 | recv_buff[6];
            xbuff[0] = (HEADER >> 8) & 0xFF;
            xbuff[1] = HEADER & 0xFF;
            xbuff[4] = PTYPE_FILE_DATA;
