...
void StreamedPlayerSource::begin(uint16_t file_id, unsigned long file_size,
                                 uint32_t first_line, unsigned long byte_position)
{
    close();
    file_id_ = file_id;
...
StreamedPlayerSource::AcceptResult StreamedPlayerSource::accept(
    uint16_t file_id, uint32_t first_line, const uint8_t *data, std::size_t size)
{
...
PlayerLineSource::ReadResult StreamedPlayerSource::read(char *buffer,
                                                        std::size_t size)
{
...
bool StreamedPlayerSource::at_end() const
{
    return open_ && eof_ && count_ == 0;
}
