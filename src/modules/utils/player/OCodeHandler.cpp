...
bool OCodeHandler::is_skipping() const { return any_frame_skipping(); }

int OCodeHandler::find_loop_frame(int num) const {
  for (int i = (int)stack_.size() - 1; i >= 0; i--) {
    BlockType t = stack_[i].type;
    if ((t == BlockType::WHILE || t == BlockType::DO || t == BlockType::REPEAT) && stack_[i].num == num) return i;
  }
  return -1;
}
...
bool OCodeHandler::skip_to(FILE* fh, const string& open_kw, const string& close_kw, string& matched, int& lines_read,
                           int target_num, string* matched_rest) const {
  int depth = 0;
  lines_read = 0;
  ...
  while (fwfs::fgets(buf, sizeof(buf), fh) != NULL) {
    ...
    if (track_do && kw == "do") {
      if (do_count < OCODE_MAX_STACK_DEPTH) do_nums[do_count++] = n;
      continue;
    }
...
bool OCodeHandler::process_line(const char* line, FILE* fh, StreamOutput* stream, unsigned long& file_line) {
  ...
  if (keyword == "endsub" || keyword == "return") {
    ...
    for (int i = (int)stack_.size() - 1; i >= 0; i--) {
      if (stack_[i].type == BlockType::SUB) {
        for (int p = 0; p < 30; p++) THEKERNEL->local_params[p] = stack_[i].saved_params[p];
  ...
  if (keyword == "call") {
    ...
    for (int p = 0; p < 30; p++) frame.saved_params[p] = THEKERNEL->local_params[p];
