...
void Configurator::config_set_command(string parameters, StreamOutput* stream) {
  ...
  if (source.empty() || setting.empty() || value.empty()) {
    stream->printf(
        "Usage: config-set source setting value # where source is sd, setting is the key and value is the new "
        "value\r\n");
    return;
  }
    ...
    if (THEKERNEL->config->config_sources[i]->is_named(source_checksum)) {
      if (THEKERNEL->config->config_sources[i]->write(setting, value)) {
        if (setting == "zprobe.probe_tip_diameter") {
          ...
          if (fvalue) {
            THEKERNEL->probe_tip_diameter = fvalue;
          }  // will fail if the tip diameter is set to 0, but it should not be anyway
        }
...
void Configurator::config_delete_command(string parameters, StreamOutput* stream) {
  ...
  if (source.empty() || setting.empty()) {
    stream->printf(
        "Usage: config-delete source setting # where source is sd (or local), setting is the key to remove\r\n");
    return;
  }
