...
void M8266HostIf_SPI_Init(void) {
  ...
  LPC_SC->LPC_SC_PCLKSELx_FOR_SSP = ((LPC_SC->LPC_SC_PCLKSELx_FOR_SSP) & (~(3UL << LPC_SC_PCLKSELx_FOR_SSP_MASK_BIT))) |
                                    (1UL << LPC_SC_PCLKSELx_FOR_SSP_MASK_BIT);  // to Select the PCLK_SSPx = CPU CLK
  // Initialise the SPI Registers
  M8266WIFI_INTERFACE_SPI->CR0 =
      0 | (0x07 << 0)  // bits(3~0)=b0111 = 8-bit transfer
  ...
  M8266WIFI_INTERFACE_SPI->CR1 = 0 | (0x00 << 0)  // bit 0 = b0 = LBM-> Normal operation other than Loopback mode
  ...
  M8266WIFI_INTERFACE_SPI->DMACR = 0 | (1 << 0)  // Receive DMA Enable
 ...
 *    None                                                                         *
 ***********************************************************************************/
void M8266HostIf_SPI_SetSpeed(
    u32 SPI_BaudRatePrescaler)  // SPI_BaudRatePrescaler should be an even number no less than 2.
{
...
void M8266HostIf_SPI_DMA_Init(void) {
  ...
  M8266WIFI_INTERFACE_SPI_RX_DMA_STREAM->DMACCConfig =
      0 | (0 << 0)                               // bit0 = Enable = 0 -> Channel not enable initially
      ...
      ;
  M8266WIFI_INTERFACE_SPI_RX_DMA_STREAM->DMACCControl =
      0 | (0 << 0)           // bits[11:0] = transfer size, use 0 for the moment during initialization
      ...
      ;
  M8266WIFI_INTERFACE_SPI_RX_DMA_STREAM->DMACCSrcAddr =
      (uint32_t)(&(M8266WIFI_INTERFACE_SPI->DR));  // DMA source address,
  M8266WIFI_INTERFACE_SPI_RX_DMA_STREAM->DMACCDestAddr =
      (uint32_t)0;  // DMA destin address, use 0 during initialization for the moment

  // Set DMA Channel for SPI TX
  M8266WIFI_INTERFACE_SPI_TX_DMA_STREAM->DMACCLLI = 0;

  M8266WIFI_INTERFACE_SPI_TX_DMA_STREAM->DMACCConfig =
      0 | (0 << 0)                               // bit0 = Enable = 0 -> Channel not enable initially
  ...
  M8266WIFI_INTERFACE_SPI_TX_DMA_STREAM->DMACCControl =
      0 | (0 << 0)           // bits[11:0] = transfer size, use 0 for the moment during initialization
      ...
      | ((uint32_t)1 << 31)  // bit31 = Terminal count interrupt enable bit = 1 -> enabled1<<31)             // bit31 =
                             // Terminal count interrupt enable bit = 0 -> interrupt disabled
      ;
  M8266WIFI_INTERFACE_SPI_TX_DMA_STREAM->DMACCSrcAddr =
      (uint32_t)0;  // DMA source address, use 0 during initialization for the moment
  M8266WIFI_INTERFACE_SPI_TX_DMA_STREAM->DMACCDestAddr =
      (uint32_t)(&(M8266WIFI_INTERFACE_SPI->DR));  // DMA destin address, use 0 during initialization for the moment
}
...
void M8266HostIf_delay_us(u8 nus) {
  ...
  SysTick->LOAD = SystemCoreClock / 1000000 *
                  nus;  //  The System Tick will use SystemFrequency. see below SysTick_CTRL_CLKSOURCE_Msk
...
u8 M8266HostIf_SPI_ReadWriteByte(u8 TxdByte) {
  ...
  while ((M8266WIFI_INTERFACE_SPI->SR & (1 << 0)) == 0) {
  }  // bit0=TFE(Transmit FIFO empty)

  // to Txd the data onto MOSI
  M8266WIFI_INTERFACE_SPI->DR = TxdByte;

  // wait the data received from MISO
  while ((M8266WIFI_INTERFACE_SPI->SR & (1 << 2)) == 0) {
  }  // bit2=RNE(Receive FIFO not empty)
