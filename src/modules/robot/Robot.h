...
class Robot : public Module {
 public:
  ...
  void get_axis_position(float position[], size_t n = 3) const {
    memcpy(position, this->machine_position, n * sizeof(float));
  }
  wcs_t get_axis_position() const {
    return wcs_t(machine_position[X_AXIS], machine_position[Y_AXIS], machine_position[Z_AXIS], machine_position[A_AXIS],
                 machine_position[B_AXIS]);
  }
  ...
  wcs_t mcs2wcs(const float* pos) const {
    return mcs2wcs(wcs_t(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], pos[A_AXIS], pos[B_AXIS]));
  }
  wcs_t mcs2selected_wcs(const float* pos, const size_t n) const {
    return mcs2selected_wcs(wcs_t(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], pos[A_AXIS], pos[B_AXIS]), n);
  }
  wcs_t wcs2mcs(const wcs_t& pos) const;
  wcs_t wcs2mcs(const float* pos) const {
    return wcs2mcs(wcs_t(pos[X_AXIS], pos[Y_AXIS], pos[Z_AXIS], pos[A_AXIS], pos[B_AXIS]));
  }
 ...
 private:
  ...
  bool append_arc(Gcode* gcode, const float target[], const float rotated_target[], const float offset[], float radius,
                  bool is_clockwise);
  bool compute_arc(Gcode* gcode, const float offset[], const float target[], const float rotated_target[],
  ...
  using saved_state_t = std::tuple<float, float, bool, bool, bool, bool, bool,
                                   uint8_t>;  // save current feedrate and absolute mode, e absolute mode, inch mode,
                                              // is_g123, inverse_time_mode, current_wcs
  std::stack<saved_state_t> state_stack;      // saves state from M120

  float machine_position[k_max_actuators];  // Last requested position, in millimeters, which is what we were requested
                                            // to move to in the gcode after offsets applied but before compensation
                                            // transform
  float
      compensated_machine_position[k_max_actuators];  // Last machine position, which is the position before converting
                                                      // to actuator coordinates (includes compensation transform)
