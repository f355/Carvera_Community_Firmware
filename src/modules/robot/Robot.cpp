...
void Robot::on_gcode_received(void* argument) {
  ...
  if (compensation_preprocessor->is_active() && gcode->has_g && (gcode->g == 2 || gcode->g == 3)) {
    if (gcode->has_letter('R')) {
      THEKERNEL->streams->printf(
          "ERROR: R-format arc (G2/G3 R...) is not supported with G41/G42 compensation. Use I/J offsets instead. "
  ...
  if (gcode->has_g && gcode->g == 91) {
    THEKERNEL->streams->printf(
        "ERROR: G91 (relative mode) is not supported while G41/G42 compensation is active. Issue G40 first.\n");
    THEKERNEL->set_halt_reason(MANUAL);
    THEKERNEL->call_event(ON_HALT, nullptr);
    return;
  }
  ...
  if (gcode->has_g && gcode->g >= 54 && gcode->g <= 59) {
    THEKERNEL->streams->printf(
        "ERROR: G5x work coordinate changes are not supported while G41/G42 compensation is active. Issue G40 "
        "first.\n");
    THEKERNEL->set_halt_reason(MANUAL);
    THEKERNEL->call_event(ON_HALT, nullptr);
    return;
    ...
    switch (gcode->g) {
      ...
      case 17:
        ...
        if (this->comp_suspended) {
          ...
          float wcs_position[3] = {std::get<X_AXIS>(wcs_pos) - this->comp_frozen_offset[X_AXIS],
      ...
      case 19:
        if (compensation_preprocessor->is_active()) {
          ...
          THEKERNEL->streams->printf(
              "INFO: G41/G42 compensation frozen for G%d plane "
      ...
      case 42:  // G42 - Compensation Right
      {
        ...
        if (plane_axis_0 != X_AXIS || plane_axis_1 != Y_AXIS || plane_axis_2 != Z_AXIS) {
          THEKERNEL->streams->printf(
              "ERROR: G41/G42 compensation requires G17 (XY plane). G18/G19 compensation is not yet supported.\n");
          THEKERNEL->set_halt_reason(MANUAL);
          THEKERNEL->call_event(ON_HALT, nullptr);
          break;
        }
        ...
        if (!compensation_preprocessor->resolve_diameter(
                ...
                THEKERNEL->eeprom_data->TOOL_DIA, THEKERNEL->eeprom_data->TOOL_DIA_WEAR, gcode->stream, &radius)) {
          THEKERNEL->set_halt_reason(MANUAL);
          THEKERNEL->call_event(ON_HALT, nullptr);
          break;
        }
        ...
        wcs_t wcs_pos = mcs2wcs(machine_position);
        float wcs_position[3] = {std::get<X_AXIS>(wcs_pos), std::get<Y_AXIS>(wcs_pos), std::get<Z_AXIS>(wcs_pos)};
        ...
        COMPENSATION_TRACE_PRINTF(gcode->stream, ">>G%d: Initial WCS pos X=%.3f Y=%.3f Z=%.3f\n", gcode->g,
                                  wcs_position[X_AXIS], wcs_position[Y_AXIS], wcs_position[Z_AXIS]);
      } break;
...
bool Robot::is_compensation_active() const { return compensation_preprocessor->is_active(); }
