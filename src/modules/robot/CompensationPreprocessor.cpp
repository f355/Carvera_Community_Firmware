/*
      ...
      Carvera Community Firmware is free software: you can redistribute it and/or modify it under the terms of the GNU
   General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your
   option) any later version. Carvera Community Firmware is distributed in the hope that it will be useful, but WITHOUT
   ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
   General Public License for more details. You should have received a copy of the GNU General Public License along with
   Carvera Community Firmware. If not, see <http://www.gnu.org/licenses/>.
*/
...
bool CompensationPreprocessor::resolve_diameter(bool has_d_word, float d_word_value, float eeprom_tool_dia,
                                                float eeprom_tool_dia_wear, StreamOutput* stream, float* out_radius) {
...
bool CompensationPreprocessor::get_motion_direction(const float start[3], const UncompPoint& end, bool use_segment_end,
                                                    float dir[2]) const {
...
void CompensationPreprocessor::format_compensated_gcode(const float uncomp_start[3], const float comp_start[3],
                                                        const UncompPoint& curr, const float comp_end[3],
                                                        char* gcode_str, size_t gcode_str_size) const {
  ...
  if (is_arc_motion(curr.motion_g)) {
    ...
    const float epsilon = 0.0001f;
    float uncomp_radius = sqrtf((center_x - uncomp_start[X_AXIS]) * (center_x - uncomp_start[X_AXIS]) +
    ...
    float eff_radius = outward ? (uncomp_radius + comp_radius) : (uncomp_radius - comp_radius);
    ...
    snprintf(gcode_str, gcode_str_size, "G%d X%.3f Y%.3f Z%.3f I%.3f J%.3f%s", curr.motion_g, arc_end_x, arc_end_y,
             comp_end[Z_AXIS], new_i, new_j, feedrate_suffix);
    return;
  }

  snprintf(gcode_str, gcode_str_size, "G%d X%.3f Y%.3f Z%.3f%s", curr.motion_g <= 1 ? curr.motion_g : 1,
           comp_end[X_AXIS], comp_end[Y_AXIS], comp_end[Z_AXIS], feedrate_suffix);
}
...
void CompensationPreprocessor::compute_and_output() {
  ...
  COMPENSATION_TRACE_PRINTF(THEKERNEL->streams,
                            ">>COMPUTE: EXIT - complete, comp_count=%d, comp_ready=%d, uncomp_tail=%d, comp_head=%d\n",
...
bool CompensationPreprocessor::calculate_perpendicular_offset(const UncompPoint& prev, const UncompPoint& curr,
                                                              float output[2]) {
...
bool CompensationPreprocessor::calculate_corner_intersection(const UncompPoint& a, const UncompPoint& b,
                                                             const UncompPoint& c, float output[2]) {
  const float epsilon = 0.0001f;
...
Gcode* CompensationPreprocessor::get_compensated_gcode() { return serve_compensated_gcode(true); }
...
void CompensationPreprocessor::print_load_balance_report(StreamOutput* stream) const {
  ...
  float avg_margin = (m.sample_count > 0) ? (float)m.cumulative_ready_margin / (float)m.sample_count : 0.0f;
  float pull_hit_rate = (pull_requests > 0) ? ((float)m.served_gcode_count * 100.0f) / (float)pull_requests : 100.0f;
  float pull_miss_rate = (pull_requests > 0) ? ((float)m.empty_serve_count * 100.0f) / (float)pull_requests : 0.0f;
  float prod_vs_pull = (pull_requests > 0) ? ((float)m.generated_gcode_count * 100.0f) / (float)pull_requests : 100.0f;
  ...
  stream->printf(
      ...
      (unsigned long)m.input_gcode_count, (unsigned long)m.arc_input_count, (unsigned long)m.generated_gcode_count,
      (unsigned long)m.served_gcode_count, (unsigned long)m.compute_count, (unsigned long)m.priming_wait_count,
      (unsigned long)m.load_balance_wait_count, (unsigned long)m.empty_serve_count, (unsigned long)pull_requests,
      pull_hit_rate, pull_miss_rate, prod_vs_pull, (unsigned long)m.sample_count, avg_margin, (long)m.min_ready_margin,
      (long)m.max_ready_margin, (unsigned)m.max_uncomp_depth, (unsigned)m.max_comp_ready_depth);
#else
  (void)stream;
#endif
}
...
Gcode* CompensationPreprocessor::serve_compensated_gcode(bool draining) {
  COMPENSATION_TRACE_PRINTF(THEKERNEL->streams,
                            ">>SERVE_GCODE: ENTRY - draining=%d comp_count=%d comp_ready=%d comp_tail=%d\n",
  ...
  COMPENSATION_TRACE_PRINTF(THEKERNEL->streams,
                            ">>SERVE_GCODE: EXIT - returning %p, counts now uncomp=%d comp=%d ready=%d\n",
...
void CompensationPreprocessor::print_offset_calc(const UncompPoint& prev, const UncompPoint& curr, const float dir[2],
                                                 const float normal[2], const float output[2]) {
...
void CompensationPreprocessor::print_buffer_state() {
  ...
  COMPENSATION_TRACE_PRINTF(
      THEKERNEL->streams,
      ">>BUFFER_STATE: uncomp_count=%d, uncomp_head=%d, uncomp_tail=%d, comp_count=%d, comp=%s, radius=%.3f\n",
