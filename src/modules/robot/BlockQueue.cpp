...
BlockQueue::BlockQueue(unsigned int length) {
  ...
  if (v == nullptr) {
    // TODO: Optionally add error reporting here (e.g., THEKERNEL->streams->printf("FATAL: BlockQueue alloc
...
BlockQueue::~BlockQueue() {
  head_i = tail_i = length = 0;
  isr_tail_i = tail_i;
  if (ring != nullptr) AHB.dealloc(ring);  // delete [] ring;
  ring = nullptr;
}
...
Block& BlockQueue::head() { return ring[head_i]; }

Block& BlockQueue::tail() { return ring[tail_i]; }

Block& BlockQueue::item(unsigned int i) { return ring[i]; }
...
Block* BlockQueue::head_ref() { return &ring[head_i]; }

Block* BlockQueue::tail_ref() { return &ring[tail_i]; }

Block* BlockQueue::item_ref(unsigned int i) { return &ring[i]; }
...
void BlockQueue::consume_tail() {
  if (!is_empty()) tail_i = next(tail_i);
}
  ...
  if (is_empty()) {
    ...
    if (newring != nullptr) {
      ...
      if (is_empty())  // check again in case something was pushed while malloc did its thing
      {
        ...
        if (oldring != nullptr) AHB.dealloc(oldring);  // delete [] oldring;
