...
void Block::init(uint8_t n) {
  n_actuators = n;
  fp_scale = (double)STEPTICKER_FPSCALE /
             pow((double)STEP_TICKER_FREQUENCY, 2.0);  // we scale up by fixed point offset first to avoid tiny values
}
...
void Block::debug() const {
  ...
  THEKERNEL->streams->printf(
      "(max:%lu) nominal:r%1.4f/s%1.4f mm:%1.4f acc:%1.2f accu:%lu decu:%lu ticks:%lu rates:%1.4f/%1.4f "
      "entry/max:%1.4f/%1.4f exit:%1.4f primary:%d ready:%d locked:%d ticking:%d recalc:%d nomlen:%d time:%f\r\n",
      this->steps_event_count, this->nominal_rate, this->nominal_speed, this->millimeters, this->acceleration,
      this->accelerate_until, this->decelerate_after, this->total_move_ticks, this->initial_rate, this->maximum_rate,
      this->entry_speed, this->max_entry_speed, this->exit_speed, this->primary_axis, this->is_ready, this->locked,
      this->is_ticking, recalculate_flag ? 1 : 0, nominal_length_flag ? 1 : 0,
...
void Block::calculate_trapezoid(float entryspeed, float exitspeed) {
  ...
  float maximum_possible_rate = sqrtf((this->steps_event_count * acceleration_per_second) +
                                      ((powf(initial_rate, 2) + powf(final_rate, 2)) / 2.0F));
  ...
  // puts "total move time: #{total_move_time}s time to accelerate: #{time_to_accelerate}, time to decelerate:
  // #{time_to_decelerate}"
  ...
  // Now we figure out the acceleration value to reach EXACTLY maximum_rate(steps/s) in EXACTLY
  // acceleration_ticks(ticks) amount of time in seconds
  float acceleration_time =
      acceleration_ticks /
      STEP_TICKER_FREQUENCY;  // This can be moved into the operation below, separated for clarity, note we need to do
                              // this instead of using time_to_accelerate(seconds) directly because
                              // time_to_accelerate(seconds) and acceleration_ticks(seconds) do not have the same value
                              // anymore due to the rounding
  float deceleration_time = deceleration_ticks / STEP_TICKER_FREQUENCY;

  float acceleration_in_steps =
      (acceleration_time > 0.0F) ? (this->maximum_rate - initial_rate) / acceleration_time : 0;
  float deceleration_in_steps = (deceleration_time > 0.0F) ? (this->maximum_rate - final_rate) / deceleration_time : 0;

  // we have a potential race condition here as we could get interrupted anywhere in the middle of this call, we need to
  // lock the updates to the blocks to get around it
  this->locked = true;
...
// this is done during planning so does not delay tick generation and step ticker can simply grab the next block during
// the interrupt
void Block::prepare(float acceleration_in_steps, float deceleration_in_steps) {
  float inv = 1.0F / this->steps_event_count;

  // Now figure out the acceleration PER TICK, this should ideally be held as a double as it's very critical to the
  // block timing steps/tick^2 was.... float acceleration_per_tick = acceleration_in_steps / STEP_TICKER_FREQUENCY_2; //
  // that is 100,000² too big for a float float deceleration_per_tick = deceleration_in_steps / STEP_TICKER_FREQUENCY_2;
  double acceleration_per_tick =
      acceleration_in_steps * fp_scale;  // this is now scaled to fit a 2.30 fixed point number
  ...
  for (uint8_t m = 0; m < n_actuators; m++) {
    ...
    this->tick_info[m].steps_per_tick =
        (int64_t)round((((double)this->initial_rate * aratio) / STEP_TICKER_FREQUENCY) *
                       STEPTICKER_FPSCALE);  // steps/sec / tick frequency to get steps per tick in 2.62 fixed point
    ...
    this->tick_info[m].plateau_rate =
        (int64_t)round(((this->maximum_rate * aratio) / STEP_TICKER_FREQUENCY) * STEPTICKER_FPSCALE);
