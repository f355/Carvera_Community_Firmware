...
int RotaryDeltaSolution::delta_calcForward(float theta1, float theta2, float theta3, float& x0, float& y0,
                                           float& z0) const {
...
void RotaryDeltaSolution::cartesian_to_actuator(const float cartesian_mm[], ActuatorCoordinates& actuator_mm) const {
  // We need to translate the Cartesian coordinates in mm to the actuator position required in mm so the stepper motor
  // functions
  ...
  float z_with_offset = cartesian_mm[Z_AXIS] + z_calc_offset;  // The delta calculation below places zero at the top.
                                                               // Subtract the Z offset to make zero at the bottom.

  int status = delta_calcAngleYZ(x0, y0, z_with_offset, alpha_theta);
  if (status == 0)
    status = delta_calcAngleYZ(x0 * cos120 + y0 * sin120, y0 * cos120 - x0 * sin120, z_with_offset,
                               beta_theta);  // rotate co-ordinates to +120 deg
  if (status == 0)
    status = delta_calcAngleYZ(x0 * cos120 - y0 * sin120, y0 * cos120 + x0 * sin120, z_with_offset,
                               gamma_theta);  // rotate co-ordinates to -120 deg
  ...
  if (status == -1) {  // something went wrong,
    ...
    // DEBUG CODE, uncomment the following to help determine what may be happening if you are trying to adapt this to
    // your own different rotary delta.
    if (debug_flag) {
  ...
  for (auto& i : options) {
    switch (i.first) {
      ...
      case 'D':
        ...
        break;
      case 'E':
        delta_z_offset = i.second;
        break;
      case 'I':
        delta_ee_offs = i.second;
        break;
      case 'H':
        tool_offset = i.second;
        break;
      case 'W':
        debug_flag = i.second != 0;
        break;
    }
  }
  init();
