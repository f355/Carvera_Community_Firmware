...
void ExperimentalDeltaSolution::cartesian_to_actuator(const float cartesian_mm[],
                                                      ActuatorCoordinates& actuator_mm) const {
...
void ExperimentalDeltaSolution::actuator_to_cartesian(const ActuatorCoordinates& actuator_mm,
                                                      float cartesian_mm[]) const {
  // unimplemented
}

float ExperimentalDeltaSolution::solve_arm(float cartesian_mm[]) const {
  return sqrtf(arm_length_squared - powf(cartesian_mm[X_AXIS] - arm_radius, 2) - powf(cartesian_mm[Y_AXIS], 2)) +
         cartesian_mm[Z_AXIS];
}
