...
// It goes over the list in both direction, every time a block is added, re-doing the math to make sure everything is
// optimal
...
void Planner::config_load() {
  ...
  this->z_junction_deviation =
      THEKERNEL->config->value(z_junction_deviation_checksum)->as_number(NAN);  // disabled by default
  this->minimum_planner_speed = THEKERNEL->config->value(minimum_planner_speed_checksum)->as_number(0.0f);
}

// Append a block to the queue, compute it's speed factors
// 2024
bool Planner::append_block(ActuatorCoordinates& actuator_pos, uint8_t n_motors, float rate_mm_s, float distance,
                           float* unit_vec, float acceleration, float s_value, bool g123, unsigned int _line)
// bool Planner::append_block( ActuatorCoordinates &actuator_pos, uint8_t n_motors, float rate_mm_s, float distance,
// float *unit_vec, float acceleration, float *s_values, int s_count, bool g123, unsigned int _line)
{
  ...
  */
  ...
  // sometimes even though there is a detectable movement it turns out there are no steps to be had from such a small
  // move
  if (!has_steps) {
    block->clear();
  ...
  // NOTE however it does not take into account independent axis, in most cartesian X and Y and Z are totally
  // independent and this allows one to stop with little to no decleration in many cases. This is particualrly bad on
  // leadscrew based systems that will skip steps.
    ...
    if (junction_deviation > 0.0F && previous_nominal_speed > 0.0F) {
      ...
      if (cos_theta <= 0.9999F) {
        ...
        if (cos_theta >= -0.9999F) {
          ...
          vmax_junction =
              std::min(vmax_junction, sqrtf(acceleration * junction_deviation * sin_theta_d2 / (1.0F - sin_theta_d2)));
        }
      }
    }
  }
