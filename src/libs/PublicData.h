...
class PublicData {
 public:
  ...
  // 1. pass in a pointer to a data storage area that the caller creates, the callee module will put the returned data
  // in that pointer
  // 2. pass in a pointer to a pointer, the callee will set that pointer to some storage the callee has control over,
  // with the requested data the version used is dependent on the target (callee) module
