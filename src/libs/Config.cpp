...
Config::Config() {
  ...
  if (fcs != NULL) this->config_sources.push_back(fcs);
}
...
// Get a list of modules, used by module "pools" that look for the "enable" keyboard to find things like
// "moduletype.modulename.enable" as the marker of a new instance of a module
...
void Config::config_cache_load(bool parse) {
  ...
  if (heap_top > cache_start) {
    THEKERNEL->streams->printf(
        "ERROR: not enough memory to load config cache "
...
void Config::config_cache_clear() {
  if (this->config_cache != NULL) {
    ...
    if (heap_top > cache_start) {
      THEKERNEL->streams->printf(
          "FATAL: heap collided with config cache "
          "(heap=0x%x, cache=0x%x)\n",
          heap_top, cache_start);
      system_reset(false);
    }
...
// Because we don't like to waste space in Flash with lengthy config parameter names, we take a checksum instead so that
// the name does not have to be stored See get_checksum
