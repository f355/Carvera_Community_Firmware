...
static void sample_isr(int chan, uint32_t value) { Adc::instance->new_sample(chan, value); }
...
void Adc::new_sample(int chan, uint32_t value) {
  ...
  if (chan < num_channels) {
    memmove(&sample_buffers[chan][0], &sample_buffers[chan][1],
            sizeof(sample_buffers[0]) - sizeof(sample_buffers[0][0]));
