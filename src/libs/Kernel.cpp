...
Kernel::Kernel() {
  ...
  // we expect ok per line now not per G code, setting this to false will return to the old (incorrect) way of ok per G
  // code
  ...
  this->add_module(this->conveyor =
                       new (AHB) Conveyor());  // must stay in AHB: shares volatile queue indices with step ISR
...
std::string Kernel::get_query_string() {
  ...
  if (running) {
    ...
    if (robot->compensationTransform)
      robot->compensationTransform(mpos, true, false);  // get inverse compensation transform

    // machine position
    n = snprintf(buf, sizeof(buf), "%1.4f,%1.4f,%1.4f", robot->from_millimeters(mpos[0]),
                 robot->from_millimeters(mpos[1]), robot->from_millimeters(mpos[2]));
    ...
    n = snprintf(buf, sizeof(buf), "%1.4f,%1.4f,%1.4f", robot->from_millimeters(std::get<X_AXIS>(pos)),
                 robot->from_millimeters(std::get<Y_AXIS>(pos)), robot->from_millimeters(std::get<Z_AXIS>(pos)));
    if (n > sizeof(buf)) n = sizeof(buf);
    str.append("|WPos:").append(buf, n);

    // n = snprintf(buf, sizeof(buf), ",%1.4f,%1.4f", robot->from_millimeters(std::get<A_AXIS>(pos)),
    // robot->from_millimeters(std::get<B_AXIS>(pos)));
    ...
    size_t n =
        snprintf(buf, sizeof(buf), "%1.4f,%1.4f,%1.4f", robot->from_millimeters(std::get<X_AXIS>(mpos)),
                 robot->from_millimeters(std::get<Y_AXIS>(mpos)), robot->from_millimeters(std::get<Z_AXIS>(mpos)));
    if (n > sizeof(buf)) n = sizeof(buf);

    str.append("|MPos:").append(buf, n);

    // n = snprintf(buf, sizeof(buf), ",%1.4f,%1.4f", robot->from_millimeters(std::get<A_AXIS>(mpos)),
    // robot->from_millimeters(std::get<B_AXIS>(mpos)));
  ...
  ok = PublicData::get_value(temperature_control_checksum, current_temperature_checksum, spindle_temperature_checksum,
                             &temp);
  if (ok) {
  ...
  ok = PublicData::get_value(temperature_control_checksum, current_temperature_checksum, power_temperature_checksum,
                             &temp);
  ...
  if (ok) {
    n = snprintf(buf, sizeof(buf), "|T:%d,%1.3f,%d,%d", tool.active_tool, tool.tool_offset, tool.target_tool,
                 tool.target_collet_type);
    if (n > sizeof(buf)) n = sizeof(buf);
    str.append(buf, n);
  }
  ...
  if (PublicData::get_value(laser_checksum, get_laser_status_checksum, &ls)) {
    n = snprintf(buf, sizeof(buf), "|L:%d, %d, %d, %1.1f,%1.1f", int(ls.mode), int(ls.state), int(ls.testing), ls.power,
                 ls.scale);
    if (n > sizeof(buf)) n = sizeof(buf);
    str.append(buf, n);
  }
  ...
  if (ok) {
    ...
    n = snprintf(buf, sizeof(buf), "|P:%lu,%d,%lu,%d,%lu", p.played_lines, p.percent_complete, p.elapsed_secs,
                 p.is_playing ? 1 : 0, p.parsed_lines);
    if (n > sizeof(buf)) n = sizeof(buf);
    str.append(buf, n);
  }
  ...
  if (!is_grbl_mode()) {
    ...
    if (ok) {
      ...
      for (auto& c : controllers) {
        size_t n = snprintf(buf, sizeof(buf), "|%s:%1.1f,%1.1f", c.designator.c_str(), c.current_temperature,
  ...
  // machine state
  n = snprintf(buf, sizeof(buf), "|C:%d,%d,%d,%d", THEKERNEL->factory_set->MachineModel,
...
void Kernel::register_for_event(_EVENT_ENUM id_event, Module* mod) { this->hooks[id_event].push_back(mod); }
...
void Kernel::write_eeprom_data() {
  ...
  while (writenum < size) {
    bytenum = (size - pagenum * EEP_MAX_PAGE_SIZE) >= EEP_MAX_PAGE_SIZE ? EEP_MAX_PAGE_SIZE
                                                                        : size - pagenum * EEP_MAX_PAGE_SIZE;
...
void Kernel::check_eeprom_data() {
  ...
  if (needrewtite) this->write_eeprom_data();
}
...
void Kernel::write_Factory_data() {
  ...
  while (writenum < datalen) {
    bytenum = (datalen - pagenum * EEP_MAX_PAGE_SIZE) >= EEP_MAX_PAGE_SIZE ? EEP_MAX_PAGE_SIZE
                                                                           : datalen - pagenum * EEP_MAX_PAGE_SIZE;
    result = iic_page_write(u8Pagebegin + pagenum, bytenum, (unsigned char*)writeptr);
    wait(0.1);
    if (result == 0) {
      pagenum++;
