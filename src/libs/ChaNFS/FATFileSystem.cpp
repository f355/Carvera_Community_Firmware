...
DWORD get_fattime(void) {
  ...
  DWORD fat_time = ((timeinfo->tm_year - 80) << 25) | ((timeinfo->tm_mon + 1) << 21) | (timeinfo->tm_mday << 16) |
...
#if FFSDEBUG_ENABLED
static const char* FR_ERRORS[] = {"FR_OK = 0",         "FR_NOT_READY",       "FR_NO_FILE",     "FR_NO_PATH",
...
int FATFileSystem::format() {
  FFSDEBUG("format()\n");
  FRESULT res =
      f_mkfs(_fsid, 0, 512);  // Logical drive number, Partitioning rule, Allocation unit size (bytes per cluster)
  if (res) {
