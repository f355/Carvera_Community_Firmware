...
uint16_t get_checksum(const string& to_check) { return get_checksum(to_check.c_str()); }
...
void safe_delay_ms(uint32_t delay) { safe_delay_us(delay * 1000); }
...
// trim from left
void ltrim(std::string& s, const char* t) { s.erase(0, s.find_first_not_of(t)); }
