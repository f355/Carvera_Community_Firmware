...
{
    const size_t payload_size = size == 0 ? strlen(s) : size;
    if (payload_size >= 0x201) return;
	
    unsigned char* frame =
        static_cast<unsigned char*>(malloc(payload_size + 9));
    ...
    puts(reinterpret_cast<char*>(frame), payload_size + 9);
    free(frame);
}
