...
class PublicDataRequest {
 public:
  PublicDataRequest(uint16_t addrcs1) {
    target[0] = addrcs1;
    target[1] = 0;
    target[2] = 0;
    data_taken = false;
  ...
  PublicDataRequest(uint16_t addrcs1, uint16_t addrcs2) {
    target[0] = addrcs1;
    target[1] = addrcs2;
    target[2] = 0;
    data_taken = false;
  ...
  PublicDataRequest(uint16_t addrcs1, uint16_t addrcs2, uint16_t addrcs3) {
    target[0] = addrcs1;
    target[1] = addrcs2;
    target[2] = addrcs3;
    data_taken = false;
  ...
  void set_data_ptr(void* d, bool flag = true) {
    this->data = d;
    returned_data = flag;
  }
 ...
 private:
  ...
  struct {
    bool data_taken : 1;
    bool returned_data : 1;  // this is set if the callee returns the data, it is false if the caller supplied the
                             // storage for the return
  };
};
