...
#define SET_STEPTICKER_DEBUG_PIN(n) \
  {                                 \
    if (n)                          \
      stepticker_debug_pin.set();   \
    else                            \
      stepticker_debug_pin.clear(); \
  }
#else
#define SET_STEPTICKER_DEBUG_PIN(n)
#endif
...
void StepTicker::set_unstep_time(float microseconds) {
  uint32_t delay = floorf((SystemCoreClock / 4.0F) *
                          (microseconds / 1000000.0F));  // SystemCoreClock/4 = Timer increments in a second
  LPC_TIM1->MR0 = delay;
...
// slightly lower priority than TIMER0, the whole end of block/start of block is done here allowing the timer to
// continue ticking
...
void StepTicker::step_tick(void) {
  ...
  for (uint8_t m = 0; m < num_motors; m++) {
    ...
    if (current_tick == current_block->tick_info[m].next_accel_event) {
      if (current_tick ==
          current_block->accelerate_until) {  // We are done accelerating, deceleration becomes 0 : plateau
    ...
    if (current_block->tick_info[m].counter >= STEPTICKER_FPSCALE) {  // >= 1.0 step time
      ...
      // step the motor
      bool ismoving =
          motor[m]->step();  // returns false if the moving flag was set to false externally (probes, endstops etc)
