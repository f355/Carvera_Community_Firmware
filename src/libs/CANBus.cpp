...
CANBus::CANBus(PinName rd, PinName td)
    : can(rd, td), controller(valid_can2_pins(rd, td) ? LPC_CAN2 : nullptr),
      rx_head(0), rx_tail(0), rx_count(0), rx_overflow_count(0), tx_count(0),
      tx_failed_count(0), tx_timeout_count(0), configured_bitrate(0), ready(false)
{
}
...
{
    if(!ready || message.len > 8 || message.format != CANStandard ||
       message.type != CANData || message.id > 0x7ff || controller == nullptr) return false;
...
void CANBus::on_receive()
{
    ...
    while(can.read(message)) {
        if(message.format != CANStandard || message.type != CANData ||
           message.id > 0x7ff || !relevant_canopen_id(message.id)) {
            continue;
        }
...
{
    return Statistics{
        tx_count, rx_count, rx_overflow_count, tx_failed_count, tx_timeout_count,
        can.tderror(), can.rderror()
    };
}
