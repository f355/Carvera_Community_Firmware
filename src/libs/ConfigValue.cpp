...
float ConfigValue::as_number() {
  ...
  if (endptr <= cp) {
    reportConfigError(
        "config setting with value '%s' and checksums[%04X,%04X,%04X] is not a valid number, please see "
...
int ConfigValue::as_int() {
  ...
  if (endptr <= cp) {
    reportConfigError(
        "config setting with value '%s' and checksums[%04X,%04X,%04X] is not a valid int, please see "
        "http://smoothieware.org/configuring-smoothie\r\n",
        this->value, this->check_sums[0], this->check_sums[1], this->check_sums[2]);
  }
  return result;
...
std::string ConfigValue::as_string() { return string(this->value); }
...
float ConfigValue::as_number(float dflt) { return this->found() ? this->as_number() : dflt; }

int ConfigValue::as_int(int dflt) { return this->found() ? this->as_int() : dflt; }

bool ConfigValue::as_bool(bool dflt) { return this->found() ? this->as_bool() : dflt; }

string ConfigValue::as_string(const string& dflt) { return this->found() ? this->as_string() : dflt; }

bool ConfigValue::has_characters(const char* mask) { return strpbrk(this->value, mask) != nullptr; }

bool ConfigValue::is_inverted() { return this->has_characters("!"); }
