  ...
  do {
    ...
    if (!p_is_used && (p_block_size >= nsize)) {  // we found a free space that's big enough
      ...
      if (!pool_check(&p->data, POOL_FREE_PATTERN, free_payload_size)) {
        handle_pool_error("Free block payload corrupted before alloc! Block at %p (%lu bytes)", p, p_block_size);
        return NULL;
      }
...
void MemoryPool::dealloc(void* d) {
  ...
  if (!has(d)) {
    handle_pool_error("Attempt to dealloc pointer %p outside pool bounds [%p, %p)!", d, base, (uint8_t*)base + size);
    return;
  }
...
bool MemoryPool::has(void* p) { return ((p >= base) && (p < (void*)(((uint8_t*)base) + size))); }
...
uint32_t MemoryPool::free() {
  ...
  while (current_offset < size) {
    ...
    if (p_block_size < sizeof(_poolregion)) {
      pool_printf(
          "POOL WARNING: Invalid block size %u (less than header) detected in free() at offset %lu. "
          "Aborting count.\n",
          (unsigned int)p_block_size, current_offset);
#if POOL_DEBUG_ENABLED == 1
      pool_break();
...
static bool check_pattern32(const void* src, uint32_t pattern, size_t size) {
  if (src == nullptr || size == 0) return true;
  ...
  for (size_t i = 0; i < count; ++i) {
    if (p[i] != pattern) {
      printf("CORRUPTION DETECTED: Expected pattern 0x%08lX, found 0x%08lX at offset %lu within block %p\n", pattern,
             p[i], (unsigned long)(i * sizeof(uint32_t)), (const void*)src);
      return false;
    }
  }
  // --- MODIFICATION START ---
  ...
  if (remainder > 0) {
    ...
    for (size_t i = 0; i < remainder; ++i) {
      if (byte_ptr[i] != pattern_bytes[i]) {
        printf("CORRUPTION DETECTED: Expected pattern byte 0x%02X, found 0x%02X at offset %lu within block %p\n",
...
static bool validate_pool_integrity_internal(const MemoryPool* pool, bool check_free_pattern) {
  ...
  if (current_offset != pool->getSize() && ok) {
    pool_printf(
        "  ERROR: Pool validation walk ended prematurely at offset %lu (Pool Size %u). Likely due to prior "
        "error.\n",
