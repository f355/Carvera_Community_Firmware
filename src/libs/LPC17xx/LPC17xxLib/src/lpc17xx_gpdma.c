...
/********************************************************************/ /**
                                                                        ...
                                                                        * @return		ERROR if selected
                                                                        * channel is enabled before or SUCCESS if
                                                                        * channel is configured successfully
                                                                        *********************************************************************/
  ...
  switch (GPDMAChannelConfig->TransferType) {
    ...
    case GPDMA_TRANSFERTYPE_M2M:
      ...
      pDMAch->DMACCControl = GPDMA_DMACCxControl_TransferSize(GPDMAChannelConfig->TransferSize) |
                             ...
                             GPDMA_DMACCxControl_DWidth(GPDMAChannelConfig->TransferWidth) | GPDMA_DMACCxControl_SI |
    ...
    case GPDMA_TRANSFERTYPE_M2P:
      ...
      pDMAch->DMACCControl = GPDMA_DMACCxControl_TransferSize((uint32_t)GPDMAChannelConfig->TransferSize) |
...
/*********************************************************************/ /**
                                                                         * @brief		Check if corresponding
                                                                         * channel does have an active interrupt request
                                                                         * or not
                                                                         * @param[in]	type		type of status,
                                                                         * should be:
                                                                         * 					-
                                                                         * GPDMA_STAT_INT: 		GPDMA Interrupt
                                                                         * Status
                                                                         * 					-
                                                                         * GPDMA_STAT_INTTC: 	GPDMA Interrupt Terminal
                                                                         * Count Request Status
                                                                         * 					-
                                                                         * GPDMA_STAT_INTERR:	GPDMA Interrupt Error
                                                                         * Status
                                                                         * 					-
                                                                         * GPDMA_STAT_RAWINTTC:	GPDMA Raw Interrupt
                                                                         * Terminal Count Status
                                                                         * 					-
                                                                         * GPDMA_STAT_RAWINTERR:	GPDMA Raw Error
                                                                         * Interrupt Status
                                                                         * 					-
                                                                         ...
                                                                         * of DMA channel interrupt after masking Should
                                                                         * be:
                                                                         * 					- SET:
                                                                         * the corresponding channel has no active
                                                                         * interrupt request
                                                                         * 					- RESET:
                                                                         * the corresponding channel does have an active
                                                                         * interrupt request
                                                                         **********************************************************************/
IntStatus GPDMA_IntGetStatus(GPDMA_Status_Type type, uint8_t channel) {
  ...
  switch (type) {
    case GPDMA_STAT_INT:  // check status of DMA channel interrupts
      if (LPC_GPDMA->DMACIntStat & (GPDMA_DMACIntStat_Ch(channel))) return SET;
      return RESET;
    case GPDMA_STAT_INTTC:  // check terminal count interrupt request status for DMA
      if (LPC_GPDMA->DMACIntTCStat & GPDMA_DMACIntTCStat_Ch(channel)) return SET;
      return RESET;
    case GPDMA_STAT_INTERR:  // check interrupt status for DMA channels
      if (LPC_GPDMA->DMACIntErrStat & GPDMA_DMACIntTCClear_Ch(channel)) return SET;
      return RESET;
    case GPDMA_STAT_RAWINTTC:  // check status of the terminal count interrupt for DMA channels
      if (LPC_GPDMA->DMACRawIntErrStat & GPDMA_DMACRawIntTCStat_Ch(channel)) return SET;
      return RESET;
    case GPDMA_STAT_RAWINTERR:  // check status of the error interrupt for DMA channels
      if (LPC_GPDMA->DMACRawIntTCStat & GPDMA_DMACRawIntErrStat_Ch(channel)) return SET;
      return RESET;
    default:  // check enable status for DMA channels
      if (LPC_GPDMA->DMACEnbldChns & GPDMA_DMACEnbldChns_Ch(channel)) return SET;
      return RESET;
  }
}

/*********************************************************************/ /**
                                                                         * @brief		Clear one or more
                                                                         * interrupt requests on DMA channels
                                                                         * @param[in]	type		type of
                                                                         * interrupt request, should be:
                                                                         * 					-
                                                                         * GPDMA_STATCLR_INTTC: 	GPDMA Interrupt
