...
#define MCPWM_CON_CENTER(n) \
  ((n <= 2) ? ((uint32_t)(1UL << ((n * 8) + 1))) : (0)) /**< Edge/center aligned operation for channel n */
#define MCPWM_CON_POLAR(n) \
  ((n <= 2) ? ((uint32_t)(1UL << ((n * 8) + 2))) : (0)) /**< Select polarity of the MCOAn and MCOBn pin */
#define MCPWM_CON_DTE(n) \
  ((n <= 2) ? ((uint32_t)(1UL << ((n * 8) + 3))) : (0)) /**< Control the dead-time feature for channel n */
#define MCPWM_CON_DISUP(n)                       \
  ((n <= 2) ? ((uint32_t)(1UL << ((n * 8) + 4))) \
...
typedef struct {
  ...
  uint32_t channelPolarity;          /**< Polarity of the MCOA and MCOB pins, should be:
                                                                     - MCPWM_CHANNEL_PASSIVE_LO: Passive state is LOW, active
                                        state is HIGH
