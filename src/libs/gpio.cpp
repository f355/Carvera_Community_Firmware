...
void GPIO::set_direction(uint8_t direction) { FIO_SetDir(port, 1UL << pin, direction); }

void GPIO::output() { set_direction(1); }

void GPIO::input() { set_direction(0); }
...
uint8_t GPIO::get() { return (FIO_ReadValue(port) & (1UL << pin)) ? 255 : 0; }
