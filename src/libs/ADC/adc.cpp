...
ADC::ADC(int sample_rate, int cclk_div) {
  ...
  LPC_ADC->ADCR = ((clock_div - 1) << 8) |  // Clkdiv
...
void ADC::_adcisr(void) { instance->adcisr(); }
...
void ADC::adcisr(void) {
  ...
  if (_adc_isr[chan] != NULL) _adc_isr[chan](_adc_data[chan]);
  if (_adc_g_isr != NULL) _adc_g_isr(chan, _adc_data[chan]);
  return;
}
...
PinName ADC::channel_to_pin(int chan) {
  ...
  if ((chan < 0) || (chan > 5)) fprintf(stderr, "ADC channel %u is outside range available to MBED pins.\n", chan);
...
void ADC::burst(int state) {
  if ((state & 1) == 1) {
    if (startmode(0) != 0) fprintf(stderr, "ADC Warning. startmode is %u. Must be 0 for burst mode.\n", startmode(0));
    LPC_ADC->ADCR |= (1 << 16);
  } else
    LPC_ADC->ADCR &= ~(1 << 16);
}
// Return burst mode state
int ADC::burst(void) { return ((LPC_ADC->ADCR & (1 << 16)) >> 16); }
...
void ADC::start(void) { startmode(1, 0); }
...
void ADC::interrupt_state(PinName pin, int state) {
  ...
  if (state == 1) {
    ...
    if ((LPC_ADC->ADINTEN & 0xFF) == 0) NVIC_DisableIRQ(ADC_IRQn);
  }
}
...
void ADC::append(void (*fptr)(int chan, uint32_t value)) { _adc_g_isr = fptr; }

// Detach global interrupt handler to function isr
void ADC::unappend() { _adc_g_isr = NULL; }
...
int ADC::offset(void) { return ((LPC_ADC->ADTRM >> 4) & 0x07); }
...
int ADC::done(PinName pin) { return ((_data_of_pin(pin) >> 31) & 0x01); }

// Return OVERRUN flag of ADC on pin
int ADC::overrun(PinName pin) { return ((_data_of_pin(pin) >> 30) & 0x01); }

int ADC::actual_adc_clock(void) { return (_adc_clk_freq); }

int ADC::actual_sample_rate(void) { return (_adc_clk_freq / CLKS_PER_SAMPLE); }
