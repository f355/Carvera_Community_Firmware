...
class TSRingBuffer {
 public:
  TSRingBuffer() : m_size(RingSize), m_buffer(new T[RingSize]), m_rIndex(0), m_wIndex(0) {}

  ~TSRingBuffer() { delete[] m_buffer; };

  size_t next(size_t n) const { return (n + 1) % m_size; }

  bool empty() const { return (m_rIndex == m_wIndex); }

  bool full() const { return (next(m_wIndex) == m_rIndex); }
