...
SDFileSystem::SDFileSystem(PinName mosi, PinName miso, PinName sclk, PinName cs, int hz)
    : m_Spi(mosi, miso, sclk), m_Cs(cs), m_Cd(NC), m_FREQ(hz) {
  m_Cs.output();
  m_Cs = 1;
...
int SDFileSystem::disk_initialize() {
  ...
  checkSocket();
  if (m_Status & STA_NODISK) return m_Status;

  // Make sure we're not already initialized before proceeding
  if (!(m_Status & STA_NOINIT)) return m_Status;
...
int SDFileSystem::disk_read(char* buffer, uint32_t sector, uint32_t count) {
  ...
  if (m_Status & STA_NOINIT) return RES_NOTRDY;
...
int SDFileSystem::disk_write(const char* buffer, uint32_t sector, uint32_t count) {
  ...
  if (m_Status & STA_PROTECT) return RES_WRPRT;
...
uint32_t SDFileSystem::disk_sectors() {
  // Make sure the card is initialized before proceeding
  if (m_Status & STA_NOINIT) return 0;
...
bool SDFileSystem::busy() { return false; }
...
char SDFileSystem::writeCommand(char cmd, unsigned int arg, unsigned int* resp) {
  ...
  for (int f = 0; f < 3; f++) {
    ...
    for (int i = 0; i < 6; i++) m_Spi.write(cmdPacket[i]);

    // Discard the stuff byte immediately following CMD12
    if (cmd == CMD12) m_Spi.write(0xFF);
...
bool SDFileSystem::readData(char* buffer, int length) {
  ...
  if (m_LargeFrames) {
    ...
    for (int i = 0; i < length; i++) buffer[i] = m_Spi.write(0xFF);
...
char SDFileSystem::writeData(const char* buffer, char token) {
  ...
  if (m_LargeFrames) {
    ...
    for (int i = 0; i < 512; i += 2) m_Spi.write((buffer[i] << 8) | buffer[i + 1]);
    ...
    // Write the data block from the buffer
    for (int i = 0; i < 512; i++) m_Spi.write(buffer[i]);
