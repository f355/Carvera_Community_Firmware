...
void Host_DelayUS(USB_INT32U delay) { wait_us((int)delay); }
...
USB_INT32S Host_ResetRootPort(USB_INT32U timeout_ms, HostPortResetStatus* status) {
  ...
  if (timeout_ms == 0) {
    while (LPC_USB->HcRhPortStatus1 & OR_RH_PORT_PRS) __WFI();
  } else {
...
USB_INT32S Host_ReadDeviceDescriptor(void) {
  ...
  USB_INT32S rc =
      HOST_GET_DESCRIPTOR(USB_DESCRIPTOR_TYPE_DEVICE, 0, TDBuffer, USB_ENDPOINT_ZERO_INITIAL_DESCRIPTOR_BYTES);
  if (rc == OK) {
...
USB_INT32S Host_ReadConfigurationDescriptor(USB_INT16U max_len, USB_INT16U* fetched_len) {
  USB_INT32S rc =
      HOST_GET_DESCRIPTOR(USB_DESCRIPTOR_TYPE_CONFIGURATION, 0, TDBuffer, USB_CONFIGURATION_DESCRIPTOR_HEADER_SIZE);
  if (rc != OK) {
    return (rc);
  }
...
USB_INT32U ReadLE32U(volatile USB_INT08U* pmem) {
  return ((USB_INT32U)pmem[0]) | ((USB_INT32U)pmem[1] << 8) | ((USB_INT32U)pmem[2] << 16) | ((USB_INT32U)pmem[3] << 24);
}
...
USB_INT16U ReadLE16U(volatile USB_INT08U* pmem) {
  return (USB_INT16U)(((USB_INT16U)pmem[0]) | ((USB_INT16U)pmem[1] << 8));
}
...
USB_INT32U ReadBE32U(volatile USB_INT08U* pmem) {
  return ((USB_INT32U)pmem[0] << 24) | ((USB_INT32U)pmem[1] << 16) | ((USB_INT32U)pmem[2] << 8) | ((USB_INT32U)pmem[3]);
}
...
USB_INT16U ReadBE16U(volatile USB_INT08U* pmem) {
  return (USB_INT16U)(((USB_INT16U)pmem[0] << 8) | ((USB_INT16U)pmem[1]));
}
