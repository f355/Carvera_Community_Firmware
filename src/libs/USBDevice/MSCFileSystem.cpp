...
void print_inquiry(USB_INT08U* inqReply) {
  ...
  if (tmp != 0) printf("\t- warning! must be 0\n");
  tmp = (inqReply[2] >> 3) & 3;
  printf("ECMA Version: %02Xh\n", tmp);
  if (tmp != 0) printf("\t- warning! should be 0\n");
  tmp = inqReply[2] >> 6;
  printf("ISO Version: %02Xh\n", tmp);
  if (tmp != 0) printf("\t- warning! should be 0\n");
  tmp = inqReply[3] & 0xF;
  printf("Response Data Format: %02Xh\n", tmp);
  if (tmp != 1) printf("\t- warning! should be 1\n");
  tmp = inqReply[4];
  printf("Additional length: %02Xh\n", tmp);
  if (tmp != 0x1F) printf("\t- warning! should be 1Fh\n");
...
int MSCFileSystem::disk_initialize() {
  if (initialise_msc() != OK) return 1;

  return 0;
}

int MSCFileSystem::disk_write(const char* buffer, uint32_t block_number, uint32_t count) {
  if (OK == MS_BulkSend(block_number, 1, reinterpret_cast<const volatile USB_INT08U*>(buffer))) return 0;
  return 1;
}

int MSCFileSystem::disk_read(char* buffer, uint32_t block_number, uint32_t count) {
  if (OK == MS_BulkRecv(block_number, 1, (USB_INT08U*)buffer)) return 0;
  return 1;
}
