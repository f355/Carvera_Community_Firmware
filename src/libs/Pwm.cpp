...
void Pwm::pwm(int new_pwm) { _pwm = confine(new_pwm, 0, _max); }
...
uint32_t Pwm::on_tick(uint32_t dummy) {
   ...
   *  145 -181   1
   *  -36   75   0
   * etcetera. This pattern has 6 '1's over a total of 21 lines which is on 28.57% of the time. If we let it run longer,
   * it would get closer to the target as time went on
   */

  // this line should never actually do anything, it's just a sanity check in case our accumulator gets corrupted
  // somehow. If we didn't check and the accumulator is corrupted, we could leave a heater on for quite a long time the
  // accumulator is kept within these limits by the normal operation of the Sigma-Delta algorithm
  ...
  if (_sd_direction == false) {
    ...
    if (_sd_accumulator >= (PID_PWM_MAX >> 1)) _sd_direction = true;
  }
  // when _sd_direction == true, our output is 1 and our accumulator is decreasing by (MAX - _pwm)
    ...
    if (_sd_accumulator <= 0) _sd_direction = false;
  }
