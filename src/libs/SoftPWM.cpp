...
SoftPWM::SoftPWM(Pin* _outpin, bool _positive)
    : pulse(_outpin)  // constructa
{
  pulse->set(!_positive);
  positive = _positive;
  interval = 0.02F;
...
void SoftPWM::start() { _ticker.attach(this, &SoftPWM::TickerInterrapt, interval); }
...
void SoftPWM::pulsewidth_ms(int _width) { pulsewidth((float)_width / 1000); }

void SoftPWM::pulsewidth_us(int _width) { pulsewidth((float)_width / 1000000); }
...
void SoftPWM::end() { pulse->set(!positive); }
