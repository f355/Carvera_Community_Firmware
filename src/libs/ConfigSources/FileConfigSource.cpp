    ...
    if (readLine(line, ln++, lp)) {
      ...
      if (cv->check_sums[0] == include_checksum) {
        ...
        if (!file_exists(inc_file_name)) {
          ...
          else if (file_exists("/sd" + inc_file_name))
            inc_file_name = "/sd" + inc_file_name;
          else if (file_exists("/local" + inc_file_name))
            inc_file_name = "/local" + inc_file_name;
        }
        if (file_exists(inc_file_name)) {
...
bool FileConfigSource::is_named(uint16_t check_sum) { return check_sum == this->name_checksum; }
