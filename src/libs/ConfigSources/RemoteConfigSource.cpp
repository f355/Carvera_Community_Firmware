...
constexpr remote::PacketTypes config_packets{PTYPE_CONFIG_START,
                                             PTYPE_CONFIG_VIEW,
                                             PTYPE_CONFIG_DATA,
                                             PTYPE_CONFIG_FINISH,
                                             PTYPE_CONFIG_CANCEL,
...
void RemoteConfigSource::transfer_values_to_cache(ConfigCache* cache) {
  ...
  if (result != remote::Result::success) {
    THEKERNEL->streams->printf("ERROR: remote configuration transfer failed (%u); using embedded profile\n",
                               static_cast<unsigned>(result));
    THEKERNEL->set_config_load_error(true);
  }
}

uint32_t RemoteConfigSource::accept(uint32_t, const uint8_t* data, std::size_t size) {
  if (active_cache_ == nullptr || data == nullptr || size == 0 || size > remote::config_record_payload_size) {
    return 0;
  }
