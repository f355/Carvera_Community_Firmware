...
float Vector3::dot(const Vector3& vec) const {
  return elem[0] * vec.elem[0] + elem[1] * vec.elem[1] + elem[2] * vec.elem[2];
}

float Vector3::magsq() const { return powf(elem[0], 2) + powf(elem[1], 2) + powf(elem[2], 2); }

float Vector3::mag() const { return sqrtf(magsq()); }
