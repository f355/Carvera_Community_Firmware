...
#define TO_S_E_DELTA_F(expected, actual, delta)                                                          \
  StringFrom("Expected : ") + StringFrom(expected) + StringFrom(" but Actual : ") + StringFrom(actual) + \
      StringFrom(" with delta = ") + StringFrom(delta)
...
#define ASSERT_TRUE(condition)                                                              \
  {                                                                                         \
    if (condition) {                                                                        \
      ...
      addTestPartResult(new TestPartResult(this, __FILE__, __LINE__, #condition, failure)); \
      return;                                                                               \
    }                                                                                       \
  }
...
#define ASSERT_TRUE_M(condition, message)                                                   \
  {                                                                                         \
    if (condition) {                                                                        \
      ...
      addTestPartResult(new TestPartResult(this, __FILE__, __LINE__, message, failure));    \
      return;                                                                               \
    }                                                                                       \
  }
...
#define ASSERT_EQUALS(expected, actual)                                                                   \
  {                                                                                                       \
    if (expected == actual) {                                                                             \
      addTestPartResult(                                                                                  \
          new TestPartResult(this, __FILE__, __LINE__, TO_STRING_EQUALS_S(#expected, #actual), success)); \
    } else {                                                                                              \
      addTestPartResult(                                                                                  \
          new TestPartResult(this, __FILE__, __LINE__, TO_STRING_EQUALS_F(#expected, #actual), failure)); \
      return;                                                                                             \
    }                                                                                                     \
  }
...
#define ASSERT_EQUALS_V(expected, actual)                                                                             \
  {                                                                                                                   \
    if (expected == actual) {                                                                                         \
      ...
      addTestPartResult(new TestPartResult(this, __FILE__, __LINE__, TO_STRING_EQUALS_F(expected, actual), failure)); \
      return;                                                                                                         \
    }                                                                                                                 \
  }
...
#define ASSERT_EQUALS_DELTA(expected, actual, delta)                                                          \
  {                                                                                                           \
    if (EQUALS_DELTA(expected, actual, delta)) {                                                              \
      addTestPartResult(                                                                                      \
          new TestPartResult(this, __FILE__, __LINE__, TO_S_E_DELTA_S(#expected, #actual, #delta), success)); \
    } else {                                                                                                  \
      addTestPartResult(                                                                                      \
          new TestPartResult(this, __FILE__, __LINE__, TO_S_E_DELTA_F(#expected, #actual, #delta), failure)); \
      return;                                                                                                 \
    }                                                                                                         \
  }
...
#define FAIL()                                                                                  \
  {                                                                                             \
    addTestPartResult(new TestPartResult(this, __FILE__, __LINE__, ("Test failed."), failure)); \
    return;                                                                                     \
  }
...
#define FAIL_M(text)                                                                \
  {                                                                                 \
    addTestPartResult(new TestPartResult(this, __FILE__, __LINE__, text, failure)); \
    return;                                                                         \
  }
...
#define TEST(testCaseName, testName)                                   \
  class testCaseName##testName##Test : public Test {                   \
   public:                                                             \
    testCaseName##testName##Test() : Test(#testCaseName, #testName) {} \
    void run();                                                        \
...
#define TESTF(testCaseName, testName)                                                         \
  class testCaseName##testName##Test : public testCaseName##Declare##Test {                   \
   public:                                                                                    \
    testCaseName##testName##Test() : testCaseName##Declare##Test(#testCaseName, #testName) {} \
    void run();                                                                               \
  } testCaseName##testName##Instance;                                                         \
  void testCaseName##testName##Test::run()
...
#define DECLARE(testCaseName)                                                                   \
  class testCaseName##Declare##Test : public Test {                                             \
   public:                                                                                      \
    testCaseName##Declare##Test(const SimpleString& testCaseName, const SimpleString& testName) \
