...
void DefaultTestPrinter::setHeaderLevel(headerLevel level) { level_ = level; }

void DefaultTestPrinter::showSuccessDetail(bool show) { showSuccessDetail_ = show; }

void DefaultTestPrinter::setOutput(FILE* output) { output_ = output; }
...
void DefaultTestPrinter::printResults(Test* test) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (type == failure) {
      fprintf(output_, "%s%s%s%s%s%ld%s%s\n", indent, "Failure: \"", testPR->getMessage().asCharString(), "\" ",
              "line ", testPR->getLineNumber(), " in ", testPR->getFileName().asCharString());
    } else if (type == error) {
      fprintf(output_, "%s%s%s%s%s%s\n", indent, "Error in ", test->getTestName().asCharString(), ": \"",
              testPR->getMessage().asCharString(), "\"");
    } else if (type == success && showSuccessDetail_) {
      fprintf(output_, "%s%s%s%s%s%ld%s%s\n", indent, "Success: \"", testPR->getMessage().asCharString(), "\" ",
              "line ", testPR->getLineNumber(), " in ", testPR->getFileName().asCharString());
    }
